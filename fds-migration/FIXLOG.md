...
| F29 | `alignItems: flex-end` rows holding a field | **Raised by review, ASSESSED — no change made.** 12 sites align a field to a neighbouring control by its bottom edge. The defect this could cause comes from MISMATCHED heights, and the flip removed the mismatch: MUI fields were ~30px (standard) beside 36px controls and are now 37px, so bottoms line up within 1px. Not exhaustively verified site by site — stated so Sandy can call it rather than have it read as checked. | d | ACCEPTED (named) |
...
| **#18001** | `fds/night4-rightnav` | right-bar redesign + content switcher md/centred + the field-bearing bars' layer — **now stacked on #18019** (it uses `utils/fdsLayer.ts`) | F1, part of F5 |
...
| **#18023** | `fds/night6-alignment` | the whole filter bar on one line, both list engines; the clear-filters control joins its toolbar — **stacked on #18021** | F13, F19, F20 |
