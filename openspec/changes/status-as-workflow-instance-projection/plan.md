...
- [x] **Step 1.1: Add `order` field to workflow state schema/types**
...
- [x] **Step 1.2: Write failing unit tests for topological ordering**
...
- [x] **Part A — implement before Task 3, no dependency on it:**
- [x] **Step 1.1: Write failing unit test for projection function**
...
- [x] **Step 1.2: Implement `projectWorkflowState`** in `workflow-projection.ts` using the existing `updateAttribute` middleware (same one used elsewhere in `workflow-domain.ts`).
- [x] **Step 1.3: Run test to verify pass**
- [x] **Part B — implement after Task 3 is complete (needs its `WorkflowInstance.scope` field, Step 5, and its creation/backfill hooks):**
- [x] **Step 0.1: Write failing test — `WorkflowInstance.scope` already exists** (implemented by Task 3, Step 5): `projectWorkflowState`'s callers read `instance.scope` without re-deriving it.
- [x] **Step 0.2: Run test to verify pass**
- [x] **Step 2.1: Write failing test — sync transition calls projection after instance update**
  - Assert `updateAttribute` (instance) is called, then `projectWorkflowState`, in that order, using a mock/spy.
- [x] **Step 2.2: Wire `projectWorkflowState` into `triggerWorkflowEvent`** immediately after `history` is updated (existing code around `{ key: 'history', value: [...] }` update in `workflow-domain.ts`).
- [x] **Step 2.3: Run test to verify pass**
- [x] **Step 3.1: Write failing test — async completion calls projection**
- [x] **Step 3.2: Wire `projectWorkflowState` into the async completion handler** (`workflow-async-completion.ts`).
- [x] **Step 3.3: Run test to verify pass**
- [x] **Step 4.1: Write failing test for read-repair**
...
- [x] **Step 1.1: Write failing test for the hook registry** — a registered hook is invoked with the created entity after `createEntity` completes; an entity type with no hooks registered is unaffected.
- [x] **Step 1.2: Implement the registry** in `entity-lifecycle-hooks.ts` and wire `runPostEntityCreationHooks` into `createEntity` right after the existing `triggerCreateEntityAutoEnrichment` call. Registration functions de-dupe by function reference so repeated registration calls are no-ops.
- [x] **Step 1.3: Register `initializeEntityWorkflow`** into the registry from `workflow-domain.ts`'s module-load path (verify no import cycle results — `workflow-domain.ts` importing the registry file is fine since the registry file imports nothing feature-specific).
- [x] **Step 1.4: Guarantee bootstrap order (per review, previously unspecified)**: do not rely on ES module import-order side effects alone. Add an explicit `registerWorkflowLifecycleHooks()` export from `workflow-domain.ts`, called once, synchronously, from the server's central bootstrap sequence (the same place GraphQL resolvers/schema are assembled before the server starts accepting connections) — before any mutation handling begins. Add a startup assertion test verifying the hook is registered prior to the first simulated `createEntity` call in the test suite, and a duplicate-registration test per the idempotency note above.
- [x] **Step 1.5: Run test to verify pass**
- [x] **Step 2.1: Write failing test — eager creation on generic entity add**
...
- [x] **Step 0: Proceed with option (b) by default** (per review: made deterministic to avoid re-litigating this each pass) — resolve workflow-status filtering at the domain-query layer via a shared, explicitly-parameterized helper, not by changing `completeSpecialFilterKeys`'s signature. Only switch to option (a) if a maintainer explicitly requests it before this task starts; otherwise implement (b) directly, no further confirmation step needed.
- [x] **Step 1.1: Write failing test** for the shared helper resolving a non-draft entity type by workflow status to an id-list filter, mirroring the existing `resolveWorkflowInstanceStatusFilter` behavior in `draftWorkspace-domain.ts`.
- [x] **Step 1.2: Extract the logic** from `resolveWorkflowInstanceStatusFilter` in `draftWorkspace-domain.ts` into the shared helper, generalized to accept an explicit entity type parameter (remove the draft-specific assumptions).
- [x] **Step 1.3: Wire the shared helper** into each workflow-enabled entity type's list resolver, passing its own entity type explicitly.
- [x] **Step 1.4: Update `draftWorkspace-domain.ts`** to call into the shared helper instead of its own copy.
- [x] **Step 1.5: Run tests to verify pass**
- [x] **Step 2.1: Write test — legacy entity types unaffected** (no workflow configured → filter still resolves via legacy `Status`).
- [x] **Step 2.2: Run test to verify pass**
- [x] **Step 3.1: Manual check — bounded query, not full scan**: confirm the generalized resolver still uses `first: 5000`/bounded `fullEntitiesList` calls as the current draft implementation does, not an unbounded scan.
- [x] **Step 3.2: Document the 5000-item bound's scope (per review, corrected — this affects list correctness itself, not only analytics)**: the `first: 5000` cap already exists in `draftWorkspace-domain.ts` today. Its effect is not limited to `count`/`distribution`/`timeSeries` — the filter-resolution step rewrites a status filter into a bounded id-list (at most 5000 ids), and that id-list is what actually feeds list/pagination results too. If more than 5000 entities of a given type match a given status, list filtering itself silently omits matches beyond the cap, not just widget counts. This is a pre-existing limitation this task inherits and generalizes to more entity types, same as the already-descoped analytics surfaces — call it out identically in release notes rather than treating list filtering as "safe" while analytics is "unsafe." **Prioritization note (per review, round 15):** for high-cardinality entity types (Indicators, Observables), this is a realistic support-ticket generator, not a hypothetical edge case — recommend prioritizing the search-after pagination fix ahead of enabling `ENTITIES_WORKFLOW` broadly for those specific types, rather than shipping the truncation risk widely and fixing it reactively.
- [x] **Step 4: Commit**
  ```bash
...
- [x] **Step 0.8: Run test to verify pass**
- [x] **Step 1: Add `ENTITIES_WORKFLOW` flag** following the existing feature-flag registration pattern used elsewhere in the codebase (locate one similar flag as a template).
- [x] **Step 2: Gate workflow UI rendering** on entity views behind the flag for non-`DraftWorkspace` types, reusing `WorkflowStatus`/`WorkflowTransitions` components.
- [x] **Step 3: Hide validate-draft action and skip its validation** for non-`DraftWorkspace` types (locate validate-draft action component and its validation check).
- [x] **Step 4: Hide Authorized Members actions for Container entities** and add the corresponding validation check.
- [x] **Step 4.5: Write a failing test asserting `StatusField` is disabled/hidden** on an entity edition form for an entity type that (a) has `ENTITIES_WORKFLOW` enabled and (b) has a published `WorkflowDefinition` — then make it pass by adding a guard in `StatusField.tsx` (or its call sites) that renders it read-only (showing current status only, with a pointer to the new transition UI) instead of an editable autocomplete, when both conditions hold. When either condition is false (flag off, or no published definition for that type), `StatusField` keeps its current free-choice behavior unchanged — this must not regress any non-migrated entity type. Apply this per call site across all ~40 forms (a single shared guard inside `StatusField.tsx` itself, keyed on `type`/`scope` props it already receives, is preferable to editing every call site individually).
- [x] **Step 4.6: Run test to verify pass**
- [x] **Step 5: Frontend tests** for flagged behavior per entity type category (draft, container, other).
- [x] **Step 6: Commit**
  ```bash
...
- [x] **Step 1.1: Write failing tests for the pure conversion function** covering: well-formed ordered `Status[]` (with `order` set) → valid definition with empty diagnostics; missing `order` → diagnostic + best-effort definition; name conflicts (via joined `StatusTemplate.name`) → diagnostic; **mixed `scope` values within one entity type → two separate entries in `byScope`, each independently valid/diagnosed, never merged into one definition** (per review, corrects the earlier "diagnostic only" treatment).
- [x] **Step 1.1b: Decide and document the transition-synthesis policy (per review, round 18 — a genuine, previously-unstated design gap):** legacy `Status` data has no edge/transition concept at all, only a flat `order` integer, and today's `StatusField` UI lets users jump to any status in any order (no enforcement). `convertStatusToDefinition` must synthesize `WorkflowSerializedTransition[]` (`from`/`to`/`event`) from that flat ordering, and the choice is a real behavior decision, not an implementation detail: **default to fully-connected transitions** (every state can transition to every other state) rather than a linear forward-only or forward+backward-adjacent-only chain — this exactly preserves today's unenforced, jump-anywhere behavior for migrated data, so the migration itself introduces zero new ordering restrictions. Any *narrowing* to a stricter transition graph (e.g. sequential-only) is a separate, explicit, opt-in follow-up an admin can apply after migration by editing the generated `WorkflowDefinition` — not something this migration imposes automatically. **Flag to a maintainer/PO before this task starts:** teams using statuses as unordered parallel labels (e.g. "duplicate"/"false positive" closing reasons) must not be silently forced into sequential ordering by this migration; the fully-connected default avoids that, but confirm this default matches product intent, since it directly determines how disruptive Task 5's `StatusField` lockdown (Step 4.5/4.6) will feel to existing users on day one of migration.
- [x] **Step 1.1c: Write a test asserting the fully-connected default** — given N ordered statuses with no explicit transition/edge data, the synthesized definition contains transitions allowing every state to reach every other state (not just adjacent-order pairs).
- [x] **Step 1.2: Implement `convertStatusToDefinition`** as a pure function (no context/user/store access) in `status-to-definition-converter.ts`, grouping by `scope` before converting each group independently, and synthesizing fully-connected transitions per Step 1.1b's decision.
- [x] **Step 1.3: Run tests to verify pass**
- [x] **Step 2.1: Add GraphQL preview query** `workflowMigrationPreview(entityType: String!): WorkflowMigrationPreview` returning **one result per scope present** for that entity type's current `Status` set (matching `convertStatusToDefinition`'s `byScope` shape), read-only. **RBAC (frozen per review):** gate with `@auth(for: [SETTINGS_SETCUSTOMIZATION])`, matching the existing `workflowDefinition`/`workflowDefinitionSet` pattern in `workflow.graphql` — this is a config-preview surface for admins, not a general knowledge-read query.
- [ ] **Step 2.2: Integration test** (deferred, no live-integration environment exercised in this session): query preview for a sample entity type with both global and `request_access`-scoped statuses configured; assert two distinct, independently-correct results are returned (not one merged/ambiguous definition), and no persisted changes occur.
- [x] **Step 3.1: Implement the versioned migration** reusing `convertStatusToDefinition`, following the existing migration file format (`params`, `up`/`next` function shape as in `1651939301056-workflow_rename.js`), creating one `WorkflowDefinition` **per scope** returned by `byScope` and setting the corresponding `EntitySetting.workflow_id` (standard scope). **Corrected ordering dependency (per review, round 18 — supersedes the round-16 "not dead data" note, which incorrectly assumed a scope-aware lookup mechanism already existed without Task 7):** for the `request_access` scope, this migration MUST set `RequestAccessFlow.workflow_definition_id` (the field added by Task 7 Step 1.2) to the newly-created `request_access`-scoped `WorkflowDefinition`'s id — there is no other field through which Task 2/8 can resolve it (verified: `getDefinitionData` only reads the singular `EntitySetting.workflow_id`, added scope-awareness only by Task 7 Step 1.3). **This means Task 7's Steps 1.1-1.3 (schema field + scope-aware `getWorkflowConfig`/`getDefinitionData`) must land before this migration runs for any entity type with `request_access`-scoped `Status` data** — add this as an explicit precondition check in the migration (fail loudly, not silently, if `request_access`-scoped statuses are found for a type but the `workflow_definition_id` field/routing doesn't exist yet on the running codebase). For entity types with no `request_access`-scoped `Status` data, this ordering dependency does not apply.
- [x] **Step 3.2: Integration test**: new `tests/03-integration/02-resolvers/workflow-migration-test.ts` (entity type `Incident`) seeds legacy `Status` data, asserts `workflowMigrationPreview` reflects it, runs `migrateEntityTypeStatusToWorkflowDefinition` directly and asserts the persisted `WorkflowDefinition` matches the preview, and asserts idempotency on a second run. 3/3 passing in a full-clean integration run.
- [x] **Step 3.3: Canary rollout guidance (per review)**: running this migration sets `EntitySetting.workflow_id`, which — per Global Constraints — immediately activates backend mechanics (Tasks 1-3, 6-8) for that entity type regardless of the `ENTITIES_WORKFLOW` UI flag, since backend gating is by workflow existence, not the flag. This is intentional (staged backend-then-UI rollout) but must be run per-entity-type as an explicit canary, not in bulk: migrate one low-traffic entity type first, monitor read-repair metrics (Step 4.7 of Task 2) and stream event volume for a defined observation window, then proceed to the next type. Rollback = clear `EntitySetting.workflow_id` for that type (existing `workflowDefinitionDelete`-style path), which reverts reads/writes to legacy `Status` behavior immediately since Task 2's projection/Task 8's sync both no-op without a published definition. **Rollback completeness (per review):** clearing `workflow_id` does not delete already-created `WorkflowInstance` rows, their history, or any recorded `pendingError` diagnostics for that entity type — these become inert (no longer read or written by Tasks 2/8 once ungated) rather than actively harmful, so no cleanup step is required for correctness. Any downstream consumer that had started handling `event_external`/`event_bypass` history entries simply stops receiving new ones after rollback; existing ones already delivered are not retracted, which is the expected, accepted behavior of an event stream (no retroactive un-send).
- [x] **Step 3.4: Fix the rollback/re-enable asymmetry** (implemented: new `reconcileExternalWritesOnRepublish(context, user, entityType)` in `workflow-domain.ts`, called from inside `publishWorkflowDefinition` right after the publish write, gated on `workflowDefinitionEntity.published_version` already being truthy *before* this call — i.e. this only runs on a genuine republish/re-enable, never on a type's first-ever publish) (per review, round 19 — a real data-loss scenario, not just a documentation gap):** confirmed by tracing the actual read/write paths: while `workflow_id` is cleared (rolled back), Task 5's `StatusField` read-only guard also un-gates (it's keyed on "has a published definition"), so users can once again freely edit `x_opencti_workflow_id` directly — and since Task 8's sync also no-ops without a published definition, those edits are **not** reflected into `WorkflowInstance.currentState`, which goes stale. If `workflow_id` is later re-set (re-enabling the same type), the **next read** of an affected entity triggers Task 2's read-repair, which treats `currentState` as authoritative and unconditionally overwrites `x_opencti_workflow_id` back to match it — **silently discarding the legitimate edit made during the rollback window**, with no diff/merge and no warning. This is asymmetric with Task 8's own external-write handling (which treats a direct `Status` write as authoritative, the opposite direction), and it only manifests via a plain read racing ahead of any write-triggered sync. **Fix, required before Task 6's canary rollout guidance can be trusted operationally:** implemented as a one-time reconciliation pass hooked on republish rather than on merely re-setting `workflow_id`, since the actual read/write gating (`getDefinitionData`) requires a `published_version`, not just a `workflow_id` — republishing is the true re-enable event. The pass scans all entities of `entityType` (via `fullEntitiesList`) for a truthy `x_opencti_workflow_id`, looks up the matching `WorkflowInstance` (`findWorkflowInstanceEntity`), and — for exactly those entities whose `updated_at` is strictly newer than the instance's `updated_at` — invokes Task 8's `syncWorkflowInstanceFromExternalWrite` (Status-write-wins direction) once, instead of allowing the next ordinary read to invoke Task 2's read-repair (instance-wins direction) on them; failures are caught per-entity and logged as warnings so one bad entity doesn't abort the whole republish. Entities untouched during the rollback window are unaffected either way and need no special handling. **Verified via unit tests** (`workflow-domain-test.ts`, `describe('publishWorkflowDefinition — republish reconciliation (Task 6, Step 3.4)')`, 3 tests, all passing): (1) reconciles an entity whose Status was written more recently than its stale `WorkflowInstance`; (2) does not reconcile when the instance is already at least as fresh; (3) does not scan/reconcile at all on a type's very first-ever publish. A live-integration-environment version of this test (roll back, edit `x_opencti_workflow_id` directly, re-enable, read the entity, assert the manual edit survives) remains deferred, consistent with this plan's other integration-test deferrals.
- [x] **Step 3.5: Add a throughput/latency benchmark gate** (implemented: `tests/01-unit/database/entity-lifecycle-hooks-benchmark-test.ts`, modeled on the existing `merge-filterTargetByExisting-benchmark-test.ts` precedent — a CI-enforced unit-level benchmark, not a one-off manual script) before migrating any high-cardinality type (per review, round 19 — no prior task quantifies this, only qualitatively flags "extra cost" in Global Constraints):** Task 3's `runPostEntityCreationHooks` runs synchronously inside `createEntity`'s hot path, adding one extra store round-trip per entity creation. For bulk STIX-bundle ingestion (workers/connectors creating thousands of entities per batch), this multiplies across the whole batch. **Implementation:** the benchmark exercises the real production code (`runPostEntityCreationHooks`/`registerPostEntityCreationHook` from `entity-lifecycle-hooks.ts` and `initializeEntityWorkflow` from `workflow-domain.ts`, unmocked) with only their store-level dependencies mocked (`findByType`, `storeLoadById`, `loadEntity`, `createEntity`, `createRelation`, `projectWorkflowState`) — no live worker/connector bulk-import test fixture with a reusable entity-count constant was found in this codebase to literally reuse (the only bulk-import-shaped tests are `03-integration` CSV-parser tests requiring live infra), so a full end-to-end worker-level benchmark remains deferred, consistent with this plan's other integration-test deferrals (2.7, 5.0.4, 6.5). Two tests: (1) a growth-factor gate — batch sizes 500/2000/8000 (4x each step) with a published workflow active, asserting runtime grows near-linearly (well under the 16x a quadratic regression would produce), catching an accidental per-entity full-scan regression before it hits a real ingestion pipeline; (2) a quantified per-entity overhead figure — 5000-entity batch, no-workflow vs with-workflow, asserting the fixed (mocked-store) per-entity overhead stays under a documented bound (currently ~0.013ms measured, gated at <5ms). Both passing. **Recorded results (mocked-store baseline, to be combined with real store round-trip latency when sizing an actual production canary per Step 3.3):** at 8000 entities, no-workflow ≈2.3ms vs with-workflow ≈101ms total (growth factors ≈3.0x and ≈4.0x for the two 4x size steps); at 5000 entities, per-entity overhead ≈0.013ms. This mocked-store figure is a lower bound on real overhead — before migrating a representative high-cardinality type (Indicators or Observables), the actual observed ingestion-duration delta (with real ES round trips) must still be recorded as part of that type's canary observation window (Step 3.3) alongside read-repair/event-volume metrics, not left as a purely qualitative cost callout.
- [x] **Step 4: Commit**
  ```bash
...
- [x] **Step 0: Determine per-type applicability, not a blanket skip (corrected per review, round 18 — the previous "default to skip" framing was wrong: `getWorkflowConfig`/`getDefinitionData` has no scope parameter without this task, confirmed by reading `workflow-domain.ts`, so Task 8's "generic sync already reconciles it" reasoning does not hold).** For each migrated entity type, check Task 6's `workflowMigrationPreview`/`byScope` output: if that type has **no** `request_access`-scoped `Status` data, this task's Steps 1-3 are genuinely inapplicable for that type (nothing to route) — skip only for that type, and note it in the migration's rollout checklist. If the type **does** have `request_access`-scoped `Status` data (confirmed today for `CaseRfi` via `requestAccess-domain.ts`), Steps 1.1-2.3 below are a **mandatory prerequisite**, not an optional enhancement — implement them before or alongside Task 6's migration for that type (see Task 6 Step 3.1's corrected ordering note).
- [x] **Step 1.1: Write failing test** — entity type with both a standard and `request_access` workflow definition; creating an entity within `request_access` scope initializes against the `request_access` definition.
- [x] **Step 1.2: Add `workflow_definition_id`** to the existing `RequestAccessFlow` type/schema (not a new top-level field).
- [x] **Step 1.3: Extend `getWorkflowConfig`/`getDefinitionData`** to accept a scope and look up the corresponding field.
- [x] **Step 1.4: Wire scope detection** at entity-creation time in `requestAccess-domain.ts` into `initializeEntityWorkflow`'s call to `getWorkflowConfig`. Implemented generically instead: `resolveEntityCreationScope` (`workflow-domain.ts`) derives scope from the supplied `x_opencti_workflow_id`'s own `Status.scope`, so `requestAccess-domain.ts` itself needed no changes (its existing `firstStatus.id` write already supplies everything the generic resolver needs).
- [x] **Step 1.5: Run test to verify pass**
- [x] **Step 2.1: Write failing test — fallback to standard definition** when no `workflow_definition_id` is configured in `RequestAccessFlow`.
- [x] **Step 2.2: Implement fallback** in `getWorkflowConfig`.
- [x] **Step 2.3: Run test to verify pass**
- [x] **Step 3: Commit**
  ```bash
...
- [x] **Step 0.1: Write failing test — gated by published workflow existence**
...
- [x] **Step 0.2: Write failing test — no feedback loop**
...
- [x] **Step 0.3: Add the internal-write marker option** to `updateAttribute`'s options parameter.
- [x] **Step 0.4: Extend the shared registry** (`entity-lifecycle-hooks.ts`) with `registerPostAttributeUpdateHook`/`runPostAttributeUpdateHooks`; wire `runPostAttributeUpdateHooks` into `updateAttribute`, gated on the internal-write marker being absent and the field being `x_opencti_workflow_id`.
- [x] **Step 0.5: Register `syncWorkflowInstanceFromExternalWrite`** into the registry from `workflow-domain.ts`, following the same bootstrap-order guarantee established in Task 3 (Step 1.4).
- [x] **Step 0.6: Implement the published-workflow-existence gate** as the first check inside `syncWorkflowInstanceFromExternalWrite` (reuse `getWorkflowConfig`/`getDefinitionData`'s existing no-op pattern).
- [x] **Step 0.7: Run tests to verify pass**
- [x] **Step 1.1: Write failing test** — a direct write of `x_opencti_workflow_id` (without the internal marker) to a `Status` mapped to a different state than the current `WorkflowInstance.currentState` updates the instance and appends an `event_external` history entry.
- [x] **Step 1.2: Implement `syncWorkflowInstanceFromExternalWrite`** in `workflow-domain.ts`, reusing the state↔status mapping from Task 1/2.
- [x] **Step 1.3: Verify the hook wiring from Step 0.4/0.5 reaches this function** for a real `updateAttribute` call (no direct import needed beyond the registry).
- [x] **Step 1.4: Run test to verify pass**
- [x] **Step 2.1: Write failing test — no-op when status unchanged** (direct write to the already-current mapped status records no new history entry, and is idempotent under repeated calls).
- [x] **Step 2.2: Implement the no-op guard** in `syncWorkflowInstanceFromExternalWrite`.
- [x] **Step 2.3: Run test to verify pass**
- [x] **Step 3.1: Write failing test — unmapped status write**
...
- [x] **Step 1: Add apply-transition action** to the entity view with pending/error UI states, reusing `WorkflowTransitions.tsx` patterns from `DraftWorkspace`, calling the existing `triggerWorkflowEvent`.
- [x] **Step 2: Ensure `WorkflowInstance` is created on first transition apply** if only legacy `Status` exists (reuses Task 3's `ensureWorkflowInstance`).
- [x] **Step 3: Add bypass-update popover** with the two modes (status-only vs. status+transition actions), calling the new `setWorkflowStatus` mutation.
- [x] **Step 4: Implement `setWorkflowStatus`** in `workflow-domain.ts` per the frozen contract above (no `allowedTransitions` check; `applyTransitionActions` toggles onExit/onEnter; `event_bypass` history entries). **RBAC (frozen per review):** gate the new `setWorkflowStatus` mutation with `@auth(for: [KNOWLEDGE_KNUPDATE])`, matching the existing `triggerWorkflowEvent` mutation \u2014 this is a knowledge-edit action available to any user who can already edit the entity, not an admin-only operation.
- [x] **Step 5: Frontend + backend tests** for pending/error states and both update modes, including a test that `setWorkflowStatus` does not require an allowed transition edge.
- [x] **Step 6: Commit**
  ```bash
...
- [x] **Step 1: Add the two update modes to the mass-operation background task** (reusing the `applyTransitionActions` parameter from Task 9).
- [x] **Step 2: Add the two update modes to the playbook status action** in `manipulate-knowledge-component.ts`.
- [x] **Step 3: Unit/integration tests** for mass operation and playbook mode selection.
- [x] **Step 4: Commit**
  ```bash
...
- [x] **Step 1: Implement closing-reason backend storage**, modeled on the existing comments implementation (same relationship/attachment pattern).
- [x] **Step 2: Add dedicated closing-reason UI**, separate from the comments section.
- [x] **Step 3: Wire closing-reason capture into the closing transition flow** (entities transitioning into a closing state can supply a reason).
- [x] **Step 4: Unit/frontend tests** for setting and displaying a closing reason.
- [x] **Step 5: Commit**
  ```bash
...
- [x] **Step 1: Merge the `workflowInstance` and `x_opencti_workflow_id` Status columns** into a single column definition.
- [x] **Step 2: Keep the `initial-` id fallback check only as a transitional safety net**, documented as such in a one-line comment.
- [ ] **Step 3: Merge the corresponding Status filter definitions** into a single UI filter option. Deferred (explicit decision) — see tasks.md 12.3 for the investigation: no entity type registers both filter keys today, and the clean fix requires a new `DraftWorkspace` schema attribute + migration, out of scope for this frontend-only step.
- [x] **Step 4: Update `dataTableUtils.test.tsx` and related filter tests** for the merged column/filter.
- [x] **Step 5: Commit**
  ```bash
...
- [x] **Step 1: Run the migration preview query** (Task 6) against **local dev data** (no staging/production data pulled, per explicit instruction — the local podman-run stack + full app via `nx run-many --target dev` was used instead). `workflowMigrationPreview(entityType: "Report")` returned one `GLOBAL`-scope result (3 states, fully-connected transitions, lowest-order state as `initialState`, no diagnostics); `workflowMigrationPreview(entityType: "Case-Rfi")` correctly returned a separate `REQUEST_ACCESS`-scope result, never blended with `GLOBAL`. Confirmed read-only (no persisted side effects). No PO available in this session to share results with — that part of the step is a process step for a real rollout, not something an automated session can complete.
- [x] **Step 2: Run full non-regression check** confirming sorting/filtering/widgets are unchanged for entity types without a configured workflow — done live rather than via an automated suite run, since every type in this dev DB except `DraftWorkspace` currently has `workflow_id: null` (confirmed via `entitySettings`), i.e. the whole existing dataset already exercises exclusively the legacy path today. Verified: `orderBy: x_opencti_workflow_id` sort, an `x_opencti_workflow_id` filter, and a `reportsDistribution` widget-style aggregation on `Report` (legacy `Status` data present) all return correct, unchanged legacy shapes (`status.template.name/color`, `workflowInstance: null`).
- [x] **Step 3: Verify stream/history/notification event shapes** are unchanged after projection writes — partially verified live: a freshly-created `DraftWorkspace` gets an eager `WorkflowInstance` with `lastHistoryEntry = {state, event: "initialization", timestamp}` (matches the documented shape, not a placeholder). Could not complete a full before/after event-payload diff for a *successful* transition this session: the only available real transition on a fresh instance was RBAC-condition-gated (see Step 4), and there was no legacy `Status` entity for `DraftWorkspace` in this dev DB to test the `setWorkflowStatus` bypass path to a mapped state (it correctly failed safely with a clear reason for an unmapped one, which is itself a valid regression check of the error path). The two/one-history-array `lastHistoryEntry` shape itself is unchanged and correct; the specific claim of Step 5 (≥ 2 update events per transition) rests on existing unit-test coverage, not a fresh live capture this session.
- [x] **Step 4: Verify EE gating**: confirmed live — this dev instance has a valid EE license (`platform_enterprise_edition.license_validated: true`), and its real `DraftWorkspace` `WorkflowDefinition` uses transition `conditions` (RBAC-style restriction), which `setWorkflowDefinition` gates behind `checkEnterpriseEdition` whenever a definition's transitions/states use conditions or on-transition/on-status actions (`definitionRequiresEE` check in `workflow-domain.ts`) — the same mechanism already used for `DraftWorkspace`. Live-tested that a condition-gated transition is rejected **both** by `allowedTransitions` (list-level filtering) **and** by calling `triggerWorkflowEvent` directly with that event name bypassing the list (`"Condition failed for transition 'Request MO manager review'"`) — confirms server-side re-validation (defense-in-depth), not merely UI-level hiding.
- [x] **Step 5: Accept explicit event-cardinality criteria** (process note from review): accepted as documented in this plan; no downstream external consumers were available to validate against in this local session (would require a real deployed stream consumer, out of scope for local verification) — recorded here as an explicit residual follow-up for whoever owns the actual production rollout.
- [x] **Step 5.1: Event-type semantic compatibility check** (per review): same as Step 5 — accepted/documented, actual downstream-consumer compatibility validation deferred to the real rollout owner since no such consumer exists to test against in this local session.
- [x] **Step 6: Commit** — no code fixes were needed during this verification pass itself; see the new Step 7 finding below, which does need a follow-up fix/decision before Task 6's migration can be used on a real entity type.
- [x] **Step 7 (new, per review of this verification pass): fix the migration-availability gap found live during Step 1/2.** `workflowDefinitionSet`/the migration's `setWorkflowDefinition` call could not succeed for any legacy-status entity type (confirmed live: `workflowDefinitionSet(entityType: "Report", ...)` → `UnsupportedError: This setting is not available for this entity`, setting `workflow_id`) because `entitySetting-utils.ts`'s `availableSettings` allow-list for `workflow_id` only covered `STIX_SIGHTING_RELATIONSHIP`/`DraftWorkspace` (Task 5). Since `setWorkflowDefinition`'s `updateAttribute` call is the same central write path used by both the GraphQL mutation and Task 6's migration function, this blocked the migration for every type it targets. **Fixed per user decision (extend to all entities):** `workflow_id` added alongside every `workflow_configuration` entry in `availableSettings` — `templateObjectSettings`, `ABSTRACT_STIX_DOMAIN_OBJECT`, `ABSTRACT_STIX_CORE_RELATIONSHIP`, and the four container-specific overrides. Re-verified live end-to-end: `workflowDefinitionSet` + `workflowDefinitionPublish` for `Report` both now succeed (test workflow removed afterward via `workflowDefinitionDelete`, dev DB left unchanged). New unit test `entitySetting-utils-test.ts` (5/5 passing) pins this behavior; full existing suite (255 + 6 tests across the affected files) re-run and still green.
  ```bash
