...
- `client-python/pycti/`: source code package, `examples/`: sample scripts, `tests/`: pytest suite (requires a running OpenCTI instance). Dependencies and dependency groups (`test`, `lint`, `doc`) in `client-python/pyproject.toml`.
- `opencti-worker/src/`: worker scripts (`worker.py`). Dependencies in `opencti-worker/pyproject.toml`.
...
cd opencti-worker/src && uv run --package opencti-worker python worker.py
```
