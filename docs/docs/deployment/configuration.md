...
| app:keep_alive_timeout       | APP__KEEP_ALIVE_TIMEOUT        | 65000                 | Keep-alive timeout of idle HTTP connections, in ms. Must be greater than the idle timeout of your load balancer / reverse proxy (AWS ALB defaults to 60s), otherwise the platform closes sockets the proxy still reuses and clients get intermittent 502. `0` disables the timeout. |
