...
| minio:bucket_prefix | MINIO__BUCKET_PREFIX |                | Optional key prefix prepended to every object written by the platform, to namespace OpenCTI under a shared bucket. Empty by default. Changing it on an existing deployment requires moving the existing objects under the new prefix, otherwise already stored files become unreachable. |
