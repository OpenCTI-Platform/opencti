...
| app:health_monitoring:usage_metrics_interval | APP__HEALTH_MONITORING__USAGE_METRICS_INTERVAL | 300000 | Interval in milliseconds between two collections of storage and ingestion usage metrics. The collection is shared across the cluster through Redis, so a single node computes it per interval. Set to `0` to disable the collection.                                                 |
