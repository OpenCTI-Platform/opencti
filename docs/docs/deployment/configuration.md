...
| app:liveness_port            | APP__LIVENESS_PORT             | 4001                  | Port for the [liveness probe](liveness-probe.md) HTTP server. Starts immediately on process launch, before platform initialization.                                              |
...
| app:allow_unsecure_http_resources                   | APP__ALLOW_UNSECURE_HTTP_RESOURCES                     | true                                                                                                                               | When set to false, prevents the application from loading insecure (HTTP) resources (images, manifests, connections, objects) including from itself |
