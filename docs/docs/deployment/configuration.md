...
| app:health_monitoring:connectivity_interval | APP__HEALTH_MONITORING__CONNECTIVITY_INTERVAL | 30000 | Interval in milliseconds between two dependency connectivity checks (ElasticSearch/OpenSearch, S3/MinIO, RabbitMQ, Redis). The `/health` endpoint answers from the state collected by these checks. |
| app:health_monitoring:usage_metrics_interval | APP__HEALTH_MONITORING__USAGE_METRICS_INTERVAL | 300000 | Interval in milliseconds between two collections of storage and ingestion usage metrics. Set to `0` to disable the collection.                                                 |
