...
## Steps to do before migrating

Migrating to a new version should be done carefully. In regards of the changes related to management of authentication strategies, we advise you, before migrating to version 7: 
- To enable Local Authentication
- To ensure that you have the configuration `APP__ADMIN__EXTERNALLY_MANAGED` set to `FALSE`


## Migration process

At platform initialization, configuration file will be read and migrated: the configuration of the specific strategy will be stored in our database and marked as migrated. It will still exists in the configuration file.
...
To avoid any platform from being locked, the migration will ensure that the Local Authentication strategy is enabled: this way, any platform that is under community edition & did disable the Local Strategy will have a way to login after the migration.
