...
From individual users tailoring their alert preferences to administrators orchestrating collaborative triggers for Groups or Organizations, the Notification Center is a versatile tool for keeping cybersecurity stakeholders in the loop.

The main entry point is the bell icon in the top bar. It opens the notification center on the default Alerts tab.

![Notifications](assets/notifications.png)

## Notification Center

The Notification Center is a single page with two tabs:

- Alerts: displays the current notification history (`myNotifications`)
- Triggers: lets users create and manage personalized live or digest triggers

The active tab is reflected in the breadcrumb and URL. The browser title is `Notification Center` on Alerts and `Triggers` on the Triggers tab.

When an alert is a digest, clicking it opens the digest drawer. When the alert points to a single item, OpenCTI navigates directly to the related entity.
...
The Alerts tab displays the notification feed generated by the platform. Users can search, filter, sort, mark notifications as read or unread, and delete entries when they are no longer relevant.

This tab keeps the full alert history available without separate navigation. The list supports the same filtering and pagination behavior as the previous standalone page.
