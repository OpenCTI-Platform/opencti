...
def test_campaign_import_bulk_copies_extension_fields():
    opencti = _OpenCTI()
    campaign = Campaign(opencti)
    campaign.create = lambda **kwargs: kwargs
    stix_object = {
        "id": "campaign--benchmark",
        "type": "campaign",
        "name": "Benchmark Campaign",
        "extensions": {
            _PRIMARY_EXTENSION: {
                "granted_refs": ["identity--organization"],
                "workflow_id": "workflow--campaign",
            }
        },
    }

    result = campaign.import_from_stix2(stixObject=stix_object)

    assert result["objectOrganization"] == ["identity--organization"]
    assert result["x_opencti_workflow_id"] == "workflow--campaign"
