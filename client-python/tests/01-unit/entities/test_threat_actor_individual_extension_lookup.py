...
def test_threat_actor_individual_import_bulk_copies_extension_fields():
    opencti = _OpenCTI()
    ...
    stix_object = {
        ...
        "type": "threat-actor",
        "name": "Benchmark Threat Actor Individual",
        "extensions": {
            _PRIMARY_EXTENSION: {
    ...
    result = threat_actor_individual.import_from_stix2(stixObject=stix_object)

    assert result["objectOrganization"] == ["identity--organization"]
    assert result["x_opencti_workflow_id"] == "workflow--threat-actor-individual"
