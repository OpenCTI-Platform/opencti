...
def test_intrusion_set_import_bulk_copies_extension_fields():
    opencti = _OpenCTI()
    intrusion_set = IntrusionSet(opencti)
    intrusion_set.create = lambda **kwargs: kwargs
    stix_object = {
        "id": "intrusion-set--benchmark",
        "type": "intrusion-set",
        "name": "Benchmark Intrusion Set",
        "extensions": {
            _PRIMARY_EXTENSION: {
                "granted_refs": ["identity--organization"],
                "workflow_id": "workflow--intrusion-set",
            }
        },
    }

    result = intrusion_set.import_from_stix2(stixObject=stix_object)

    assert result["objectOrganization"] == ["identity--organization"]
    assert result["x_opencti_workflow_id"] == "workflow--intrusion-set"
