...
def test_incident_import_bulk_copies_extension_fields():
    opencti = _OpenCTI()
    incident = Incident(opencti)
    incident.create = lambda **kwargs: kwargs
    stix_object = {
        "id": "incident--benchmark",
        "type": "incident",
        "name": "Benchmark Incident",
        "extensions": {
            _PRIMARY_EXTENSION: {
                "granted_refs": ["identity--organization"],
                "workflow_id": "workflow--incident",
            }
        },
    }

    result = incident.import_from_stix2(stixObject=stix_object)

    assert result["objectOrganization"] == ["identity--organization"]
    assert result["x_opencti_workflow_id"] == "workflow--incident"
