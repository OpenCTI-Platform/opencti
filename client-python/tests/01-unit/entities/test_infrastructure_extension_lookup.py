...
def test_infrastructure_import_bulk_copies_extension_fields():
    opencti = _OpenCTI()
    infrastructure = Infrastructure(opencti)
    infrastructure.create = lambda **kwargs: kwargs
    stix_object = {
        "id": "infrastructure--benchmark",
        "type": "infrastructure",
    ...
    result = infrastructure.import_from_stix2(stixObject=stix_object)

    assert result["objectOrganization"] == ["identity--organization"]
    assert result["x_opencti_workflow_id"] == "workflow--infrastructure"
