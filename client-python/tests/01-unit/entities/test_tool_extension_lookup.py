...
def test_tool_import_bulk_copies_extension_fields():
    opencti = _OpenCTI()
    tool = Tool(opencti)
    ...
    result = tool.import_from_stix2(stixObject=stix_object)

    assert result["objectOrganization"] == ["identity--organization"]
    assert result["x_opencti_workflow_id"] == "workflow--tool"
