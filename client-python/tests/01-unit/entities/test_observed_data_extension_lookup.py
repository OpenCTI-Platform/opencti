...
def test_observed_data_import_bulk_copies_extension_fields():
    opencti = _OpenCTI()
    ...
    result = observed_data.import_from_stix2(stixObject=stix_object)

    assert result["objectOrganization"] == ["identity--organization"]
    assert result["x_opencti_workflow_id"] == "workflow--observed-data"
