...
def test_data_component_import_bulk_copies_extension_fields():
    opencti = _OpenCTI()
    data_component = DataComponent(opencti)
    data_component.create = lambda **kwargs: kwargs
    stix_object = {
        "id": "data-component--benchmark",
        "type": "data-component",
        "name": "Benchmark Data Component",
        "data_source_ref": "data-source--benchmark",
        "extensions": {
            _PRIMARY_EXTENSION: {
                "granted_refs": ["identity--organization"],
                "workflow_id": "workflow--data-component",
            }
        },
    }

    result = data_component.import_from_stix2(stixObject=stix_object)

    assert result["dataSource"] == "data-source--benchmark"
    assert result["objectOrganization"] == ["identity--organization"]
    assert result["x_opencti_workflow_id"] == "workflow--data-component"
