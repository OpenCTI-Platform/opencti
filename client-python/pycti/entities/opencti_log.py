...
class Log:
    ...
    def list(self, **kwargs) -> List[Dict]:
        ...
        self.opencti.admin_logger.info(
            "Fetching logs with filters", {"filters": filters}
        )
        query = (
            """
            query LogList($first: Int, $after: ID, $orderBy: LogsOrdering, $orderMode: OrderingMode, $filters: FilterGroup, $search: String) {
                logs(first: $first, after: $after, orderBy: $orderBy, orderMode: $orderMode, filters: $filters, search: $search) {
                    edges {
                        node {
                    """
        ...
        if get_all:
            ...
            final_data = []
            data = self.opencti.process_multiple(result["data"]["logs"])
            ...
            return self.opencti.process_multiple(
                result["data"]["logs"], with_pagination
            )
    ...
    def read(self, **kwargs) -> Optional[Dict]:
        ...
        :return: Representation of the log as a Python dictionary.
        :rtype: Optional[Dict]
        """
        ...
        search = kwargs.get("search", None)
        if id is not None:
            self.opencti.admin_logger.info("Fetching log with ID", {"id": id})
            query = (
                """
                query LogRead($id: String!) {
                    log(id: $id) {
                        """
                + (self.properties if custom_attributes is None else custom_attributes)
                + """
            ...
            return self.opencti.process_multiple_fields(result["data"]["log"])
            ...
            self.opencti.admin_logger.error(
                "[opencti_log] Missing parameters: id, search, or filters"
            )
            return None