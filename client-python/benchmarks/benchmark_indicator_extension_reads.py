"""Benchmark repeated extension reads during Indicator STIX import."""
...
def _build_indicator(index: int) -> dict:
    return {
        ...
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                ...
                "stix_ids": [f"indicator--legacy-{index}"],
            ...
            MITRE_EXTENSION_ID: {"platforms": ["Windows"]},
        },
    }
...
def _run_once(iterations: int) -> tuple[float, int, int, int]:
    ...
    try:
        for index in range(iterations):
            indicator.import_from_stix2(stixObject=_build_indicator(index))
    finally:
