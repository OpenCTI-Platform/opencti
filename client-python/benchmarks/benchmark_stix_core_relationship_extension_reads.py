"""Benchmark repeated extension reads during StixCoreRelationship STIX import."""
...
def _build_relationship(index: int) -> dict:
    return {
        "id": f"relationship--{index}",
        "type": "relationship",
        "relationship_type": "uses",
        "source_ref": f"identity--{index}",
        "target_ref": f"malware--{index}",
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "stix_ids": [f"relationship--legacy-{index}"],
                ...
                "opencti_upsert_operations": [
                    {"key": "description", "operation": "replace"}
                ],
            }
        },
    }
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    try:
        for index in range(iterations):
            relationship.import_from_stix2(stixRelation=_build_relationship(index))
    finally:
