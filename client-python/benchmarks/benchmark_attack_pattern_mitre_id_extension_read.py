"""Benchmark duplicate extension-backed AttackPattern MITRE ID reads."""
...
def _build_attack_pattern(index: int):
    return {
        ...
        "extensions": {_MITRE_EXTENSION: {"id": f"T{index}"}},
    }
...
def _run_once(iterations: int) -> tuple[float, Counter, Counter, int]:
    ...
    try:
        for index in range(iterations):
            result = api.import_from_stix2(stixObject=_build_attack_pattern(index))
