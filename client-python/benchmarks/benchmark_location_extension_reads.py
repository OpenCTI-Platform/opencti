"""Benchmark ordinary extension reads during Location STIX import."""
...
def _build_location(index: int) -> dict:
    return {
        "id": f"location--{index}",
        "type": "location",
        ...
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "type": "City",
                "aliases": [f"Location Alias {index}"],
                "stix_ids": [f"location--legacy-{index}"],
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    try:
        for index in range(iterations):
            location.import_from_stix2(stixObject=_build_location(index))
    finally:
