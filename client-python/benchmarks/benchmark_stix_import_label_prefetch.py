"""Benchmark bundle-level label prefetching during STIX import."""
...
class _OpenCTI:
    def __init__(self, request_delay_ms: float):
        ...
        self.app_logger = logging.getLogger("benchmark_stix_import_label_prefetch")
    ...
    def logger_class(_name):
        return logging.getLogger("benchmark_stix_import_label_prefetch.worker")
...
def _run_once(object_count: int, request_delay_ms: float) -> tuple[float, int, int]:
    ...
    return (
        elapsed_seconds,
        opencti.label.read_or_create_calls,
...
def main() -> None:
    ...
    read_or_create_call_samples = [sample[1] for sample in samples]
    ...
    result = {
        ...
        "median_total_requests": statistics.median(
            [
                read_or_create_calls + list_calls
                for _, read_or_create_calls, list_calls in samples
            ]
        ),
    }
