"""Microbenchmark for batching nested ref listing across simple exports.

The benchmark isolates OpenCTIStix2.export_selected(..., mode="simple") with
many root entities that have no nested ref relationships. Without a shared
prefetch cache, the simple export path issues one nested-ref list request per
root before learning that each result is empty.
"""
...
def _build_helper(
    ...
    helper.opencti = SimpleNamespace(stix_nested_ref_relationship=nested_refs)
