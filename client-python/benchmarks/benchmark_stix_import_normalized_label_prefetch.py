"""Benchmark reusing normalized label matches returned by bundle prefetch."""
...
class _LabelCollection:
    ...
    def list(self, **kwargs):
        ...
        return [
            {"id": f"label--{value.lower()}", "value": value.lower()}
            for value in values
        ]
    ...
    def read_or_create_unchecked(self, **kwargs):
        ...
        self._delay()
        value = kwargs["value"]
        return {"id": f"label--{value.lower()}", "value": value.lower()}
...
class _OpenCTI:
    def __init__(self, request_delay_ms: float):
        ...
        self.app_logger = logging.getLogger(
            "benchmark_stix_import_normalized_label_prefetch"
        )
    ...
    def logger_class(_name):
        return logging.getLogger(
            "benchmark_stix_import_normalized_label_prefetch.worker"
        )
