"""Microbenchmark for batching core relationship listing across full exports.

The benchmark isolates OpenCTIStix2.export_selected(..., mode="full") with many
root entities that have no core relationships. The current export path still
issues one stixCoreRelationships query per root before learning that each
result is empty, so a small synthetic request delay makes the round-trip cost
visible without mixing in related-object serialization work.
"""
...
class _CountingCoreRelationshipCollection:
...
def _build_helper(
    delay_seconds: float,
) -> tuple[OpenCTIStix2, _CountingCoreRelationshipCollection]:
    helper = OpenCTIStix2.__new__(OpenCTIStix2)
    core_relationships = _CountingCoreRelationshipCollection(delay_seconds)
    helper.opencti = SimpleNamespace(
        stix_nested_ref_relationship=_StaticCollection([]),
        stix_core_relationship=core_relationships,
    ...
    return helper, core_relationships


def _run_once(item_count: int, delay_seconds: float) -> tuple[float, int, int]:
    helper, core_relationships = _build_helper(delay_seconds)
    ...
    return elapsed_seconds, peak_bytes, core_relationships.list_calls
...
def main() -> None:
    ...
    result = {
        ...
        "median_core_relationship_list_calls": int(
            statistics.median(list_call_samples)
        ),
    }
