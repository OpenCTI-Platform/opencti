"""Benchmark repeated extension reads during Narrative STIX import."""
...
def _build_narrative(index: int) -> dict:
    return {
        ...
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "stix_ids": [f"narrative--legacy-{index}"],
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    try:
        for index in range(iterations):
            narrative.import_from_stix2(stixObject=_build_narrative(index))
    finally:
