"""Benchmark STIX2 patch external-reference relation request amplification."""
...
class _ExternalReference:
    ...
    def create(self, **kwargs):
        self.calls += 1
        external_reference_id = f"external-reference--{self.calls - 1}"
...
class _OpenCTI:
    def __init__(self, latency_seconds):
        self.created_ids = []
        self.added_ids = []
        self.external_reference = _ExternalReference(latency_seconds, self.created_ids)
...
def _run_once(refs, latency_seconds):
    ...
    return {
        ...
        "create_calls": opencti.external_reference.calls,
