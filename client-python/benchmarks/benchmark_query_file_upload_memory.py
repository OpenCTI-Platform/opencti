"""Benchmark multipart upload memory while sending a file-like object.

The benchmark uses a local HTTP server so `requests` executes its real
multipart encoding path. The server drains the body and returns a minimal
GraphQL response; the measured cost stays on the pycti client side.
"""
...
def _run_once(client: OpenCTIApiClient, file_name: str) -> tuple[float, int]:
    ...
    with open(file_name, "rb") as file_handle:
        client.query(
            ...
            {
                "file": File(
                    os.path.basename(file_name), file_handle, "application/octet-stream"
                )
            },
        )
