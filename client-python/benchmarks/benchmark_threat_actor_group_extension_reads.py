"""Benchmark repeated extension reads during ThreatActorGroup STIX import."""
...
def _build_threat_actor_group(index: int) -> dict:
    return {
        "id": f"threat-actor--{index}",
        "type": "threat-actor",
        "name": f"Threat Actor Group {index}",
        "extensions": {
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    try:
        for index in range(iterations):
            threat_actor_group.import_from_stix2(
                stixObject=_build_threat_actor_group(index)
            )
    finally:
