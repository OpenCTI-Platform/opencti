"""Microbenchmark for batching container object reads across full exports.

The benchmark isolates ``OpenCTIStix2.export_selected(..., mode="full")`` with
many report roots that each contain one distinct object through ``objects``.
Those contained objects are known before the per-root conversion loop starts,
so one related-object read per report root is avoidable.
"""
...
def _build_root_entities(item_count: int, targets_by_id: dict[str, dict]) -> list[dict]:
    return [
        {
            "id": f"report--root-{index:08d}",
            "type": "report",
            "x_opencti_id": f"root-{index:08d}",
            "objects": [targets_by_id[f"target-{index:08d}"]],
            "objectsIds": [f"target-{index:08d}"],
        }
        for index in range(item_count)
    ]
...
def _run_once(item_count: int, delay_seconds: float) -> tuple[float, int, int, int]:
    ...
    if len(result["objects"]) != item_count * 2:
        raise AssertionError(
            "export_selected() did not preserve report root and contained object count"
        )
