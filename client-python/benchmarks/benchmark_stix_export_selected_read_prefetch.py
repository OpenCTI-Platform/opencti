"""Microbenchmark for batching unique related-object reads across full exports.

The benchmark isolates OpenCTIStix2.export_selected(..., mode="full") with
many root entities that each have one relationship to a distinct target of the
same type. The top-level relationship prefetch already knows every target ID
before the per-root conversion loop starts, so one related-object read per root
is avoidable.
"""
...
def _build_helper(
...
) -> tuple[OpenCTIStix2, _CountingReader, _CountingLister]:
    ...
    helper.opencti = SimpleNamespace(
        ...
        opencti_stix_object_or_stix_relationship=_AccessCollection(),
    )
