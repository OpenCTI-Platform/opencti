"""Benchmark repeated extension reads during Grouping STIX import."""
...
def _build_grouping(index: int) -> dict:
    return {
        "id": f"grouping--{index}",
        "type": "grouping",
        "name": f"Grouping {index}",
        "context": "suspicious-activity",
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "aliases": [f"Grouping Alias {index}"],
                "stix_ids": [f"grouping--legacy-{index}"],
                "granted_refs": ["identity--organization"],
                "content": f"Grouping content {index}",
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    try:
        for index in range(iterations):
            grouping.import_from_stix2(stixObject=_build_grouping(index))
    finally:
