"""Benchmark repeated extension reads during Language STIX import."""
...
def _build_language(index: int) -> dict:
    return {
        "id": f"language--{index}",
        "type": "language",
        ...
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "stix_ids": [f"language--legacy-{index}"],
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    opencti = _OpenCTI()
    language = Language(opencti)
    language.create = lambda **kwargs: kwargs
    ...
    try:
        for index in range(iterations):
            language.import_from_stix2(stixObject=_build_language(index))
    finally:
