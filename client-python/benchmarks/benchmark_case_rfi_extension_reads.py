"""Benchmark repeated extension reads during CaseRfi STIX import."""
...
def _build_case_rfi(index: int) -> dict:
    return {
        "id": f"case-rfi--{index}",
        "type": "case-rfi",
        "name": f"Case RFI {index}",
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "stix_ids": [f"case-rfi--legacy-{index}"],
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    try:
        for index in range(iterations):
            case_rfi.import_from_stix2(stixObject=_build_case_rfi(index))
    finally:
