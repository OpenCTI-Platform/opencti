"""Benchmark repeated extension reads during Channel STIX import."""
...
def _build_channel(index: int) -> dict:
    return {
        "id": f"channel--{index}",
        "type": "channel",
        ...
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "stix_ids": [f"channel--legacy-{index}"],
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    opencti = _OpenCTI()
    channel = Channel(opencti)
    channel.create = lambda **kwargs: kwargs
    ...
    try:
        for index in range(iterations):
            channel.import_from_stix2(stixObject=_build_channel(index))
    finally:
