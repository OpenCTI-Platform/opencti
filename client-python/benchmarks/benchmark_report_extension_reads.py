"""Benchmark repeated extension reads during Report STIX import."""
...
def _build_report(index: int) -> dict:
    return {
        "id": f"report--{index}",
        "type": "report",
        "name": f"Report {index}",
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    try:
        for index in range(iterations):
            report.import_from_stix2(stixObject=_build_report(index))
    finally:
