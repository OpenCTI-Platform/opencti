"""Benchmark repeated extension reads during Incident STIX import."""
...
def _build_incident(index: int) -> dict:
    return {
        "id": f"incident--{index}",
        "type": "incident",
        ...
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "stix_ids": [f"incident--legacy-{index}"],
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    try:
        for index in range(iterations):
            incident.import_from_stix2(stixObject=_build_incident(index))
    finally:
