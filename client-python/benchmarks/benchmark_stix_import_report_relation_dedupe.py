"""Benchmark repeated report relation additions during STIX relationship import."""
...
class _OpenCTI:
    def __init__(self, request_delay_ms: float):
        ...
        self.app_logger = logging.getLogger(
            "benchmark_stix_import_report_relation_dedupe"
        )
    ...
    def logger_class(_name):
        return logging.getLogger("benchmark_stix_import_report_relation_dedupe.worker")
...
def _run_once(
    ...
    return (
        ...
        len(set(opencti.report.add_calls)),
    )
...
def main() -> None:
    ...
    add_call_samples = [sample[1] for sample in samples]
    unique_add_call_samples = [sample[2] for sample in samples]
    result = {
        ...
        "median_report_relation_add_calls": statistics.median(add_call_samples),
        "median_unique_report_relation_add_pairs": statistics.median(
            unique_add_call_samples
        ),
    }
