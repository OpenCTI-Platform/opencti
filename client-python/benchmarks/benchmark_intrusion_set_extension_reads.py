"""Benchmark repeated extension reads during IntrusionSet STIX import."""
...
def _build_intrusion_set(index: int) -> dict:
    return {
        "id": f"intrusion-set--{index}",
        "type": "intrusion-set",
        "name": f"Intrusion Set {index}",
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "stix_ids": [f"intrusion-set--legacy-{index}"],
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    try:
        for index in range(iterations):
            intrusion_set.import_from_stix2(stixObject=_build_intrusion_set(index))
    finally:
