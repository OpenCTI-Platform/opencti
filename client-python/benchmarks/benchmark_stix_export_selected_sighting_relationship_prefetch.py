"""Microbenchmark for batching sighting relationship listing across full exports.

The benchmark isolates OpenCTIStix2.export_selected(..., mode="full") with many
root entities that have no sighting relationships. The current export path
still issues one stixSightingRelationships query per root before learning that
...
class _CountingSightingRelationshipCollection:
...
def _build_helper(
    delay_seconds: float,
) -> tuple[OpenCTIStix2, _CountingSightingRelationshipCollection]:
    helper = OpenCTIStix2.__new__(OpenCTIStix2)
    sighting_relationships = _CountingSightingRelationshipCollection(delay_seconds)
    helper.opencti = SimpleNamespace(
        stix_nested_ref_relationship=_StaticCollection([]),
        stix_core_relationship=_StaticCollection([]),
        stix_sighting_relationship=sighting_relationships,
    ...
    return helper, sighting_relationships


def _run_once(item_count: int, delay_seconds: float) -> tuple[float, int, int]:
    helper, sighting_relationships = _build_helper(delay_seconds)
    ...
    return elapsed_seconds, peak_bytes, sighting_relationships.list_calls
...
def main() -> None:
    ...
    result = {
        ...
        "median_sighting_relationship_list_calls": int(
