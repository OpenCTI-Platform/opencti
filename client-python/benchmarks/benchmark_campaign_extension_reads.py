"""Benchmark repeated extension reads during Campaign STIX import."""
...
def _build_campaign(index: int) -> dict:
    return {
        "id": f"campaign--{index}",
        "type": "campaign",
        ...
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "stix_ids": [f"campaign--legacy-{index}"],
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    try:
        for index in range(iterations):
            campaign.import_from_stix2(stixObject=_build_campaign(index))
    finally:
