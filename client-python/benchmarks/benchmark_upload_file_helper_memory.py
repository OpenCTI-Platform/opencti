"""Benchmark path-based upload helper memory usage.

The benchmark uses a local HTTP server so `upload_file(file_name=...)` runs
through the real GraphQL multipart request path. The server drains the request
body and returns a minimal GraphQL response; the measured cost stays on the
pycti client side.
"""
    ...
    def do_POST(self):
        ...
        while remaining > 0:
            ...
            if not chunk:
                break
            remaining -= len(chunk)
        body = b'{"data":{"uploadImport":{"id":"benchmark","name":"benchmark"}}}'
...
def _build_client(api_url: str) -> OpenCTIApiClient:
    ...
    client.app_logger = logging.getLogger("benchmark_upload_file_helper_memory")
    return client
...
def _run_once(client: OpenCTIApiClient, file_name: str) -> tuple[float, int]:
    ...
    client.upload_file(file_name=file_name)
