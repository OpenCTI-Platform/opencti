"""Microbenchmark for repeated related-object access checks during full export.

The benchmark isolates OpenCTIStix2.prepare_export(..., mode="full") with many
unique relationships that all share the same source and target. Each relation
needs to confirm that both endpoints are exportable, but repeated endpoint
checks should not require repeated GraphQL list calls within one export.
"""
...
class _CountingCollection:
    def __init__(self, items, list_delay_seconds: float):
        self.items = items
...
def _build_helper(
    relationships: list[dict], list_delay_seconds: float
...
def _run_once(item_count: int, list_delay_seconds: float) -> tuple[float, int, int]:
    helper, access_collection = _build_helper(
        _build_relationships(item_count), list_delay_seconds
    )
