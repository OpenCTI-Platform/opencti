"""Benchmark repeated extension reads during Infrastructure STIX import."""
...
def _build_infrastructure(index: int) -> dict:
    return {
        "id": f"infrastructure--{index}",
        "type": "infrastructure",
        "name": f"Infrastructure {index}",
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "stix_ids": [f"infrastructure--legacy-{index}"],
                "granted_refs": ["identity--organization"],
                "workflow_id": "workflow--infrastructure",
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    infrastructure.create = lambda **kwargs: kwargs
    ...
    try:
        for index in range(iterations):
            infrastructure.import_from_stix2(stixObject=_build_infrastructure(index))
    finally:
