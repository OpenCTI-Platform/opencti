"""Benchmark repeated extension reads during Note STIX import."""
...
def _build_note(index: int) -> dict:
    return {
        "id": f"note--{index}",
        "type": "note",
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    try:
        for index in range(iterations):
            note.import_from_stix2(stixObject=_build_note(index))
    finally:
