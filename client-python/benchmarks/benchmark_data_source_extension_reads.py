"""Benchmark repeated extension reads during DataSource STIX import."""
...
def _build_data_source(index: int) -> dict:
    return {
        "id": f"data-source--{index}",
        "type": "data-source",
        "name": f"Data Source {index}",
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "stix_ids": [f"data-source--legacy-{index}"],
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    try:
        for index in range(iterations):
            data_source.import_from_stix2(stixObject=_build_data_source(index))
    finally:
