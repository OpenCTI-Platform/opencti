"""Benchmark repeated extension reads during ThreatActorIndividual STIX import."""
...
def _build_threat_actor_individual(index: int) -> dict:
    return {
        "id": f"threat-actor--{index}",
        "type": "threat-actor",
        "name": f"Threat Actor Individual {index}",
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "stix_ids": [f"threat-actor--legacy-{index}"],
                "granted_refs": ["identity--organization"],
                "workflow_id": "workflow--threat-actor-individual",
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    opencti = _OpenCTI()
    threat_actor_individual = ThreatActorIndividual(opencti)
    threat_actor_individual.create = lambda **kwargs: kwargs
    ...
    try:
        for index in range(iterations):
            threat_actor_individual.import_from_stix2(
                stixObject=_build_threat_actor_individual(index)
            )
    finally:
