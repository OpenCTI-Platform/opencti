"""Benchmark wasted embedded markdown fetches for non-image paths.

The benchmark isolates ``export_selected()`` over markdown fields that use
image syntax for ``embedded/...`` paths whose extension is not an image. The
exporter should leave those targets unchanged, so any storage fetch and
base64 serialization done before the MIME check is avoidable work.
"""
    ...
    return [
        {
            ...
            "description": f"![doc](embedded/Report/{index:08d}/payload.pdf)",
        }
...
def _run_once(
    ...
    if any(
        entity["description"] != f"![doc](embedded/Report/{index:08d}/payload.pdf)"
        for index, entity in enumerate(result["objects"])
    ):
        raise AssertionError("export_selected() rewrote a non-image embedded path")
    return (
        elapsed_seconds,
        peak_bytes,
        opencti.fetch_calls,
