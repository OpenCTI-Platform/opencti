"""Benchmark repeated extension reads during Tool STIX import."""
...
def _build_tool(index: int) -> dict:
    return {
        "id": f"tool--{index}",
        "type": "tool",
        "name": f"Tool {index}",
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    opencti = _OpenCTI()
    tool = Tool(opencti)
    tool.create = lambda **kwargs: kwargs
    ...
    try:
        for index in range(iterations):
            tool.import_from_stix2(stixObject=_build_tool(index))
    finally:
