"""Benchmark ordinary extension reads during CourseOfAction STIX import."""
...
def _build_course_of_action(index: int) -> dict:
    return {
        "id": f"course-of-action--{index}",
        "type": "course-of-action",
        "name": f"Course of action {index}",
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "aliases": [f"Course Alias {index}"],
                "stix_ids": [f"course-of-action--legacy-{index}"],
            ...
            MITRE_EXTENSION_ID: {"id": f"M{index}"},
        },
    }
...
def _run_once(iterations: int) -> tuple[float, int, int, int]:
    ...
    try:
        for index in range(iterations):
            course_of_action.import_from_stix2(
                stixObject=_build_course_of_action(index)
            )
    finally:
