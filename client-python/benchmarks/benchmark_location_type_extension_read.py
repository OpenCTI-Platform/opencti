"""Benchmark duplicate extension-backed Location type reads."""
...
def _build_location(index: int):
    return {
        "id": f"location--{index}",
        "type": "location",
        "name": f"Benchmark location {index}",
        "extensions": {_OPENCTI_EXTENSION: {"type": "City"}},
    }


def _build_api() -> tuple[_OpenCTI, Location]:
    opencti = _OpenCTI()
    api = Location(opencti)
...
def _run_once(iterations: int) -> tuple[float, Counter, int]:
    ...
    try:
        for index in range(iterations):
            result = api.import_from_stix2(stixObject=_build_location(index))
            checksum += len(result["type"])
    finally:
