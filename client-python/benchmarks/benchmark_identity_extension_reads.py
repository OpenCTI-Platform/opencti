"""Benchmark repeated extension reads during Identity STIX import."""
...
def _build_identity(index: int) -> dict:
    return {
        "id": f"identity--{index}",
        "type": "identity",
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    try:
        for index in range(iterations):
            identity.import_from_stix2(stixObject=_build_identity(index))
    finally:
