"""Benchmark repeated extension reads during Task STIX import."""
...
def _build_task(index: int) -> dict:
    return {
        "id": f"task--{index}",
        "type": "task",
        "name": f"Task {index}",
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    try:
        for index in range(iterations):
            task.import_from_stix2(stixObject=_build_task(index))
    finally:
