"""Microbenchmark for batching unique endpoint access checks in full export.

The benchmark isolates OpenCTIStix2.prepare_export(..., mode="full") with one
root entity linked to many unique targets. The export already has every
relationship endpoint ID before it converts relation objects, so issuing one
visibility query per target represents avoidable GraphQL round trips.
"""
...
def _run_once(item_count: int, delay_seconds: float) -> tuple[float, int, int]:
    ...
    if len(result) != item_count + 1:
        raise AssertionError(
            "prepare_export() did not preserve root and relation count"
        )
