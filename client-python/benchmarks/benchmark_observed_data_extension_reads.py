"""Benchmark repeated extension reads during ObservedData STIX import."""
...
def _build_observed_data(index: int) -> dict:
    return {
        "id": f"observed-data--{index}",
        "type": "observed-data",
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "stix_ids": [f"observed-data--legacy-{index}"],
                "granted_refs": ["identity--organization"],
                "workflow_id": "workflow--observed-data",
                "modified_at": "2026-07-15T00:00:00.000Z",
                "opencti_upsert_operations": [
                    {"key": "number_observed", "operation": "replace"}
                ],
            }
        },
    }
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    opencti = _OpenCTI()
    observed_data = ObservedData(opencti)
    observed_data.create = lambda **kwargs: kwargs
    ...
    try:
        for index in range(iterations):
            observed_data.import_from_stix2(stixObject=_build_observed_data(index))
    finally:
