"""Microbenchmark for repeated shared marking-definition construction during export.

The benchmark isolates export_selected() over many roots that share one marking
definition. Before export-scoped marking caching, prepare_export() reconstructs
the same marking-definition dictionary once per root even though bundle
...
def _run_once(item_count: int) -> tuple[float, int]:
    ...
    if len(result["objects"]) != item_count + 1:
        raise AssertionError("export_selected() did not deduplicate shared marking")
