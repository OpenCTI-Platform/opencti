"""Microbenchmark for batching unique related-object reads in full export.

The benchmark isolates OpenCTIStix2.prepare_export(..., mode="full") with one
root entity linked to many unique targets of the same type. The export already
knows every target ID before it enters the read loop, so issuing one reader
call per target represents avoidable GraphQL round trips.
"""
