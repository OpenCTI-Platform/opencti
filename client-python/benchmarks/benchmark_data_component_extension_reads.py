"""Benchmark repeated extension reads during DataComponent STIX import."""
...
def _build_data_component(index: int) -> dict:
    return {
        "id": f"data-component--{index}",
        "type": "data-component",
        "name": f"Data Component {index}",
        "data_source_ref": "data-source--benchmark",
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "stix_ids": [f"data-component--legacy-{index}"],
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    try:
        for index in range(iterations):
            data_component.import_from_stix2(stixObject=_build_data_component(index))
    finally:
