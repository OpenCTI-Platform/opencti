"""Benchmark STIX2 patch object-ref removal request amplification."""
...
class _RelationRemover:
    def __init__(self, latency_seconds, removed_ids):
    ...
    def remove_stix_object_or_stix_relationship(
        ...
        self.calls += 1
        self.removed_ids.append(stixObjectOrStixRelationshipId)
...
class _NestedRefRelationship:
    ...
    def remove_many_to_stix_core_object(self, from_id, to_ids, relationship_type):
        self.calls += 1
        self.removed_ids.extend(to_ids)
        if self.latency_seconds:
            time.sleep(self.latency_seconds)
        return True
...
class _OpenCTI:
    def __init__(self, latency_seconds):
        self.removed_ids = []
        self.report = _RelationRemover(latency_seconds, self.removed_ids)
        self.stix_nested_ref_relationship = _NestedRefRelationship(
            latency_seconds, self.removed_ids
        )

    @staticmethod
    def supports_api_feature(feature):
        return feature == "BULK_REF_RELATION_DELETE"
...
def _run_once(refs, latency_seconds):
    ...
    updater.remove_object_refs("report", "report--benchmark", object_refs)
    runtime_ms = (time.perf_counter() - start) * 1000
    _, peak_bytes = tracemalloc.get_traced_memory()
    tracemalloc.stop()

    expected_ids = [object_ref["value"] for object_ref in object_refs]
    if opencti.removed_ids != expected_ids:
        raise AssertionError(
            "remove_object_refs() changed ref ordering or dropped refs"
        )
