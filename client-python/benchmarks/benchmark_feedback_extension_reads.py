"""Benchmark repeated extension reads during Feedback STIX import."""
...
def _build_feedback(index: int) -> dict:
    return {
        "id": f"feedback--{index}",
        "type": "feedback",
        "name": f"Feedback {index}",
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "stix_ids": [f"feedback--legacy-{index}"],
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    try:
        for index in range(iterations):
            feedback.import_from_stix2(stixObject=_build_feedback(index))
    finally:
