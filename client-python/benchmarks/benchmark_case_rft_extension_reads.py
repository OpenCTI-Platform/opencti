"""Benchmark repeated extension reads during CaseRft STIX import."""
...
def _build_case_rft(index: int) -> dict:
    return {
        "id": f"case-rft--{index}",
        "type": "case-rft",
        "name": f"Case RFT {index}",
        "extensions": {
            PRIMARY_EXTENSION_ID: {
                "stix_ids": [f"case-rft--legacy-{index}"],
...
def _run_once(iterations: int) -> tuple[float, int, int]:
    ...
    try:
        for index in range(iterations):
            case_rft.import_from_stix2(stixObject=_build_case_rft(index))
    finally:
