...
class OrganizationsLines extends Component {
  ...
  _isRowLoaded({ index }) {
    ...
    const list = this.filterList(pathOr([], ['organizations', 'edges'], this.props.data));
  ...
  _rowRenderer({ index, key, style }) {
    ...
    if (dummy) {
      return <div key={key} style={style}><OrganizationLineDummy/></div>;
    }
  ...
  render() {
    ...
    const list = dummy ? [] : this.filterList(pathOr([], ['organizations', 'edges'], this.props.data));
...
export default withStyles(styles)(createPaginationContainer(
  ...
  {
    data: graphql`
        fragment OrganizationsLines_data on Query @argumentDefinitions(
            count: {type: "Int", defaultValue: 25}
            cursor: {type: "ID"}
            orderBy: {type: "OrganizationsOrdering", defaultValue: ID}
            orderMode: {type: "OrderingMode", defaultValue: "asc"}
        ) {
            organizations(first: $count, after: $cursor, orderBy: $orderBy, orderMode: $orderMode) @connection(key: "Pagination_organizations") {
                edges {
                    node {
                        id
                        name
