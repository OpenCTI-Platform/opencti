...
import {
  ...
  difference, head, sortWith, ascend, path, union,
} from 'ramda';
...
const attackPatternMutationFieldPatch = graphql`
    mutation AttackPatternEditionOverviewFieldPatchMutation($id: ID!, $input: EditInput!) {
...
const attackPatternMutationRelationAdd = graphql`
    mutation AttackPatternEditionOverviewRelationAddMutation($id: ID!, $input: RelationAddInput!) {
        attackPatternEdit(id: $id) {
...
const attackPatternMutationRelationDelete = graphql`
    mutation AttackPatternEditionOverviewRelationDeleteMutation($id: ID!, $relationId: ID!) {
        attackPatternEdit(id: $id) {
...
class AttackPatternEditionOverviewComponent extends Component {
  ...
  searchKillChainPhases(event) {
    fetchQuery(killChainPhasesLinesSearchQuery, { search: event.target.value })
      .then((data) => {
        ...
        this.setState({ killChainPhases: union(this.state.killChainPhases, killChainPhases) });
      });
  }
...
const AttackPatternEditionOverview = createFragmentContainer(AttackPatternEditionOverviewComponent, {
