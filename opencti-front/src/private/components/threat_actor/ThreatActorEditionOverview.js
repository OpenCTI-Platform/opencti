...
const threatActorMutationFieldPatch = graphql`
    mutation ThreatActorEditionOverviewFieldPatchMutation($id: ID!, $input: EditInput!) {
...
const threatActorMutationRelationAdd = graphql`
    mutation ThreatActorEditionOverviewRelationAddMutation($id: ID!, $input: RelationAddInput!) {
        threatActorEdit(id: $id) {
...
const threatActorMutationRelationDelete = graphql`
    mutation ThreatActorEditionOverviewRelationDeleteMutation($id: ID!, $relationId: ID!) {
        threatActorEdit(id: $id) {
...
class ThreatActorEditionOverviewComponent extends Component {
  ...
  handleSubmitField(name, value) {
    threatActorValidation(this.props.t).validateAt(name, { [name]: value }).then(() => {
      commitMutation({
        mutation: threatActorMutationFieldPatch,
        variables: { id: this.props.threatActor.id, input: { key: name, value } },
      });
    }).catch(() => false);
  }
...
const ThreatActorEditionOverview = createFragmentContainer(ThreatActorEditionOverviewComponent, {
  threatActor: graphql`
