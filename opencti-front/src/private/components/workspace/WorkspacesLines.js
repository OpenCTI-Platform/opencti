...
class WorkspacesLines extends Component {
  ...
  _rowRenderer({ index, key, style }) {
    ...
    if (dummy) {
      return <div key={key} style={style}><WorkspaceLineDummy/></div>;
    }
    ...
    return <div key={key} style={style}>
        <WorkspaceLine key={workspace.id} workspace={workspace} paginationOptions={this.props.paginationOptions}/>
    </div>;
  }
  ...
  render() {
    ...
    const list = dummy ? [] : pathOr([], ['workspaces', 'edges'], this.props.data);
...
export const workspacesLinesQuery = graphql`
    query WorkspacesLinesPaginationQuery($count: Int!, $cursor: ID, $orderBy: WorkspacesOrdering, $orderMode: OrderingMode) {
        ...WorkspacesLines_data @arguments(count: $count, cursor: $cursor, orderBy: $orderBy, orderMode: $orderMode)
    }
`;
...
export default withStyles(styles)(createPaginationContainer(
  ...
  {
    data: graphql`
        fragment WorkspacesLines_data on Query @argumentDefinitions(
            count: {type: "Int", defaultValue: 25}
            cursor: {type: "ID"}
            orderBy: {type: "WorkspacesOrdering", defaultValue: ID}
            orderMode: {type: "OrderingMode", defaultValue: "asc"}
        ) {
            workspaces(first: $count, after: $cursor, orderBy: $orderBy, orderMode: $orderMode) @connection(key: "Pagination_workspaces") {
                edges {
                    node {
