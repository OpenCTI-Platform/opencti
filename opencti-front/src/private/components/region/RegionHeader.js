...
class RegionHeaderComponent extends Component {
  ...
  onSubmitCreateAlias(data) {
    if (this.props.region.alias === null
      || !this.props.region.alias.includes(data.new_alias)) {
      commitMutation({
        mutation: regionMutation,
        variables: {
          id: this.props.region.id,
          input: { key: 'alias', value: append(data.new_alias, this.props.region.alias) },
        },
      });
    }
      ...
      <div>
        ...
        {variant !== 'noalias'
          ? <div className={classes.aliases}>
            {propOr([], 'alias', region).map(label => (label.length > 0 ? <Chip key={label} classes={{ root: classes.alias }} label={label} onDelete={this.deleteAlias.bind(this, label)}/> : ''))}
