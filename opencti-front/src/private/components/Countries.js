...
import CountriesLines, { countriesLinesQuery } from './country/CountriesLines';
import inject18n from '../../components/i18n';
import SearchInput from '../../components/SearchInput';
import CountryCreation from './country/CountryCreation';
...
const exportCountriesQuery = graphql`
    query CountriesExportCountriesQuery($count: Int!, $cursor: ID, $orderBy: CountriesOrdering, $orderMode: OrderingMode) {
      ...
      <div>
        ...
        <List classes={{ root: classes.linesContainer }}>
          ...
          <QueryRenderer
            ...
            render={({ props }) => {
              if (props) {
                return <CountriesLines data={props} paginationOptions={paginationOptions} searchTerm={this.state.searchTerm}/>;
              }
              return <CountriesLines data={null} dummy={true} searchTerm={this.state.searchTerm}/>;
            }}
          />
        </List>
        ...
        >
          ...
          <DialogActions>
            ...
            {this.state.exportCsvData !== null
              ? <Button component={CSVLink} data={this.state.exportCsvData} separator={';'} enclosingCharacter={'"'} color='primary' filename={`${t('Countries')}.csv`}>
                {t('Download')}
              </Button>
              : ''}
          </DialogActions>
