...
class IncidentsCards extends Component {
  ...
  _cellRenderer({
    ...
    if (dummy) {
      return <div className={className} key={key} style={style}><IncidentCardDummy/></div>;
    }

    const list = pathOr([], ['incidents', 'edges'], data);
  ...
  render() {
    const { classes, dummy, data } = this.props;
    const list = dummy ? [] : pathOr([], ['incidents', 'edges'], data);
...
export const incidentsCardsQuery = graphql`
    query IncidentsCardsPaginationQuery($count: Int!, $cursor: ID, $orderBy: IncidentsOrdering, $orderMode: OrderingMode) {
        ...IncidentsCards_data @arguments(count: $count, cursor: $cursor, orderBy: $orderBy, orderMode: $orderMode)
    }
`;
...
export default withStyles(styles)(createPaginationContainer(
  ...
  {
    data: graphql`
        fragment IncidentsCards_data on Query @argumentDefinitions(
