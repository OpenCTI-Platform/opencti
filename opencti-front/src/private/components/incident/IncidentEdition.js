...
const incidentEditionCleanContext = graphql`
    mutation IncidentEditionCleanContextMutation($id: ID!) {
        incidentEdit(id: $id) {
            contextClean {
...
class IncidentEdition extends Component {
  ...
  handleClose() {
    if (WS_ACTIVATED) {
      commitMutation({
        mutation: incidentEditionCleanContext,
        variables: { id: this.props.incidentId },
      });
    }
      ...
      <div>
        ...
        <Drawer open={this.state.open} anchor='right' classes={{ paper: classes.drawerPaper }} onClose={this.handleClose.bind(this)}>
          <QueryRenderer
              ...
              render={({ props }) => {
                if (props) { // Done
                  return <IncidentEditionContainer me={props.me} incident={props.incident}
