...
const exportPersonsQuery = graphql`
    query PersonsExportPersonsQuery($count: Int!, $cursor: ID, $orderBy: UsersOrdering, $orderMode: OrderingMode) {
      ...
      <div>
        ...
        <List classes={{ root: classes.linesContainer }}>
          ...
          <QueryRenderer
            ...
            render={({ props }) => {
              if (props) {
                return <PersonsLines data={props} paginationOptions={paginationOptions} searchTerm={this.state.searchTerm}/>;
              }
              return <PersonsLines data={null} dummy={true} searchTerm={this.state.searchTerm}/>;
            }}
          />
        </List>
        ...
        >
          ...
          <DialogActions>
            ...
            {this.state.exportCsvData !== null
              ? <Button component={CSVLink} data={this.state.exportCsvData} separator={';'} enclosingCharacter={'"'} color='primary' filename={`${t('Persons')}.csv`}>
                {t('Download')}
              </Button>
              : ''}
          </DialogActions>
