...
class CampaignsLines extends Component {
  ...
  _isRowLoaded({ index }) {
    ...
    const list = this.filterList(pathOr([], ['campaigns', 'edges'], this.props.data));
  ...
  _rowRenderer({ index, key, style }) {
    ...
    if (dummy) {
      return <div key={key} style={style}><CampaignLineDummy/></div>;
    }
    ...
    return <div key={key} style={style}><CampaignLine key={campaign.id} campaign={campaign}/></div>;
  }

  render() {
    const { dummy } = this.props;
    const { scrollToIndex } = this.state;
    const list = dummy ? [] : this.filterList(pathOr([], ['campaigns', 'edges'], this.props.data));
...
export const campaignsLinesQuery = graphql`
    query CampaignsLinesPaginationQuery($count: Int!, $cursor: ID, $orderBy: CampaignsOrdering, $orderMode: OrderingMode) {
        ...CampaignsLines_data @arguments(count: $count, cursor: $cursor, orderBy: $orderBy, orderMode: $orderMode)
    }
`;
...
export default withStyles(styles)(createPaginationContainer(
  ...
  {
    data: graphql`
        fragment CampaignsLines_data on Query @argumentDefinitions(
