...
class CampaignsCards extends Component {
  ...
  _cellRenderer({
    ...
    if (dummy) {
      return <div className={className} key={key} style={style}><CampaignCardDummy/></div>;
    }

    const list = pathOr([], ['campaigns', 'edges'], data);
  ...
  render() {
    const { classes, dummy, data } = this.props;
    const list = dummy ? [] : pathOr([], ['campaigns', 'edges'], data);
...
export const campaignsCardsQuery = graphql`
    query CampaignsCardsPaginationQuery($count: Int!, $cursor: ID, $orderBy: CampaignsOrdering, $orderMode: OrderingMode) {
        ...CampaignsCards_data @arguments(count: $count, cursor: $cursor, orderBy: $orderBy, orderMode: $orderMode)
    }
`;
...
export default withStyles(styles)(createPaginationContainer(
  ...
  {
    data: graphql`
        fragment CampaignsCards_data on Query @argumentDefinitions(
