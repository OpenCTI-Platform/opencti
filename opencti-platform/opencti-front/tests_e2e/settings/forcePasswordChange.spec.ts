...
test.describe('Force password change - navigation blocking', { tag: ['@ce'] }, () => {
  ...
  test('should redirect to change-password when password is expired', async ({ page, request }) => {
    ...
    // Should be redirected to the change-password page
  ...
  test('should block direct navigation to private routes when password is expired', async ({ page, request }) => {
    ...
    // Wait for the redirect to change-password
    await expect(page).toHaveURL(new RegExp(CHANGE_PASSWORD_PATH));

    // Try to navigate directly to another private route
