...
export const convertStatus = (t, element) =>
  (element?.status?.template?.name ?? null) === null
    ? ''
    : {
...
export const convertMarkings = (element) =>
  (element?.objectMarking ?? []).filter(Boolean).map((n) => convertMarking(n));

export const convertMarkingsWithoutEdges = (element, field = 'objectMarking') =>
  (element?.[field] ?? []).filter(Boolean).map((n) => convertMarking(n));

export const convertTriggers = (element) =>
  (element?.triggers ?? []).map((n) => ({
    label: n.name,
    value: n.id,
  }));

export const convertAuthorizedMembers = (element) =>
  authorizedMembersToOptions(element?.authorized_members ?? []);

export const convertAssignees = (element) =>
  (element?.objectAssignee ?? []).map((n) => ({
    label: n.name,
    value: n.id,
  }));

export const convertParticipants = (element) =>
  (element?.objectParticipant ?? []).map((n) => ({
    label: n.name,
    value: n.id,
  }));

export const convertOrganizations = (element) =>
  (element?.objectOrganization?.edges ?? []).map((n) => ({
    label: n.node.name,
    value: n.node.id,
  }));

export const convertKillChainPhases = (element) =>
  (element?.killChainPhases ?? []).map((n) => ({
    label: `[${n.kill_chain_name}] ${n.phase_name}`,
    value: n.id,
  }));

export const convertExternalReferences = (element) =>
  (element?.externalReferences?.edges ?? []).map((n) => ({
...
export const convertCreatedBy = (element, field = 'createdBy') =>
  isEmptyField(element?.[field])
    ? ''
    : {
...
export const convertUser = (element, field = 'user') =>
  isEmptyField(element?.[field])
    ? ''
    : {
        label: element[field].name,
...
export const convertNotifiers = (element) =>
  element?.notifiers?.map(({ id, name }) => ({ value: id, label: name }));
...
export const convertEventTypes = (element) =>
  element?.event_types?.map((event_type) => {
