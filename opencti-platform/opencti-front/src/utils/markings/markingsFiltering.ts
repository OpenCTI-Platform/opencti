...
export function filterMarkingsOutFor(
  selectedOptions: MarkingDefinition[],
  markingsOptions: MarkingDefinition[],
) {
  return markingsOptions.filter(
    ({ entity }) =>
      ...
      selectedOptions.every(
        (selectedOption) => selectedOption.entity.definition_type !== entity.definition_type,
      ),
  );
}
...
export const checkIsMarkingAllowed = (
  ...
  return allowedMarkings.some(
    (m) =>
      m.definition_type === marking.definition_type && m.x_opencti_order >= marking.x_opencti_order,
  );
};
