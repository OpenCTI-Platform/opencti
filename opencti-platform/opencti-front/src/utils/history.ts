...
export const useGenerateAuditMessage = <
  T extends {
...
>(
  ...
  return `${data.context_data?.message} ${isHistoryUpdate ? `for \`${data.context_data?.entity_name}\` (${t_i18n(displayEntityTypeForTranslation(entityType))})` : ''}`;
};
