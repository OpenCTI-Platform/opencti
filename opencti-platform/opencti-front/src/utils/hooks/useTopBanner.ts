...
const useTopBanner = (): TopBannerState => {
  ...
  useBus(
    SMTP_REFRESH_TOKEN_BANNER_VISIBLE_BUS,
    (value: boolean) => setShowSmtpRefreshTokenBanner(value),
    [],
  );
  ...
  const showRegisterBanner = shouldDisplayRegisterBanner(
    settings,
    isXTMHubAccessible,
    isGrantedToXtmHub,
    isDismissed,
  ...
  const height =
    showLicenseBanner || showTrialBanner || showRegisterBanner || showSmtpRefreshTokenBanner
      ? TOP_BANNER_HEIGHT
      : 0;
