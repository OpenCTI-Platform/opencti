...
const useFiltersState = (
  initFilters: FilterGroup | null = emptyFilterGroup,
  defaultClearFilters: FilterGroup = emptyFilterGroup,
): [FilterGroup, handleFilterHelpers] => {
    ...
    () => ({
      ...
      handleAddFilterWithEmptyValue: (filter: Filter) => {
        ...
        setFiltersState((prevState) =>
          handleAddFilterWithEmptyValueUtil({ filters: prevState ?? emptyFilterGroup, filter }),
        );
      },
      handleAddRepresentationFilter: (id: string, value: string | null) => {
        if (value === null) {
          ...
          setFiltersState((prevState) =>
            handleAddRepresentationFilterUtil({ filters: prevState, id, value }),
          );
        }
      },
      handleAddSingleValueFilter: (id: string, valueId?: string) => {
        latestAddFilterIdRef.current = undefined;
        setFiltersState((prevState) =>
          handleAddSingleValueFilterUtil({ filters: prevState, id, valueId }),
        );
      },
      handleReplaceFilterValues: (id: string, values: string[] | FilterGroup[]) => {
        latestAddFilterIdRef.current = undefined;
        setFiltersState((prevState) =>
          handleReplaceFilterValuesUtil({ filters: prevState, id, values }),
        );
      },
      handleChangeOperatorFilters: (id: string, operator: string) => {
        latestAddFilterIdRef.current = undefined;
        setFiltersState((prevState) =>
          handleChangeOperatorFiltersUtil({ filters: prevState, id, operator }),
        );
      },
      ...
      handleRemoveRepresentationFilter: (id: string, value: string | Filter | undefined | null) => {
        latestAddFilterIdRef.current = undefined;
        setFiltersState((prevState) =>
          handleRemoveRepresentationFilterUtil({ filters: prevState, id, value }),
        );
      },
      ...
      handleChangeRepresentationFilter: (
        ...
        if (oldValue && newValue) {
          setFiltersState((prevState) =>
            handleChangeRepresentationFilterUtil({ filters: prevState, id, oldValue, newValue }),
          );
        } else if (oldValue) {
          setFiltersState((prevState) =>
            handleRemoveRepresentationFilterUtil({ filters: prevState, id, value: oldValue }),
          );
        } else if (newValue) {
          setFiltersState((prevState) =>
            handleAddRepresentationFilterUtil({ filters: prevState, id, value: newValue }),
          );
        }
      },
    }),
