...
import usePreloadedPaginationFragment, {
  UsePreloadedPaginationFragment,
} from '../../utils/hooks/usePreloadedPaginationFragment';
...
export const useLineData = (lineFragment: GraphQLTaggedNode) => (row: KeyType) =>
  useFragment(lineFragment, row);

export const useDataTable = (args: UsePreloadedPaginationFragment<OperationType>): UseDataTable =>
  usePreloadedPaginationFragment(args) as UseDataTable;

export const useDataCellHelpers =
  (storageHelpers: UseLocalStorageHelpers | Record<string, unknown>, variant: DataTableVariant) =>
