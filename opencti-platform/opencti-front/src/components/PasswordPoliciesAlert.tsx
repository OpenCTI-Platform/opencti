...
const PasswordPoliciesAlert: FunctionComponent<PasswordPoliciesAlertProps> = ({
  policies,
  value,
}) => {
  const { t_i18n } = useFormatter();
  const theme = useTheme();
  const { minLength, maxLength, minSymbols, minNumbers, minWords, minLowercase, minUppercase } =
    policies;
  const hasPolicy =
    (minLength ?? 0) > 0 ||
    (maxLength ?? 0) > 0 ||
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        {(minLength ?? 0) > 0 && (
          <PolicyLine
            ...
            label={`${t_i18n('Number of chars must be greater or equals to')} ${minLength}`}
          />
        )}
        {(maxLength ?? 0) > 0 && (
          <PolicyLine
            met={checks.maxLength}
            label={`${t_i18n('Number of chars must be lower or equals to')} ${maxLength}`}
          />
        )}
        {(minSymbols ?? 0) > 0 && (
          <PolicyLine
            met={checks.minSymbols}
            label={`${t_i18n('Number of symbols must be greater or equals to')} ${minSymbols}`}
          />
        )}
        {(minNumbers ?? 0) > 0 && (
          <PolicyLine
            met={checks.minNumbers}
            label={`${t_i18n('Number of digits must be greater or equals to')} ${minNumbers}`}
          />
        )}
        {(minWords ?? 0) > 0 && (
          <PolicyLine
            met={checks.minWords}
            label={`${t_i18n('Number of words (split on hyphen, space) must be greater or equals to')} ${minWords}`}
          />
        )}
        {(minLowercase ?? 0) > 0 && (
          <PolicyLine
            met={checks.minLowercase}
            label={`${t_i18n('Number of lowercase chars must be greater or equals to')} ${minLowercase}`}
          />
        )}
        {(minUppercase ?? 0) > 0 && (
          <PolicyLine
            met={checks.minUppercase}
            label={`${t_i18n('Number of uppercase chars must be greater or equals to')} ${minUppercase}`}
          />
        )}
      </div>
    </Alert>
