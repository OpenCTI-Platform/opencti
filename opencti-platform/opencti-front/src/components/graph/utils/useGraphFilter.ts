...
const useGraphFilter = () => {
  ...
  const { disabledEntityTypes, disabledCreators, disabledMarkings, selectedTimeRangeInterval } =
    graphState;

  const filterNodes = (disabledTargets: string[]) => {
    graphData?.nodes.forEach((node) => {
      node.disabled =
        disabledEntityTypes.includes(node.entity_type) ||
  ...
  const filterLinks = () => {
    ...
    graphData?.links.forEach((link) => {
      link.disabled =
        disabledCreators.includes(link.createdBy.id) ||
