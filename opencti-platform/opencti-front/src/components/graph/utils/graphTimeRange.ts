 ...
 */
export const computeTimeRangeInterval = (objects: GraphLink[]): GraphTimeRange['interval'] => {
...
export const computeTimeRangeValues = (
  ...
  return Array(100)
    ...
    .map((_, i) => {
      ...
      const datesInsideInterval = elementsDates.filter(
        (d) => d >= time && d <= time + intervalInSeconds,
      );
      return {
        time,
        index: 1,
 ...
 */
export const computeTimeRangeValuesDomain = (data: ReturnType<typeof computeTimeRangeValues>) => {
  return [
    0,
    Math.max.apply(
      null,
