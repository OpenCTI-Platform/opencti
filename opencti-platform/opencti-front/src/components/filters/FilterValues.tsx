...
const FilterValues: FunctionComponent<FilterValuesProps> = ({
  ...
  const deactivatePopoverMenu =
    !isFilterEditable(filtersRestrictions, filterKey, filterValues) || !isReadWriteFilter;
  ...
  if (isOperatorNil) {
    return (
      <>
        <strong style={labelStyle} onClick={onCLick}>
          {label}
        </strong>{' '}
        <span>{filterOperator === 'nil' ? t_i18n('is empty') : t_i18n('is not empty')}</span>
      </>
    );
  }
  ...
  if (filterOperator === 'within' && isDateIntervalTranslatable(filterValues)) {
  ...
  const values = filterValues.map((id) => {
    ...
    const operatorOnClick = isLocalModeSwitchable
    ...
    const isRegardingOfFilter =
      parentFilter?.key === 'regardingOf' || parentFilter?.key === 'dynamicRegardingOf';
    return (
      <Fragment key={id}>
        {filterOperator === 'within' ? (
          <>
            ...
            <FilterValuesContent
              ...
              host={host}
            />
            <span>{last(filterValues) === id ? ']' : ', '}</span>
          </>
        ) : (
          <>
            <FilterValuesContent
  ...
  if (filterKey === 'regardingOf' || filterKey === 'dynamicRegardingOf') {
    ...
    const isWarning = isRegardingOfFilterWarning(
      currentFilter,
      scos.map((n) => n.id),
      filtersRepresentativesMap,
    );
            ...
            title={t_i18n('', {
              ...
              values: {
                link: (
                  <Link
                    target="_blank"
                    to="https://docs.opencti.io/latest/reference/filters/?h=regarding#the-regardingof-filter-key"
                  >
