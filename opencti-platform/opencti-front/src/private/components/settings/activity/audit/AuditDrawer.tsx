                      ...
                      <TableBody>
                        {changes && changes.length > 0 ? (changes.map((row) => (
                          <TableRow key={row?.field} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                            ...
                            <TableCell align="left">{row?.changes_removed && row.changes_removed.length > 0
                              ? row.changes_removed.map((s) => s.raw).join(', ')
                              : '-'}
                            </TableCell>
                            <TableCell align="left">{row?.changes_added && row.changes_added.length > 0
                              ? row.changes_added.map((s) => s.raw).join(', ')
                              : '-'}
                            </TableCell>
                          </TableRow>
                        ))
