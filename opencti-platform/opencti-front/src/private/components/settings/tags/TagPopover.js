...
const tagPopoverDeletionMutation = graphql`
  mutation TagPopoverDeletionMutation($id: ID!) {
    tagEdit(id: $id) {
      delete
    }
  }
...
const tagEditionQuery = graphql`
  query TagPopoverEditionQuery($id: String!) {
    tag(id: $id) {
      ...TagEdition_tag
    }
  }
...
class TagPopover extends Component {
  ...
  submitDelete() {
    ...
    commitMutation({
      mutation: tagPopoverDeletionMutation,
      variables: {
        id: this.props.tagId,
      },
      updater: (store) => {
  ...
  render() {
    const { classes, t, tagId } = this.props;
    return (
