...
import {
  ...
  ExclusionListsStatusQuery$variables,
} from '@components/settings/exclusion_lists/__generated__/ExclusionListsStatusQuery.graphql';
...
const ExclusionListsStatusComponent: FunctionComponent<ExclusionListsStatusComponentProps> = ({
  queryRef,
  refetch,
}) => {
  const { t_i18n, fldt } = useFormatter();
  const { exclusionListCacheStatus } = usePreloadedQuery(exclusionListsStatusQuery, queryRef);
    ...
    <Grid container spacing={2}>
      <Grid item xs={4}>
        <Card title={t_i18n('Status')} sx={statusSx}>
          ...
          />
          {isInProgress && <CircularProgress size={40} thickness={2} color="primary" />}
        </Card>
      </Grid>
      <Grid item xs={4}>
        <Card title={t_i18n('Last modification date')} sx={statusSx}>
          <div>{fldt(refreshDate)}</div>
          <UpdateOutlined color="primary" style={{ fontSize: 40 }} />
        </Card>
      </Grid>
      <Grid item xs={4}>
        <Card title={t_i18n('Current cache version date')} sx={statusSx}>
...
interface ExclusionListsStatusProps {
  ...
  refetch: () => void;
  loadQuery: (
    variables: ExclusionListsStatusQuery$variables,
    options?: UseQueryLoaderLoadQueryOptions,
  ) => void;
}

const ExclusionListsStatus: FunctionComponent<ExclusionListsStatusProps> = ({
  refetch,
  queryRef,
  loadQuery,
}) => {
