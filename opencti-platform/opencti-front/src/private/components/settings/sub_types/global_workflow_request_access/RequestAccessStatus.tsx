...
const RequestAccessStatus: FunctionComponent<RequestAccessStatusProps> = ({
  ...
  const approvedLabel = approvedToRfiStatus
    ? t_i18n(approvedToRfiStatus?.template?.name)
    : 'undefined';
  const approvedColor = approvedToRfiStatus?.template?.color ?? '#000000';
  const declinedLabel = declinedToRfiStatus
    ? t_i18n(declinedToRfiStatus?.template?.name)
    : 'undefined';
  ...
  return (
    <>
      ...
      {!requestAccessWorkflowDisabled &&
        admins.map((member) => {
          return (
            <ListItemButton key={member?.id} dense={true} divider={true}>
              <ListItemIcon>
                <ItemIcon type="group" />
              </ListItemIcon>
