...
const EntitySettingCustomFieldsAddDialog: FunctionComponent<
  EntitySettingCustomFieldsAddDialogProps
> = ({ queryRef, open, onClose, entityType, onAdded }) => {
  ...
  const [commitAdd] = useApiMutation<EntitySettingCustomFieldsAddDialogAddMutation>(
    entitySettingCustomFieldsAddDialogAddMutation,
  );
  ...
  return (
    <Dialog open={open} onClose={handleClose} title={t_i18n('Add a custom field')}>
      <Autocomplete
        ...
        renderInput={(params) => (
          <MuiTextField {...params} variant="standard" label={t_i18n('Custom fields')} />
        )}
      />
      <FormControlLabel
