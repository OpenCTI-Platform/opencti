...
import {
  DraftSightingsLinesPaginationQuery,
  DraftSightingsLinesPaginationQuery$variables,
} from '@components/drafts/__generated__/DraftSightingsLinesPaginationQuery.graphql';
...
const DraftSightings: FunctionComponent<DraftSightingsProps> = ({ isReadOnly }) => {
  ...
  } = usePaginationLocalStorage<DraftSightingsLinesPaginationQuery$variables>(
    LOCAL_STORAGE_KEY,
    initialValues,
  );
  ...
  const getRedirectionLink = (stixSighting: any) => {
    if (isReadOnly) {
      const isUpdatedEntity =
        stixSighting.draftVersion?.draft_operation === 'update' ||
        stixSighting.draftVersion?.draft_operation === 'update_linked';
      return isUpdatedEntity
        ? `/dashboard/id/${stixSighting.id}`
        : `/dashboard/id/${stixSighting.standard_id}`;
    }
    ...
    <span data-testid="draft-relationships-page">
      {queryRef && (
        <DataTable
          ...
          resolvePath={(data: DraftSightingsLines_data$data) =>
            data.draftWorkspaceSightingRelationships?.edges?.map((n) => n?.node)
          }
