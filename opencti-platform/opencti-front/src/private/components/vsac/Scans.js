...
class Scans extends Component {
      ...
      function () {
        fetchAllScans(this.state.client_ID)
          .then((response) => {
            ...
            this.setState({
              scansReportDate: this.sortScansByReportDate(scans),
            });
                ...
                .then((response) => {
                  ...
                  response.data.forEach((item) => {
                    scatterPlot.push({
                      ...
                      y: item.score,
                      score: item.score,
                      host_count_total: item.host_count,
                    });
                  });
  ...
  render() {
    ...
    const handleAnalysisClick = (event, analysis_id) => {
      this.setState({
        analysisAnchorEl: event.currentTarget,
        openAnalysisMenu: analysis_id,
      });
    };
    ...
    const CustomTooltip = ({ active, payload, label }) => {
      if (active && payload && payload.length) {
        return (
          ...
          >
            <p className="label" style={{ paddingBottom: 5 }}>
              {payload[0].payload.cwe_name}
            </p>
            <p
              className="weakness"
              style={{ paddingBottom: 5 }}
            >{`Weakness Score: ${payload[0].payload.score}`}</p>
            <p
              className="host"
              style={{ paddingBottom: 5 }}
            >{`Hosts with Weakness: ${payload[0].payload.host_count_total} (${payload[0].payload.x}%)`}</p>
          </div>
        );
      }
      return null;
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            <List>
                              <ListItem>
                                Report Name: {scan.report_name}{' '}
                              </ListItem>
                              <ListItem>
                                Policy Name: {scan.policy_name}{' '}
                              </ListItem>
                              <ListItem>Scan Date: {scan.report_date}</ListItem>
                              <ListItem>Uploaded: {scan.upload_date}</ListItem>
                              <ListItem>
                                Total Vulnerabilities: {scan.total_cve}
                              </ListItem>
                              <ListItem>
                                Unique Vulnerabilities: {scan.unique_cve}
                              </ListItem>
                              <ListItem>
                                Total Records: {scan.record_count_total}
                              </ListItem>
                              <ListItem>
                              ...
                              <ListItem>
                                Total Hosts: {scan.host_count_total}
                              </ListItem>
                              <ListItem>
                                ...with vulnerabilities:
                                {scan.host_count}
                              ...
                              <ListItem>
                                Cyio Analysis {scan.analysis_count}
                              </ListItem>
                            </List>
                          </Popover>
                        </ListItem>
