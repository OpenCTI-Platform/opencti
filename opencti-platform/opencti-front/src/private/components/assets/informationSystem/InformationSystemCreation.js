...
import SoftwareCreationDetails from './InformationSystemCreationDetails';
...
const InformationSystemCreationMutation = graphql`
  mutation InformationSystemCreationMutation($input: SoftwareAssetAddInput) {
...
const InformationSystemCreationProductQuery = graphql`
  query InformationSystemCreationProductQuery($search: String, $filters: [ProductFiltering], $orderedBy: ProductOrdering, $orderMode: OrderingMode) {
...
const InformationSystemCreationProductIdQuery = graphql`
  query InformationSystemCreationProductIdQuery( $id: ID!) {
    product(id: $id) {
      id
      created
      modified
...
class InformationSystemCreation extends Component {
  constructor(props) {
    super(props);
    this.state = {
      open: false,
  ...
  searchProducts(event, value) {
    ...
    if (event.type === 'click' && value) {
      ...
      fetchQuery(InformationSystemCreationProductIdQuery, {
  ...
  handleSearchProducts() {
    ...
    (this.state.productName.length > 2) && fetchQuery(InformationSystemCreationProductQuery, {
      search: this.state.productName,
      orderedBy: 'name',
      orderMode: 'asc',
      filters: [
        { key: 'object_type', values: ['informationSystem'] }
      ],
    })
      .toPromise()
      .then((data) => {
  ...
  onSubmit(values, { setSubmitting, resetForm }) {
    ...
    commitMutation({
      mutation: InformationSystemCreationMutation,
      variables: {
        input: finalValues,
      },
      setSubmitting,
      ...
      <div className={classes.container}>
        ...
        >
          ...
          <DialogActions className={classes.dialogActions}>
            ...
            <Button
              onClick={() => this.props.history.push('/defender HQ/assets/informationSystem')}
...
InformationSystemCreation.propTypes = {
