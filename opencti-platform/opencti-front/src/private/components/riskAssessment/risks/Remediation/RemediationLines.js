...
import { RemediationLine, RemediationLineDummy } from './RemediationLine';
import { setNumberOfElements } from '../../../../../utils/Number';

const nbOfRowsToLoad = 50;

class RemediationLines extends Component {
  ...
  render() {
    ...
    return (
      <ListLinesContent
        ...
        dataList={pathOr([], ['remediations', 'edges'], this.props.data)}
        globalCount={pathOr(
          nbOfRowsToLoad,
          ['remediations', 'pageInfo', 'globalCount'],
          this.props.data,
        )}
        LineComponent={<RemediationLine />}
        DummyLineComponent={<RemediationLineDummy />}
...
export const remediationLinesQuery = graphql`
  query RemediationLinesPaginationQuery(
    $search: String
    $count: Int!
    $cursor: ID
...
export default createPaginationContainer(
  ...
  {
    data: graphql`
      fragment RemediationLines_data on Query
      @argumentDefinitions(
        search: { type: "String" }
