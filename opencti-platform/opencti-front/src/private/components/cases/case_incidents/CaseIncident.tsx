...
const CaseIncident: React.FC<CaseIncidentProps> = ({ caseIncidentData, enableReferences }) => {
  ...
  const { viewStorage, helpers, paginationOptions } =
    usePaginationLocalStorage<CaseTasksLinesQuery$variables>(LOCAL_STORAGE_KEY_CASE_TASKS, {
      searchTerm: '',
      sortBy: 'created',
      orderAsc: false,
    });
  ...
  const caseIncidentResponseOverviewLayoutCustomization = useOverviewLayoutCustomization(
    caseIncident.entity_type,
  );
      ...
      <Grid container={true} spacing={3} style={{ marginBottom: 20 }}>
        {caseIncidentResponseOverviewLayoutCustomization.map(({ key, width }) => {
          switch (key) {
            ...
            case 'task':
              return (
                <Grid key={key} size={{ xs: width }} ref={ref}>
                  {queryRef && (
                    <React.Suspense
                      fallback={
            ...
            case 'externalReferences':
              return (
                <Grid key={key} size={{ xs: width }}>
                  <StixCoreObjectExternalReferences stixCoreObjectId={caseIncident.id} />
                </Grid>
              );
            case 'mostRecentHistory':
              return (
                <Grid key={key} size={{ xs: width }}>
                  <StixCoreObjectLatestHistory stixCoreObjectId={caseIncident.id} />
                </Grid>
              );
            case 'notes':
              return (
