...
const CaseIncidents: FunctionComponent<CaseIncidentsProps> = () => {
  ...
  const {
    ...
    paginationOptions,
  } = usePaginationLocalStorage<CaseIncidentsLinesCasesPaginationQuery$variables>(
  ...
  return (
    <div data-testid="incident-response-page">
      <Breadcrumbs
        elements={[
          { label: t_i18n('Cases') },
          { label: t_i18n('Incident responses'), current: true },
        ]}
      />
      {queryRef && (
        <DataTable
          dataColumns={dataColumns}
          resolvePath={(data: CaseIncidentsLinesCases_data$data) =>
            data.caseIncidents?.edges?.map((n) => n?.node)
          }
