...
export const JsonFormArrayRenderer = (props: ControlProps) => {
  const { data, description, handleChange, path, label, errors, schema } = props;
  ...
  const handleValuesChange = useCallback(
    (event: React.SyntheticEvent, newValues: string[]) => {
...
export const jsonFormArrayTester: RankedTester = rankWith(
  ...
  and(
    ...
    schemaMatches((schema) => {
      return schema.type === 'array' && (schema.default === null || Array.isArray(schema.default));
    }),
  ),
);
