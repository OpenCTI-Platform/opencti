...
const StixCoreObjectOrStixCoreRelationshipNotes: FunctionComponent<StixCoreObjectOrStixCoreRelationshipNotesProps> = ({
  ...
  const paginationOptions = {
    count: 200,
    filters: [{ key: ['objectContains' as NotesFilter], values: [stixCoreObjectOrStixCoreRelationshipId] }],
  };
  let queryRef;
  let title;

  if (isRelationship) {
    queryRef = useQueryLoading<StixCoreObjectOrStixCoreRelationshipNotesCardsQuery>(stixCoreObjectOrStixCoreRelationshipNotesCardsQuery, paginationOptions);
