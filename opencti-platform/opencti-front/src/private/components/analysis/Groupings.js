...
class Groupings extends Component {
  constructor(props) {
    ...
    const params = buildViewParamsFromUrlAndStorage(
      props.history,
      props.location,
      `view-groupings${props.objectId ? `-${props.objectId}` : ''}`,
    );
    this.state = {
  ...
  saveView() {
    saveViewParameters(
      this.props.history,
      this.props.location,
      `view-groupings${this.props.objectId ? `-${this.props.objectId}` : ''}`,
      this.state,
    );
  }
  ...
  render() {
    const {
      match: {
        params: { groupingContext },
      },
      objectId,
      authorId,
    } = this.props;
    const { view, sortBy, orderAsc, searchTerm, filters } = this.state;
    const groupingFilterClass = groupingContext !== 'all' && groupingContext !== undefined
      ? groupingContext.replace(/_/g, ' ')
      : '';
    const finalFilters = convertFilters(filters);
    if (groupingFilterClass) {
      finalFilters.push({
        key: 'context',
        values: [groupingFilterClass],
      });
    }
...
export default R.compose(inject18n, withRouter)(Groupings);
