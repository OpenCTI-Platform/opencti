...
const StixCoreObjectOpinionsRadar: FunctionComponent<StixCoreObjectOpinionsRadarProps> = ({
  ...
  const opinionOptions = vocabularies?.edges
    ...
    .map((node, idx) => ({ label: node.name.toLowerCase(), value: idx + 1 })) ?? [];

  const myOpinionValue = opinionOptions.find((m) => m.label === myOpinion?.opinion)?.value;
  const [currentOpinion, setCurrentOpinion] = useState(myOpinionValue ?? Math.round(opinionOptions.length / 2));
  ...
  const onSubmit = (values: OpinionAddInput, { setSubmitting, resetForm }: FormikHelpers<OpinionAddInput>) => {
    ...
    if (isNotEmptyField(alreadyExistingOpinion)) {
      const finalValues = Object.entries(inputValues).map(([key, value]) => ({ key, value: adaptFieldValue(value) }));
      commitEdition({
        variables: {
        ...
        >
          ...
          >
            {({
              ...
              <Form>
                <DialogTitle>{myOpinion ? t('Update opinion') : t('Create an opinion')}</DialogTitle>
                <DialogContent>
                  ...
                  />
                  <ConfidenceField entityType="Opinion" containerStyle={fieldSpacingContainerStyle} />
                </DialogContent>
                <DialogActions>
                  <Button
                    onClick={handleReset}
