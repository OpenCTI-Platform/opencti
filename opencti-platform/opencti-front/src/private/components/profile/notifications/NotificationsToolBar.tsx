...
const NotificationsToolBar: FunctionComponent<NotificationsToolBarProps> = ({
  ...
  const [commitQueryTask] = useMutation(notificationsToolBarQueryTaskAddMutation);
  const [commitListTask] = useMutation(notificationsToolBarListTaskAddMutation);

  const [displayTask, setDisplayTask] = useState(false);
  const [actions, setActions] = useState<{
    type: string,
    context: null | { field: string, type: string, values:(string | { id: string, value: string })[],
    } }[]>([]);
  ...
  const submitTask = () => {
    ...
    const finalActions = actions.map(
      (action) => ({
        type: action.type,
        ...
        <Tooltip title={t('Mark as read')}>
            <span>
              <IconButton
                aria-label="ack"
                disabled={
                  numberOfSelectedElements === 0 || processing
                }
