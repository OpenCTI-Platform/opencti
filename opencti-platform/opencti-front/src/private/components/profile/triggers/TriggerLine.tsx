        ...
        primary={
          <>
            ...
            <div className={classes.bodyItem} style={{ width: dataColumns.notifiers.width }}>
              {data.notifiers && data.notifiers.length > 0
                ? data.notifiers.map<React.ReactNode>((n) => (
                    <code key={n.id} style={{ marginRight: 5 }}>
                      {n.name}
                    </code>
                  ))
                : EMPTY_VALUE}
            ...
            <div className={classes.bodyItem} style={{ width: dataColumns.event_types.width }}>
              {data.event_types &&
                data.event_types.map((n: string) => <Tag key={n} label={t_i18n(n)} />)}
              {data.triggers &&
                data.triggers.map((n) => (
                  <Tag key={n?.id} color={theme.palette.severity?.high} label={n?.name} />
                ))}
            </div>
            {data.trigger_type === 'live' && (
              <div className={classes.filtersItem} style={{ width: dataColumns.filters.width }}>
                {isFilterGroupNotEmpty(filters) ? (
                  <FilterIconButton
                    ...
                    redirection
                    entityTypes={
                      data.instance_trigger ? ['Instance'] : ['Stix-Core-Object', 'Stix-Filtering']
                    }
                  />
                ) : (
                  EMPTY_VALUE
