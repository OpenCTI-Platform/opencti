...
const StixDomainObjectAttackPatternsKillChainLines: FunctionComponent<
  StixDomainObjectAttackPatternsKillChainLinesProps
> = ({ data, searchTerm, paginationOptions, coursesOfAction }) => {
  ...
  const handleToggleLine = (lineKey: string) => {
    setExpandedLines({
      ...expandedLines,
      [lineKey]: expandedLines[lineKey] !== undefined ? !expandedLines[lineKey] : false,
    });
  };
  ...
  type AttackPatternNode = NonNullable<
    StixDomainObjectAttackPatternsKillChainContainer_data$data['attackPatterns']
  >['edges'][number]['node'];
  type NonNullableCoursesOfAction = NonNullable<
    NonNullable<NonNullable<AttackPatternNode['coursesOfAction']>['edges']>[number]
  >;
  ...
  const filterByKeyword = (n: FormattedAttackPattern) =>
    searchTerm === '' ||
    ...
    (n.subattackPatterns_text ?? '').toLowerCase().indexOf(searchTerm.toLowerCase()) !== -1;

  const killChainPhases = uniq(
    (data.attackPatterns?.edges ?? []).map((n) =>
      n.node.killChainPhases && n.node.killChainPhases.length > 0
        ? n.node.killChainPhases[0]
        : {
            id: 'unknown',
            phase_name: t_i18n('Unknown'),
            kill_chain_name: t_i18n('Unknown'),
            x_opencti_order: 99,
          },
    ),
  );
  ...
  const formattedAttackPatterns = (data.attackPatterns?.edges ?? [])
    ...
    .map((n) => ({
      ...n,
      killChainPhase:
        n.killChainPhases && n.killChainPhases.length > 0
          ? {
              ...n.killChainPhases[0],
              kill_chain_name: n.killChainPhases[0].kill_chain_name ?? t_i18n('Unknown'),
            }
          : {
              id: 'unknown',
              phase_name: t_i18n('Unknown'),
      ...
      kill_chain_name:
        n.killChainPhases && n.killChainPhases.length > 0
          ? n.killChainPhases[0].kill_chain_name
          : t_i18n('Unknown'),
  ...
  const sortedAttackPatternsElement = finalAttackPatternsElement.sort(
    (a, b) =>
      (b.x_opencti_order ?? Number.POSITIVE_INFINITY) -
      (a.x_opencti_order ?? Number.POSITIVE_INFINITY),
  );
        ...
        <List id="test">
          {sortedAttackPatternsElement.map((element) => (
            <div key={element.id}>
              <ListItem
                ...
                secondaryAction={
                  <IconButton
                    ...
                    aria-haspopup="true"
                  >
                    {expandedLines[element.id] === false ? <ExpandMore /> : <ExpandLess />}
                  </IconButton>
                }
              >
                <ListItemButton divider={true} onClick={() => handleToggleLine(element.id)}>
                  <ListItemIcon>
              ...
              <Collapse in={expandedLines[element.id] !== false}>
                <List>
                  {(element.attackPatterns ?? []).map((attackPattern) => {
                      ...
                      <div key={attackPattern.id}>
                          ...
                          secondaryAction={
                            ...
                            >
                              {coursesOfAction && (
                                <IconButton
                                  aria-label={
                                    expandedLines[attackPattern.id]
                                      ? t_i18n('Collapse')
                                      : t_i18n('Expand')
                                  }
                        ...
                        >
                          <ListItemButton
                            ...
                            onClick={
                              coursesOfAction ? () => handleToggleLine(attackPattern.id) : undefined
                            }
                          >
                            <ListItemIcon>
                            ...
                            <ListItemText
                              primary={
                                <span>
                                  <strong>{attackPattern.x_mitre_id}</strong> - {attackPattern.name}
                                </span>
                              }
                              secondary={
                                <MarkdownDisplay
                        ...
                        {coursesOfAction && (
                          <Collapse in={expandedLines[attackPattern.id] !== false}>
                            <List>
                              {(
                                (attackPattern.coursesOfAction?.edges ?? []).filter(
                                  (n) => !!n,
                                ) as NonNullableCoursesOfAction[]
                              ).map((courseOfActionEdge) => {
                                ...
                                return (
                                  ...
                                  >
                                    <ListItemIcon>
                                      <ProgressWrench color="primary" role="img" />
                                    </ListItemIcon>
                                    <ListItemText
