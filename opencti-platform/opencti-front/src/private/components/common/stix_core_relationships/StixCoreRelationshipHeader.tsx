...
import { defaultValue } from 'src/utils/Graph';
...
const StixCoreRelationshipHeaderOverview = ({
  ...
  const fromName = defaultValue(from);
  const toName = defaultValue(to);
  const relationName = t_i18n(`relationship_${relationship_type}`);
  const title = `${fromName} ${relationName} ${toName}`;
  ...
  const submitDelete = () => {
    ...
    commitMutation({
      ...
      onCompleted: () => {
        ...
        history.push(location.pathname.replace(`/relations/${id}`, ''));
      },
    });
  };
