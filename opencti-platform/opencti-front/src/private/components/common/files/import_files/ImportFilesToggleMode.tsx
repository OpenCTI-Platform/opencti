...
const ImportFilesToggleMode = () => {
  ...
  const { setActiveStep, importMode, setImportMode, entityId, isForcedImportToDraft } =
    useImportFilesContext();
  ...
  const modes: {
  ...
  }[] = [
    {
      ...
      description: t_i18n(
        'A guided workflow that streamlines files import, selection of connectors and allows the creation of a workbench or draft for review before final import',
      ),
  ...
  if (!isForcedImportToDraft) {
    modes.unshift({
      ...
      description: t_i18n(
        'Quick import with no configuration needed. Just upload your files and the platform takes care of the rest. Perfect if your file follows a standard format (STIX2.1, MISP).',
      ),
  ...
  if (!entityId) {
    modes.push({
      ...
      description: t_i18n(
        'Use a structured form to create and import data. Select from available forms and fill in the required information to generate properly formatted entities.',
      ),
      ...
      {modes.map(({ mode, title, description, icon }) => (
        <Card
          ...
          sx={{
            minWidth: 0,
            textAlign: 'center',
            ...(importMode === mode ? { borderColor: theme.palette.primary.main } : {}),
          }}
        >
          <CardContent
            sx={{
