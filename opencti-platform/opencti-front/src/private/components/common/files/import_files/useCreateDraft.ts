...
const useCreateDraft = (
  ...
  return useCallback(
    async (values: DraftAddInput, selectedEntityId?: string): Promise<string | undefined> => {
      try {
        const { draftWorkspaceAdd } = await new Promise<DraftCreationMutation$data>(
          (resolve, reject) => {
