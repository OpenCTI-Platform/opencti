...
import {
  AuditsListComponentQuery,
  LogsOrdering,
  OrderingMode,
} from './__generated__/AuditsListComponentQuery.graphql';
...
const AuditsListComponent: FunctionComponent<AuditsListComponentProps> = ({ queryRef }) => {
  ...
  if (queryData && queryData.audits?.edges && queryData.audits.edges.length > 0) {
    ...
    return <WidgetListAudits data={data} />;
  }
  return <WidgetNoData />;
};
...
const AuditsList: FunctionComponent<AuditsListProps> = ({
  ...
  const buildQueryVariables = useCallback(
    (resolvedSelection: WidgetDataSelection[]): AuditsListComponentQuery['variables'] => {
      ...
      const { filters } = buildFiltersAndOptionsForWidgets(selection.filters ?? undefined, {
        removeTypeAll: true,
  ...
  const { isMissingHostEntity, isMissingSavedFilters, isPreviewMode, queryRef } =
    useDashboardViz<AuditsListComponentQuery>({
      perspective: 'audits',
      dataSelection,
      host,
      refreshRate,
