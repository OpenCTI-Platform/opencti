...
import {
  AISummaryContainersSubscription,
  AISummaryContainersSubscription$data,
} from './__generated__/AISummaryContainersSubscription.graphql';
  ...
  return (
    <>
      <Alert severity="info" variant="outlined" style={{ marginTop: 20 }}>
        {t_i18n(
          'This summary is based on the whole content of related containers (description, content and attached files). It has been generated by AI and can contain mistakes.',
        )}
        {!isContainer && (
        ...
        <>
          ...
          >
            <Typography variant="caption">
              Generated on {nsdt(result?.containersAskAiSummary?.updated_at)}.
            </Typography>
...
const XtmOneContainerSummary = ({
  isContainer,
  filters,
  loading,
  setLoading,
  selectedAgent,
}: XtmOneContainerSummaryProps) => {
  ...
  const buildPrompt = useCallback((): string => {
    ...
    if (isContainer && idFilter && idFilter.values.length > 0) {
      ...
      return (
        `Summarize the OpenCTI container with ID: ${containerId}. ` +
    ...
    return 'Summarize the most recent OpenCTI containers.' + `Answer using ${language} language.`;
...
const LegacyAISummaryContainers = ({
  busId,
  isContainer,
  filters,
  loading,
  setLoading,
}: ContainersAiSummaryProps) => {
  const defaultLanguageName = getDefaultAiLanguage();
  const [first, setFirst] = useState(isContainer ? 1 : 10);
  const [relative, setRelative] = useState('none');
  const [content, setContent] = useState('');
  const [result, setResult] = useState<AISummaryContainersContainersAskAiSummaryQuery$data | null>(
    null,
  );
  ...
  const handleResponse = (response: AISummaryContainersSubscription$data | null | undefined) => {
    const newContent = response
      ? (response as AISummaryContainersSubscription$data).aiBus?.content
      : null;
  ...
  useEffect(() => {
    setLoading(true);
    fetchQuery(aISummaryContainersQuery, queryParams)
      .toPromise()
      .then((data) => {
...
const AISummaryContainers = ({
  ...
  setLoading,
  selectedAgent,
}: ContainersAiSummaryProps & { selectedAgent?: AgentOption | null }) => {
