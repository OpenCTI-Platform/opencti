...
import {
  AISummaryActivitySubscription,
  AISummaryActivitySubscription$data,
} from './__generated__/AISummaryActivitySubscription.graphql';
...
const aISummaryActivityQuery = graphql`
  query AISummaryActivityStixCoreObjectAskAiActivityQuery(
    $id: ID!
    $language: String
...
const AISummaryActivityComponent = ({
  ...
  const generateTrend = (trend: string) => {
    if (loading) {
      return (
        ...
        >
          ...
          <i>
            {t_i18n(
              'This summary is based on the evolution of the activity of this entity (indicators, victimology, etc.). It has been generated by AI and can contain mistakes.',
            )}
          </i>
        </Alert>
      );
    ...
    switch (trend) {
      case 'increasing':
        return (
          <Alert severity="error" variant="outlined" style={{ marginTop: 20 }}>
            <AlertTitle>
              {t_i18n('Activity evaluation')}: <strong>{t_i18n('Increasing')}</strong> -{' '}
              {t_i18n('The threat activity is showing a significant increase.')}
            </AlertTitle>
            <i>
              {t_i18n(
                'This summary is based on the evolution of the activity of this entity (indicators, victimology, etc.). It has been generated by AI and can contain mistakes.',
      ...
      case 'stable':
        return (
          <Alert severity="warning" variant="outlined" style={{ marginTop: 20 }}>
            <AlertTitle>
              {t_i18n('Activity evaluation')}: <strong>{t_i18n('Stable')}</strong> -{' '}
              {t_i18n('The threat activity is stable with minor fluctuations.')}
            </AlertTitle>
            <i>
              {t_i18n(
                'This summary is based on the evolution of the activity of this entity (indicators, victimology, etc.). It has been generated by AI and can contain mistakes.',
      ...
      case 'decreasing':
        return (
          <Alert severity="success" variant="outlined" style={{ marginTop: 20 }}>
            <AlertTitle>
              {t_i18n('Activity evaluation')}: <strong>{t_i18n('Decreasing')}</strong> -{' '}
              {t_i18n('The threat activity is showing a significant decrease.')}
            </AlertTitle>
            <i>
              {t_i18n(
                'This summary is based on the evolution of the activity of this entity (indicators, victimology, etc.). It has been generated by AI and can contain mistakes.',
      ...
      default:
        return (
          <Alert severity="info" variant="outlined" style={{ marginTop: 20 }}>
            <AlertTitle>
              {t_i18n('Activity evaluation')}: <strong>{t_i18n('Unknown')}</strong> -{' '}
              {t_i18n('The evaluation of the threat activity cannot be provided (lack of data).')}
            </AlertTitle>
            <i>
              {t_i18n(
                'This summary is based on the evolution of the activity of this entity (indicators, victimology, etc.). It has been generated by AI and can contain mistakes.',
        ...
        <>
          ...
          >
            <Typography variant="caption">
              Generated on {nsdt(result?.stixCoreObjectAskAiActivity?.updated_at)}.
            </Typography>
...
const LegacyAISummaryActivity = ({ id, loading, setLoading }: AISummaryActivityProps) => {
  ...
  const [result, setResult] =
    useState<AISummaryActivityStixCoreObjectAskAiActivityQuery$data | null>(null);
  ...
  const handleResponse = (response: AISummaryActivitySubscription$data | null | undefined) => {
    const newContent = response
      ? (response as AISummaryActivitySubscription$data).aiBus?.content
      : null;
  ...
  useEffect(() => {
    setLoading(true);
    fetchQuery(aISummaryActivityQuery, queryParams)
      .toPromise()
      .then((data) => {
  ...
  const refetch = useCallback(() => {
    setContent('');
    setLoading(true);
    fetchQuery(aISummaryActivityQuery, { ...queryParams, forceRefresh: true })
      .toPromise()
      .then((data) => {
        const resultData = data as AISummaryActivityStixCoreObjectAskAiActivityQuery$data;
        if (resultData && resultData.stixCoreObjectAskAiActivity) {
...
const XtmOneActivity = ({ id, loading, setLoading, selectedAgent }: XtmOneActivityProps) => {
  ...
  const executeCall = useCallback(
    (forceRefresh = false) => {
      ...
      const prompt =
        `Analyse the activity of the OpenCTI entity with ID: ${id}. ` +
...
const AISummaryActivity = ({ id, loading, setLoading, selectedAgent }: AISummaryActivityProps) => {
  ...
  return <LegacyAISummaryActivity id={id} loading={loading} setLoading={setLoading} />;
};
