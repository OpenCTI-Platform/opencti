  ...
  const renderForm = () => {
    ...
    return (
      <>
        ...
        {!isSimulatedEmailsAvailable && (
          <Alert
            severity="warning"
            variant="outlined"
            style={{ marginTop: 5 }}
          >
            {t_i18n('Simulation type : Simulated emails (generated by AI) is only available with an EE license and AI activated.')}
          </Alert>
        )}
        <FormControl style={fieldSpacingContainerStyle}>
          ...
          >
            ...
            <MenuItem value="simulated" disabled={!isSimulatedEmailsAvailable}>{t_i18n('Simulated emails (generated by AI)')}{!isEnterpriseEdition && <EEChip />}</MenuItem>
    ...
    return (
      <>
        <Alert icon={<CheckOutlined fontSize="inherit" />} severity="success">
          ...
          {result.attackPatternWithoutInjectorContracts.length > 0 && (
            <>
              {t_i18n(' The next attack patterns don\'t have any contract linked: ')}
              {result.attackPatternWithoutInjectorContracts.join(', ')}
            </>
          )}
        </Alert>
