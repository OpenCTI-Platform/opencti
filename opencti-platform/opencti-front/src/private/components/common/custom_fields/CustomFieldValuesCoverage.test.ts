...
// SecurityCoverage nests its creation wizard under its own sub-folder, and splits the form fields
// (with <CustomFieldValuesCreation />) into the last step's dedicated component, unlike every other
// SDO screen which keeps everything in a single Creation.tsx file.
const creationContentOverrides: Record<string, () => string> = {
  'analyses/security_coverages/SecurityCoverage': () => (
    source('analyses/security_coverages/security_coverage_creation/SecurityCoverageCreation.tsx')
    + source('analyses/security_coverages/security_coverage_creation/CoverageDetailsStep.tsx')
  ),
};
...
// Some Overview components were already migrated to .tsx while others remain .jsx.
const overviewPath = (prefix: string) => `${prefix}Overview.${existsSync(resolve(root, `${prefix}Overview.tsx`)) ? 'tsx' : 'jsx'}`;
...
describe('SDO custom-field integration coverage', () => {
  it.each(screens)('%s uses generic custom fields in creation and edition', (screen) => {
    const creation = creationContent(screen);
...
describe.each([
  ['common/stix_core_relationships/StixCoreRelationship', 'stix-core-relationship', 'stixCoreRelationship', 'STIX_CORE_RELATIONSHIP_TYPE'],
  ['events/stix_sighting_relationships/StixSightingRelationship', 'stix-sighting-relationship', 'stixSightingRelationship', 'STIX_SIGHTING_TYPE'],
])('%s custom-field integration coverage', (prefix, entityType, variable, typeConstant) => {
  const isSighting = entityType === 'stix-sighting-relationship';

  it('uses the configured relationship type in the shared creation form', () => {
    const creation = source(`${prefix}CreationForm.jsx`);
    expect(creation).toContain(`const ${typeConstant} = '${entityType}'`);
    expect(creation).toMatch(new RegExp(`<CustomFieldsFormik\\s+entityType=\\{${typeConstant}\\}`));
    expect(creation).toContain('<CustomFieldValuesCreation />');
    if (isSighting) {
      for (const suffix of ['Creation.jsx', 'CreationFromEntity.jsx']) {
        const owner = source(`${prefix}${suffix}`);
        expect(owner).toContain('<StixSightingRelationshipCreationForm');
        expect(owner).toContain(')(values)');
        expect(owner).toContain('input: finalValues');
      }
    } else {
  ...
  it('loads stored values and preserves reference-enforced commits and inferred guards', () => {
    ...
    expect(edition).toMatch(new RegExp(`!${variable}\\.is_inferred && \\(\\s*<CustomFieldValuesEdition`));
    if (isSighting) expect(edition).toContain('!inferred && !stixSightingRelationship.is_inferred');
    expect(edition).toContain(`entityType={${isSighting ? typeConstant : 'stixCoreRelationshipType'}}`);
    expect(edition).toContain(`values={${variable}.customFieldValues ?? []}`);
    expect(edition).toContain('fieldPatch={editor.fieldPatch}');
    expect(edition).toContain('enableReferences={enableReferences}');
    expect(edition).toContain('...otherValues,');
    expect(edition).toContain(`...${isSighting ? 'StixSightingRelationship' : 'StixCoreRelationship'}Overview_${variable}`);
  });

  it('displays custom values exactly once inside the Details card grid', () => {
    const path = overviewPath(prefix);
    const content = source(path);
    ...
    expect(display.getText(parsed)).toContain(`entityType="${entityType}"`);
    expect(display.getText(parsed)).toContain(`values={${variable}.customFieldValues ?? []}`);
    ...
    // The translation helper is named either `t` or `t_i18n` depending on when the file was last migrated.
    if (ts.isJsxElement(card)) expect(card.openingElement.getText(parsed)).toMatch(/^<Card title=\{t(_i18n)?\('Details'\)}>$/);
  });
});
