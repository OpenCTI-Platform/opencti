...
const DashboardFieldComponent: FunctionComponent<DashboardFieldProps> = ({ onChange, context, queryRef }) => {
  ...
  return (
    <Field
      ...
      options={(workspaces?.edges ?? []).map(({ node: { id, name } }) => ({ value: id, label: name }))}
