...
export const DefaultCustomViewTab = ({ basePath, value, displayMode, defaultCustomView }: DefaultCustomViewTabProps) => {
  if (!displayMode.default || !defaultCustomView) {
    return null;
  }
  return (
    <TabsTrigger value={value} asChild>
      <Link to={`${basePath}/${defaultCustomView.path}`}>{defaultCustomView.name}</Link>
    </TabsTrigger>
  );
};
