...
const AttackPatternsMatrixBadge = ({
  attackPattern,
  color,
  textColor,
  children,
}: AttackPatternsMatrixBadgeProps) => {
  const theme = useTheme<Theme>();
  const attackPatternsCount =
    (attackPattern.isCovered ? 1 : 0) +
    (attackPattern.subAttackPatterns?.filter((sub: FilteredSubAttackPattern) => sub.isCovered)
      .length || 0);
  ...
  return (
    <Badge
      ...
      badgeContent={
        attackPattern.subAttackPatternsTotal &&
        `${attackPatternsCount}/${attackPattern.subAttackPatternsTotal + 1}`
      } // We add 1 to count the parent
      overlap="rectangular"
      anchorOrigin={{
        vertical: 'top',
        horizontal: 'right',
