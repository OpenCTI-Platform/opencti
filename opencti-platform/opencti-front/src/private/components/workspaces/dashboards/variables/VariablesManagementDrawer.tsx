...
const TYPE_LABELS: Record<string, string> = {
  ...
  label: 'Label picker',
  marking: 'Marking picker',
  entity_type: 'Entity type picker',
  relation_type: 'Relation type picker',
  user: 'User picker',
  boolean: 'Boolean',
...
const resolveDefaultEntityId = (rawDefaultValue?: string | null): string | null => {
  const parsedRestriction = parseEntityPickerRestriction(rawDefaultValue ?? null);
  return parsedRestriction?.defaultEntityId ?? null;
};
                      ...
                      {variable.defaultValue
                        ? (
                            variable.filterKeyType === 'entity_ref'
                              ? (
                                  <Suspense fallback={<>{resolveDefaultEntityId(variable.defaultValue) ?? variable.defaultValue}</>}>
                                    <EntityDefaultValueLabel
                                      entityId={resolveDefaultEntityId(variable.defaultValue) ?? variable.defaultValue}
                                      fallback={resolveDefaultEntityId(variable.defaultValue) ?? variable.defaultValue}
                                    />
                                  </Suspense>
                                )
                              : variable.defaultValue
