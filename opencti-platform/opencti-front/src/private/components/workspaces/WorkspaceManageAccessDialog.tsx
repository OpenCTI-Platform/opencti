...
import {
  WorkspaceManageAccessDialog_authorizedMembers$key,
} from './__generated__/WorkspaceManageAccessDialog_authorizedMembers.graphql';
...
const workspaceManageAccessDialogEditAuthorizedMembersMutation = graphql`
  mutation WorkspaceManageAccessDialogEditAuthorizedMembersMutation($id:ID! $input: [MemberAccessInput!]!) {
...
const WorkspaceManageAccessDialog: FunctionComponent<WorkspaceManageAccessDialogProps> = ({
  ...
  const [commit] = useMutation(workspaceManageAccessDialogEditAuthorizedMembersMutation);

  const data = useFragment<WorkspaceManageAccessDialog_authorizedMembers$key>(workspaceManageAccessDialogAuthorizedMembersFragment, authorizedMembersData);
  ...
  const getInitialAuthorizedMembers = () => {
    const initialAuthorizedMembers: MemberAccess[] = authorizedMembers ? authorizedMembers.map((member) => {
      return { ...member } as MemberAccess;
    }) : [];
    const allMemberDefault = { ...allMemberAccess };
    if (!initialAuthorizedMembers.length) { // empty, no restricted access
      // add owner as admin
      initialAuthorizedMembers.push({ id: owner.id, name: owner.name, entity_type: owner.entity_type, access_right: 'admin' });
      // everyone is admin by default
      allMemberDefault.access_right = 'admin';
    }
    const allMember = initialAuthorizedMembers.find((e) => e.id === allMemberAccess.id);
    if (!allMember) {
  ...
  const onAddMember = (values: MembersForm, arrayHelpers: ArrayHelpers, setFieldValue: (field: string, value: { value: string, label: string, type: string }) => void) => {
    if (values.objectMember && values.objectMember.value && values.objectMemberAccessRight
        && !values.authorizedMembers.find((node) => node.id === values.objectMember.value)) {
      arrayHelpers.push({
  ...
  const onSubmitForm = (values: MembersForm, { setSubmitting, resetForm, setErrors }: FormikHelpers<MembersForm>) => {
    const finalValues = values.authorizedMembers.filter((item, index, array) => {
  ...
  return (
    ...
    >
      {({ submitForm, isSubmitting, dirty, values, handleReset, setFieldValue }) => (
        <Dialog
          open={open}
                ...
                render={(arrayHelpers) => (
                  <div>
                    <Alert
                        ...
                        icon={false}
                        variant="outlined"
                        style={{ width: '100%', position: 'relative', paddingBottom: '16px' }}
                    >
                        ...
                        <div style={{ paddingRight: '8px' }}>
                          <Field
                            ...
                            {accessRights.map((accessRight) => {
                              return (<MenuItem value={accessRight.value} key={accessRight.value}>
                                  {accessRight.label}
                                </MenuItem>);
                            })}
                          </Field>
                    ...
                    <DialogContentText>
                      <Typography variant="h3" gutterBottom={true} classes={{ root: classes.subtitle }}>
                        {t('Current members access')}
                      </Typography>
                    </DialogContentText>
                    <List style={{ marginBottom: 0 }}>
                      {values.authorizedMembers.map((authorizedMember, index) => (
                        <ListItem
                        ...
                        >
                          ...
                          <ListItemText
                            primary={
                              <div>
                                {(authorizedMember.name && authorizedMember.entity_type) ? authorizedMember.name : (
                                    <span style={{ opacity: 0.6, fontStyle: 'italic' }}>{t('Deleted or restricted member')}</span>
                                )}
                                <span style={{ opacity: 0.6, fontStyle: 'italic' }}>
                                  {authorizedMember.id === me.id ? ` (${t('you')})` : ''} {authorizedMember.id === owner.id ? ` - ${t('Creator')}` : ''}
                                </span>
                              </div>
                            }
                          />
                          <Field
                            ...
                            {authorizedMember.id === allMemberAccess.id && (
                              <MenuItem value={noAccessRight.value} key={noAccessRight.value}>
                                {noAccessRight.label}
                              </MenuItem>
                            )}
                            {accessRights.map((accessRight) => {
                          ...
                          {authorizedMember.id !== allMemberAccess.id && authorizedMember.id !== me.id ? (
                            <IconButton
                              color="primary"
                              aria-tag="delete"
