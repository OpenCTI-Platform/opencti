...
const WorkspaceHeaderTagManager = ({
  tags,
  workspaceId,
  canEdit,
}: WorkspaceHeaderTagManagerProps) => {
  ...
  const onSubmitCreateTag: FormikConfig<WorkspaceHeaderTagCreatorFormValues>['onSubmit'] = (
    data,
    { resetForm },
  ) => {
  ...
  return (
    <>
      <Security needs={[EXPLORE_EXUPDATE, INVESTIGATION_INUPDATE]} hasAccess={canEdit}>
        <>
          {tags.length > 1 ? (
            <IconButton color="primary" aria-label="More" onClick={toggleTagDialog}>
              <DotsHorizontalCircleOutline fontSize="small" />
            </IconButton>
          ) : (
            <Tooltip title={isTagInputOpen ? t_i18n('Cancel') : t_i18n('Add tag')}>
              <IconButton color="primary" aria-label="Add tag" onClick={toggleTagInput}>
                {isTagInputOpen ? (
          ...
          <Slide direction="right" in={isTagInputOpen} mountOnEnter unmountOnExit>
            <div>
              <Formik initialValues={{ newTag: '' }} onSubmit={onSubmitCreateTag}>
                <Form>
                  <Field
                    component={TextField}
                    variant="standard"
          ...
          <Dialog open={isTagDialogOpen} onClose={toggleTagDialog} title={t_i18n('Entity tags')}>
            <Formik initialValues={{ newTag: '' }} onSubmit={onSubmitCreateTag}>
            ...
            <List>
              {tags.map(
                (label) =>
                  label.length > 0 && (
                    <ListItem key={label} disableGutters dense>
                      <ListItemText primary={label} />
                      <ListItemSecondaryAction>
                        <IconButton aria-label="delete" onClick={deleteTag(label)}>
                          <Delete />
                        </IconButton>
                      </ListItemSecondaryAction>
                    </ListItem>
            ...
            <DialogActions>
              <Button onClick={toggleTagDialog}>{t_i18n('Close')}</Button>
            </DialogActions>
          </Dialog>
        </>
      </Security>
      <div style={{ display: 'flex', gap: 7 }}>
        {tags
          .slice(0, 2)
          .map((tag) => tag.length > 0 && <Tag key={tag} label={tag} onDelete={deleteTag(tag)} />)}
      </div>
    </>
  );
};
