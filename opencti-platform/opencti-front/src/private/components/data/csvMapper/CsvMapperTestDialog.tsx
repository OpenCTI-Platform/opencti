...
const CsvMapperTestDialog: FunctionComponent<CsvMapperTestDialogProps> = ({
  ...
  return (
    <Dialog open={open} onClose={handleClose} title={t_i18n('Testing csv mapper')}>
      <Box
        sx={{
          display: 'flex',
          flexDirection: 'row',
      ...
      <Box sx={{ display: 'inline-flex', textAlign: 'center', marginTop: '8px' }}>
        <Button disabled={!value || loading} onClick={onTest}>
          {t_i18n('Test')}
        </Button>
        {loading && (
      ...
      {result && (
        ...
        >
          ...
          <span>
            <strong>{result?.csvMapperTest?.nbEntities} </strong> {t_i18n('Entities')}
          </span>
          <span>
            <strong>{result?.csvMapperTest?.nbRelationships}</strong> {t_i18n('Relationships')}
          </span>
        </Box>
      )}
