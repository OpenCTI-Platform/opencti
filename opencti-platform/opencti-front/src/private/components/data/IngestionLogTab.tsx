...
export interface IngestionLogEntryShape {
...
interface IngestionLogTabProps {
  name: string;
  logHistory: ReadonlyArray<IngestionLogEntryShape>;
}
...
const jsonTokenColor = (type: JsonTokenType, theme: Theme): string => {
  ...
  switch (type) {
    case 'punctuation':
      return isDark ? '#fefefe' : '#5F6364';
    case 'key':
      return isDark ? '#ffa07a' : '#c92c2c';
    case 'string':
      return isDark ? '#abe338' : '#2f9c0a';
    case 'number':
      return isDark ? '#00e0e0' : '#c92c2c';
    case 'keyword':
      return isDark ? '#00e0e0' : '#1990b8';
    default:
      return theme.palette.text?.primary ?? '#000';
  }
};

const IngestionLogTab: React.FC<IngestionLogTabProps> = ({ name, logHistory }) => {
  const { t_i18n } = useFormatter();
  const theme = useTheme<Theme>();
  const [detailsOpen, setDetailsOpen] = useState<{ index: number; entry: IngestionLogEntryShape } | null>(null);
  ...
  if (!logHistory || logHistory.length === 0) {
    return (
      <Box sx={{ py: 2, color: 'text.secondary' }}>
        {t_i18n('No log entries yet. Logs appear here when ingestion attempts occur.')}
      </Box>
    );
  }
  ...
  return (
    <>
      ...
      <Drawer
        ...
        size="large"
        header={
          detailsOpen !== null && logHistory.length > 0 ? (
                  ...
                  <IconButton
                    ...
                    onClick={() => {
                      if (detailsOpen.index > 0) {
                        ...
                        setDetailsOpen({ index: prevIndex, entry: logHistory[prevIndex] });
                      }
                    }}
                  ...
                  <IconButton
                    ...
                    onClick={() => {
                      if (detailsOpen.index < logHistory.length - 1) {
                        ...
                        setDetailsOpen({ index: nextIndex, entry: logHistory[nextIndex] });
                      }
                    }}
...
export default IngestionLogTab;
