...
import {
  ObservedDatasLinesPaginationQuery,
  ObservedDatasLinesPaginationQuery$variables,
} from '@components/events/__generated__/ObservedDatasLinesPaginationQuery.graphql';
...
const ObservedDatas: FunctionComponent = () => {
  ...
  return (
    <span data-testid="observed-data">
      <Breadcrumbs
        elements={[{ label: t_i18n('Events') }, { label: t_i18n('Observed datas'), current: true }]}
      />
      {queryRef && (
        <DataTable
          dataColumns={dataColumns}
          resolvePath={(data: ObservedDatasLines_data$data) =>
            data.observedDatas?.edges?.map((n) => n?.node)
          }
