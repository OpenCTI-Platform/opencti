...
const TargetedCountriesComponent = ({ queryRef }) => {
  ...
  const countries = map(
    (x) => assoc('level', computeLevel(x.value, last(values), head(values) + 1), x.entity),
...
const DefaultDashboard = ({ timeField }) => {
  ...
  const noAccessMessage =
    settings.platform_no_access_message ??
  ...
  return (
    <Security needs={[KNOWLEDGE]} placeholder={<MarkdownDisplay content={noAccessMessage} />}>
    ...
    <UserContext.Provider value={dashboardContextValue}>
      <div className={classes.root} data-testid="dashboard-page">
        ...
        {defaultDashboard !== PLATFORM_DASHBOARD ? (
          <CustomHomeDashboard dashboard={defaultDashboard} timeField={default_time_field} />
        ) : (
