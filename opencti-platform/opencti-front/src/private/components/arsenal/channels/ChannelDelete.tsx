...
const ChannelDeleteMutation = graphql`
  mutation ChannelDeleteMutation($id: ID!) {
    channelDelete(id: $id)
  }
`;

const ChannelDelete = ({ id, handleClose }: {
  id: string,
  handleClose?: () => void,
}) => {
  const { t_i18n } = useFormatter();
  const navigate = useNavigate();
  const [commit] = useMutation(ChannelDeleteMutation);
  const {
    deleting,
    handleOpenDelete,
    displayDelete,
  ...
  const submitDelete = () => {
    ...
    commit({
      ...
      onCompleted: () => {
        setDeleting(false);
        MESSAGING$.notifySuccess(`${t_i18n('entity_Channel')} ${t_i18n('successfully deleted')}`);
