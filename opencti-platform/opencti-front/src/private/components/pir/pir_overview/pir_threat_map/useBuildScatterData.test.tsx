...
describe('Hook: useBuildScatterData', () => {
  ...
  it('should set group color as type color if one entity in group', () => {
    ...
    expect(data[0].data[0].fillColor).toEqual(pirEntityColor(sdo1.node.entity_type));
    expect(data[1].data[0].fillColor).toEqual(pirEntityColor(sdo2.node.entity_type));
  });
});
