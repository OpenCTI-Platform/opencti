...
export const pirLogRedirectUri = (
  ...
  let redirectionId;
  if (context?.entity_type === 'in-pir') {
    // in-pir: redirect to the flagged entity (i.e. the relationship source entity)
    redirectionId = context?.from_id;
  } else {
    // relationships: redirect to the flagged entity (if both from and to are flagged, redirect to the source entity)
