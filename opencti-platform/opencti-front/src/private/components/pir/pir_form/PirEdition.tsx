...
const PirEdition = ({ data, isOpen, onClose }: PirEditionProps) => {
  const { t_i18n } = useFormatter();
  const pir = useFragment(editionFragment, data);

  const [editMutation] = useApiMutation<PirEditionMutation>(pirEditMutation, undefined, {
  ...
  return (
    <Drawer title={t_i18n('Update a PIR')} open={isOpen} onClose={onClose}>
