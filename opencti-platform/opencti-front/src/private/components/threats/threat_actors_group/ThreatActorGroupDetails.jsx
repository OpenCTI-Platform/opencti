...
class ThreatActorGroupDetailsComponent extends Component {
  render() {
    ...
    const hasImages =
      (threatActorGroup.images?.edges ?? []).filter((n) => n?.node?.metaData?.inCarousel).length >
      0;
    ...
    return (
      <Card title={t('Details')}>
          ...
          <Grid item xs={hasImages ? 7 : 6}>
            <Grid container={true} spacing={2}>
              ...
              <Grid item xs={hasImages ? 8 : 12}>
                <Label>{t('Threat actor types')}</Label>
                  ...
                  <Stack direction="row" flexWrap="wrap" gap={1}>
                    {threatActorGroup.threat_actor_types &&
                      threatActorGroup.threat_actor_types.map((threatActorGroupType) => (
                        <Tag key={threatActorGroupType} label={threatActorGroupType} />
                      ))}
                  </Stack>
                </FieldOrEmpty>
              </Grid>
        ...
        <Grid container={true} spacing={2}>
          <Grid item xs={4}>
            <Label sx={{ marginTop: 2 }}>{t('Sophistication')}</Label>
          ...
          <Grid item xs={4}>
            <Label sx={{ marginTop: 2 }}>{t('Resource level')}</Label>
          ...
          <Grid item xs={4}>
            <Label sx={{ marginTop: 2 }}>{t('Primary motivation')}</Label>
          ...
          <Grid item xs={4}>
            <Label>{t('Roles')}</Label>
            <TextList list={threatActorGroup.roles} />
          </Grid>
          <Grid item xs={4}>
            <Label>{t('Goals')}</Label>
            <TextList list={threatActorGroup.goals} />
          </Grid>
          <Grid item xs={4}>
            <Label>{t('Secondary motivations')}</Label>
            <TextList list={threatActorGroup.secondary_motivations} />
          </Grid>
        </Grid>
      </Card>
    );
...
const ThreatActorGroupDetails = createFragmentContainer(ThreatActorGroupDetailsComponent, {
