...
describe('Elasticsearch computation', () => {
  it('should count accurate', async () => {
    ...
    expect(malwaresCount).toEqual(entitiesCounter.Malware);
  });
...
describe('Elasticsearch pagination', () => {
  ...
  it('should entity paginate domain objects sort by published', async () => {
    ...
    expect(entityTypeMap.get('Organization')).toBe(entitiesCounter.Organization);
    expect(entityTypeMap.get('Incident')).toBe(entitiesCounter.Incident);
    expect(entityTypeMap.get('Indicator')).toBe(2); // we fetch only the first 20
  ...
  it('should entity paginate with dates filter', async () => {
    ...
    expect(entityTypeMap.get('External-Reference')).toBe(entitiesCounter.ExternalReference);
    expect(entityTypeMap.get('Individual')).toBe(entitiesCounter.Individual);
    expect(entityTypeMap.get('Organization')).toBe(TESTING_ORGS.length); // because of created filter
    expect(entityTypeMap.get('Incident')).toBe(entitiesCounter.Incident);
    expect(entityTypeMap.get('Kill-Chain-Phase')).toBe(entitiesCounter.KillChainPhase);
    expect(entityTypeMap.get('Malware-Analysis')).toBe(entitiesCounter.MalwareAnalysis);
  ...
  it('should entity paginate with object ordering (desc)', async () => {
    ...
    expect(data.edges.length).toEqual(entitiesCounter.Group);
  ...
  it('should relation paginate everything', async () => {
    ...
    expect(internalRelationshipsByType['accesses-to'].length, 'accesses-to').toEqual(relationsCounter['accesses-to']);
    expect(internalRelationshipsByType['has-capability'].length, 'has-capability').toEqual(relationsCounter['has-capability']);
    expect(internalRelationshipsByType['has-role'].length, 'has-role').toEqual(relationsCounter['has-role']);
    expect(internalRelationshipsByType['member-of'].length, 'member-of').toEqual(relationsCounter['member-of']);
    expect(internalRelationshipsByType['participate-to'].length, 'participate-to').toEqual(relationsCounter['participate-to']);
    ...
    expect(stixCoreRelationshipsByType['attributed-to'].length, 'attributed-to').toEqual(relationsCounter['attributed-to']);
    expect(stixCoreRelationshipsByType.indicates.length, 'indicates').toEqual(relationsCounter.indicates);
    expect(stixCoreRelationshipsByType['located-at'].length, 'located-at').toEqual(relationsCounter['located-at']);
    expect(stixCoreRelationshipsByType.mitigates.length, 'mitigates').toEqual(relationsCounter.mitigates);
    expect(stixCoreRelationshipsByType['part-of'].length, 'part-of').toEqual(relationsCounter['part-of']);
    expect(stixCoreRelationshipsByType['related-to'].length, 'related-to').toEqual(relationsCounter['related-to']);
    expect(stixCoreRelationshipsByType.targets.length, 'targets').toEqual(relationsCounter.targets);
    expect(stixCoreRelationshipsByType.uses.length, 'uses').toEqual(relationsCounter.uses);
    expect(groupByIndices[`${ES_INDEX_PREFIX}_stix_core_relationships-000001`].length, 'stix_core_relationships count').toEqual(24);
    ...
    expect(stixMetaRelationshipsByType['created-by'].length, 'created-by').toEqual(relationsCounter['created-by']);
    expect(stixMetaRelationshipsByType['external-reference'].length, 'external-reference').toEqual(relationsCounter['external-reference']);
    expect(stixMetaRelationshipsByType['kill-chain-phase'].length, 'kill-chain-phase').toEqual(relationsCounter['kill-chain-phase']);
    expect(stixMetaRelationshipsByType['object-label'].length, 'object-label').toEqual(relationsCounter['object-label']);
    ...
    expect(metaByEntityType.object.length, 'object').toEqual(relationsCounter.object);
    expect(metaByEntityType['object-label'].length, 'object-label').toEqual(relationsCounter['object-label']);
    expect(metaByEntityType['created-by'].length, 'created-by').toEqual(relationsCounter['created-by']);
    expect(metaByEntityType['external-reference'].length, 'external-reference').toEqual(relationsCounter['external-reference']);
    expect(metaByEntityType['object-marking'].length, 'object-marking').toEqual(relationsCounter['object-marking']);
    expect(metaByEntityType['kill-chain-phase'].length, 'kill-chain-phase').toEqual(relationsCounter['kill-chain-phase']);
    expect(metaByEntityType['operating-system'].length, 'operating-system').toEqual(relationsCounter['operating-system']);
    ...
    expect(entityTypeMap.get('has-capability'), 'has-capability').toBe(relationsCounter['has-capability']);
    expect(entityTypeMap.get('accesses-to'), 'accesses-to').toBe(relationsCounter['accesses-to']);
    expect(entityTypeMap.get('member-of'), 'member-of').toBe(relationsCounter['member-of']);
    expect(entityTypeMap.get('has-role'), 'has-role').toBe(relationsCounter['has-role']);
    expect(entityTypeMap.get('participate-to'), 'participate-to').toBe(relationsCounter['participate-to']);
    expect(entityTypeMap.get('uses'), 'uses').toBe(relationsCounter.uses);
    expect(entityTypeMap.get('part-of'), 'part-of').toBe(relationsCounter['part-of']);
    expect(entityTypeMap.get('indicates'), 'indicates').toBe(relationsCounter.indicates);
    expect(entityTypeMap.get('located-at'), 'located-at').toBe(relationsCounter['located-at']);
    expect(entityTypeMap.get('targets'), 'targets').toBe(relationsCounter.targets);
    expect(entityTypeMap.get('mitigates'), 'mitigates').toBe(relationsCounter.mitigates);
    expect(entityTypeMap.get('related-to'), 'related-to').toBe(relationsCounter['related-to']);
    expect(entityTypeMap.get('attributed-to'), 'attributed-to').toBe(relationsCounter['attributed-to']);
    expect(entityTypeMap.get('created-by'), 'created-by').toBe(relationsCounter['created-by']);
    expect(entityTypeMap.get('object'), 'object').toBe(relationsCounter.object);
    expect(entityTypeMap.get('object-marking'), 'object-marking').toBe(relationsCounter['object-marking']);
    expect(entityTypeMap.get('object-label'), 'object-label').toBe(relationsCounter['object-label']);
    expect(entityTypeMap.get('kill-chain-phase'), 'kill-chain-phase').toBe(relationsCounter['kill-chain-phase']);
    expect(entityTypeMap.get('external-reference'), 'external-reference').toBe(relationsCounter['external-reference']);
    expect(entityTypeMap.get('operating-system'), 'operating-system').toBe(relationsCounter['operating-system']);
    expect(entityTypeMap.get('stix-sighting-relationship'), 'stix-sighting-relationship').toBe(relationsCounter['stix-sighting-relationship']);
