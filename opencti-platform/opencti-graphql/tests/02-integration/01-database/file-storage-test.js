...
describe('File storage file listing', () => {
  ...
  it('should file listing', async () => {
    const context = executionContext('should_file_listing');
    const malware = await elLoadById(context, ADMIN_USER, 'malware--faa5b705-cf44-4e50-8472-29e5fec43c3c');
    let list = await filesListing(context, ADMIN_USER, 25, `export/Malware/${malware.id}/`);
    ...
    list = await filesListing(context, ADMIN_USER, 25, 'import/global/');
    expect(list).not.toBeNull();
    expect(list.edges.length).toEqual(1);
  ...
  it('should file download', async () => {
    const malware = await elLoadById(context, ADMIN_USER, 'malware--faa5b705-cf44-4e50-8472-29e5fec43c3c');
    const fileStream = await downloadFile(context, exportFileId(malware));
  ...
  it('should load file', async () => {
    const malware = await elLoadById(context, ADMIN_USER, 'malware--faa5b705-cf44-4e50-8472-29e5fec43c3c');
    const file = await loadFile(context, ADMIN_USER, exportFileId(malware));
  ...
  it('should delete file', async () => {
    const malware = await elLoadById(context, ADMIN_USER, 'malware--faa5b705-cf44-4e50-8472-29e5fec43c3c');
    let deleted = await deleteFile(context, ADMIN_USER, exportFileId(malware));
    expect(deleted).toBeTruthy();
    deleted = await deleteFile(context, ADMIN_USER, importFileId);
    expect(deleted).toBeTruthy();
  });
});
