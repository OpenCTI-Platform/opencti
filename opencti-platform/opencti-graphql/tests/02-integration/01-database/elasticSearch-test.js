...
import { elAggregationCount, elAggregationRelationsCount, elCount, elCreateIndices, elDeleteElements, elDeleteIndices, elHistogramCount, elIndex, elIndexElements, elIndexExists, elLoadById } from '../../../src/database/engine';
import { elRebuildRelation, searchEngineInit, elPaginate } from '../../../src/database/engine-loader';
...
describe('Elasticsearch pagination', () => {
  it('should entity paginate everything', async () => {
    ...
    expect(data.edges.length).toEqual(491);
  ...
  it('should entity paginate with field not exist filter', async () => {
    ...
    expect(data.edges.length).toEqual(484);
  });
