...
describe('Fintel template resolver standard behavior', () => {
  ...
  it('should fintel template widget edited via object_path', async () => {
    ...
    expect(queryResult2).not.toBeNull();
    expect(queryResult2.data?.fintelTemplate.fintel_template_widgets.length).toEqual(1);
    const widgetResult = queryResult2.data?.fintelTemplate.fintel_template_widgets[0].widget;
    expect(widgetResult.type).toEqual('list');
    expect(widgetResult.parameters.title).toEqual('Observables contained in the container');
    expect(widgetResult.dataSelection[0].columns.length).toEqual(3);
  });
