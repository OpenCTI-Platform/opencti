...
describe('EntitySetting resolver standard behavior', () => {
  ...
  it('should update entity setting by id', async () => {
    let queryResult = await queryAsAdmin({
      query: UPDATE_QUERY,
      variables: { id: entitySettingIdCoreRelationship, input: { key: 'platform_entity_files_ref', value: ['true'] } },
    });
    expect(queryResult.errors.length > 0).toBeTruthy();

    queryResult = await queryAsAdmin({
      query: UPDATE_QUERY,
      variables: { id: entitySettingIdCoreRelationship, input: { key: 'enforce_reference', value: ['true'] } },
    });
    expect(queryResult.data.entitySettingFieldPatch.enforce_reference).toBeTruthy();
  });
  it('should mass update entity settings by ids', async () => {
    let queryResult = await queryAsAdmin({
      query: MASS_UPDATE_QUERY,
