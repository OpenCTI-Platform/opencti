...
describe('CSV-PARSER with dynamic mapping (aka different entity on one file)', () => {
  ...
  it('Parse CSV - dynamic entity with IPs and URLs', async () => {
    ...
    const objects = await bundleProcessFromFile(testContext, ADMIN_USER, filePath, csvMapperDynamicIpAndUrl as CsvMapperParsed);
  ...
  it('Parse CSV - dynamic entity with MD5 and SHA-256 files', async () => {
    ...
    const objects = await bundleProcessFromFile(testContext, ADMIN_USER, filePath, csvMapperMockFileHashHack as CsvMapperParsed);
  ...
  it('Parse CSV - dynamic entity with URL and IP with special characters', async () => {
    ...
    const objects = await bundleProcessFromFile(testContext, ADMIN_USER, filePath, csvMapperDynamicChar as CsvMapperParsed);
