...
describe('CSV ingestion resolver standard behavior', () => {
  ...
  it('should count default groups be one from initialization', async () => {
    const defaultIngestionGroupCountResult = await queryAsUserWithSuccess(USER_DISINFORMATION_ANALYST, {
      query: gql`
  ...
  it('should add auto user and update CSV feeds ingester with it', async () => {
    ...
    // Verify no longer found
    const queryResult = await queryAsAdmin({ query: READ_USER_QUERY, variables: { id: updateCsvFeedWithAutoUserResult?.data?.ingestionCsvAddAutoUser?.user?.id } });
    expect(queryResult).not.toBeNull();
    expect(queryResult.data?.user).toBeNull();
  });
