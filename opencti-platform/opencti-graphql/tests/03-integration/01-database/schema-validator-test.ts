...
describe('Create and Update Validation', () => {
  ...
  it('should verify mandatory attributes at creation for standard users', async () => {
    ...
    const queryResult = await queryAsUser(USER_EDITOR, { query: CREATE_DATA_COMPONENT_QUERY, variables: { input: dataComponent } });
    ...
    expect((queryResult.errors?.[0].extensions?.data as any).field).toEqual('description');
  });
  ...
  it('should check mandatory attributes at update for standard users', async () => {
    ...
    const queryResultShouldSucceed = await queryAsUserWithSuccess(USER_EDITOR, {
    ...
    const queryResultShouldFail = await queryAsUser(USER_EDITOR, {
      query: UPDATE_DATA_COMPONENT_QUERY,
