...
describe('buildChanges standard behavior', async () => {
  ...
  it('should build changes for "participant" added ', async () => {
    ...
    expect(changes).toEqual([{
      ...
      changes_removed: [],
      changes_added: [{ raw: '9b854803-7158-4e4e-a492-f8845ac33aad', translated: 'User 1' }],
    }]);
  });
  it('should build changes for second "participant" added ', async () => {
    ...
    expect(changes).toEqual([{
      field: 'Report--objectParticipant',
      changes_removed: [],
      changes_added: [{ raw: '7c854803-7158-4e4e-a492-f8845ac33agp', translated: 'User 2' }],
    }]);
  });
  it('should build changes for "marking" added', async () => {
    ...
    expect(changes).toEqual([{
      ...
      changes_removed: [],
      changes_added: [{ raw: '6da54f1c-8c1b-4c61-953a-2ded39adcaba', translated: 'TLP:GREEN' }],
    }]);
  });
  ...
  it('should build changes for second "marking" removed', async () => {
    ...
    expect(changes).toEqual([{
      field: 'Report--objectMarking',
      changes_removed: [{ raw: '6da54f1c-8c1b-4c61-953a-2ded39adcaba', translated: 'TLP:GREEN' }],
      changes_added: [],
    }]);
  });
  ...
  it('should build changes for labels removed', async () => {
    ...
    expect(changes).toEqual([{
      ...
      changes_removed: [
        { raw: 'd9c27d81-c003-4a0d-bfdc-397b8d12f59c', translated: 'anti-sandbox' },
      ],
      changes_added: [],
    }]);
  });
