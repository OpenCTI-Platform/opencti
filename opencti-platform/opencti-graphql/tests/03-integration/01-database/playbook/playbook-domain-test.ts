...
describe('playbook-domain', () => {
  describe('playbookImport', () => {
    ...
    it('should throw error if playbook from a too old octi version', async () => {
      const fileUrl = new URL('testData/imported-playbook-with-old-version.json', import.meta.url);
      const fileHandle = open(fileUrl);
    ...
    it('should throw error if parsedData is not of playbook type', async () => {
      const fileUrl = new URL('testData/imported-playbook-with-wrong-type.json', import.meta.url);
      const fileHandle = open(fileUrl);
    ...
    it('should update correctly a playbook imported with the old configuration', async () => {
      const fileUrl = new URL('testData/imported-playbook-with-old-scope.json', import.meta.url);
      const fileHandle = open(fileUrl);
