...
import { buildSAMLOptions, computeSamlGroupAndOrg, computeSamlUserInfo } from '../../../../src/modules/authenticationProvider/provider-saml';
import { encryptAuthValue, SECRET_TYPE } from '../../../../src/modules/authenticationProvider/authenticationProvider-domain';
...
describe('SAML Single sign on Provider coverage tests', () => {
  describe('SAML configuration coverage', () => {
    it('should build correct options for SAML', async () => {
      ...
      const samlEntity: Partial<BasicStoreEntityAuthenticationProvider> = {
      ...
      const result = await buildSAMLOptions(samlEntity as BasicStoreEntityAuthenticationProvider);
