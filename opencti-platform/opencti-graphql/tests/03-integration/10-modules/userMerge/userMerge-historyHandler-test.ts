...
const SOURCE_EMAIL = 'usermerge-history-source@opencti.invalid';
const TARGET_EMAIL = 'usermerge-history-target@opencti.invalid';
...
describe('userMerge history handler', () => {
  beforeAll(async () => {
    ...
    const source = await addUser(testContext, ADMIN_USER, { name: 'usermerge-history-source', password: 'usermerge', user_email: SOURCE_EMAIL, prevent_default_groups: true });
    const target = await addUser(testContext, ADMIN_USER, { name: 'usermerge-history-target', password: 'usermerge', user_email: TARGET_EMAIL, prevent_default_groups: true });
    SOURCE_ID = source.id;
    TARGET_ID = target.id;
