...
const CUSTOM_VIEW_ENTITY_1: Partial<BasicStoreEntityCustomView> = {
  name: 'My first custom view',
  description: 'A custom view used for tests',
  path: 'first-custom-view-772ee59226414feb81f956194e090ab5',
  manifest: DASHBOARD_MANIFEST,
  target_entity_type: 'Intrusion-Set',
};

const CUSTOM_VIEW_ENTITY_2: Partial<BasicStoreEntityCustomView> = {
  name: 'My second custom view',
  description: 'Another custom view used for tests',
  path: 'second-custom-view-772ee59226414feb81f956194e090ab5',
  manifest: DASHBOARD_MANIFEST,
  target_entity_type: 'Case-Rft',
};

const CUSTOM_VIEW_ENTITY_INVALID: Partial<BasicStoreEntityCustomView> = {
  name: 'An invalid custom view',
  description: 'Just to test the filtering',
  path: 'invalid-custom-view-772ee59226414feb81f956194e090ab5',
  manifest: DASHBOARD_MANIFEST,
    ...
    it('should retrieve custom views display context', async () => {
      ...
      expect(result.data.customViewsDisplayContext).toContainEqual({
        ...
        custom_views_info: [{
          ...
          path: CUSTOM_VIEW_ENTITY_1.path,
        }],
      });
      expect(result.data.customViewsDisplayContext).toContainEqual({
        ...
        custom_views_info: [{
          ...
          path: CUSTOM_VIEW_ENTITY_2.path,
        }],
      });
    });
