...
export const inferenceLookup = async (inferences, fromStandardId, toStandardId, type) => {
  for (let index = 0; index < inferences.length; index += 1) {
    ...
    const from = await internalLoadById(context, SYSTEM_USER, inference.fromId);
    const to = await internalLoadById(context, SYSTEM_USER, inference.toId);
...
export const getInferences = (type) => {
  ...
  return listThings(context, SYSTEM_USER, [type], opts);
};

export const changeRule = async (ruleId, active) => {
  // Change the status
  await setRuleActivation(context, SYSTEM_USER, ruleId, active);
  // Wait for rule to finish activation
  let ruleActivated = false;
  while (ruleActivated !== true) {
    const tasks = await listEntities(context, SYSTEM_USER, [ENTITY_TYPE_TASK], { connectionFormat: false });
