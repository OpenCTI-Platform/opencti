import type { StoreEntity } from '../../../../../src/types/store';

export const REGION_INSTANCE = {
  id: '10000000-0000-4000-8000-000000000010',
  standard_id: 'location--20000000-0000-4000-8000-000000000010',
  entity_type: 'Region',
  created: '2026-03-01T00:00:00.000Z',
  modified: '2026-03-02T00:00:00.000Z',
  confidence: 80,
  revoked: false,
  name: 'Europe',
  description: 'European region',
  latitude: '50.8503',
  longitude: '4.3517',
  x_opencti_aliases: ['EU'],
  createdBy: { standard_id: 'identity--c801c762-92e8-58b6-9bcb-6fa805f902cb' },
  objectLabel: [{ value: 'stix 2.0' }],
  objectMarking: [{ standard_id: 'marking-definition--89484dde-e3d2-547f-a6c6-d14824429eb1' }],
} as unknown as StoreEntity;

export const EXPECTED_REGION = {
  id: 'location--20000000-0000-4000-8000-000000000010',
  type: 'location',
  spec_version: '2.0',
  x_opencti_id: '10000000-0000-4000-8000-000000000010',
  x_opencti_type: 'Region',
  x_opencti_granted_refs: [],
  x_opencti_files: [],
  created: '2026-03-01T00:00:00.000Z',
  modified: '2026-03-02T00:00:00.000Z',
  revoked: false,
  confidence: 80,
  labels: ['stix 2.0'],
  object_marking_refs: ['marking-definition--89484dde-e3d2-547f-a6c6-d14824429eb1'],
  created_by_ref: 'identity--c801c762-92e8-58b6-9bcb-6fa805f902cb',
  external_references: [],
  name: 'Europe',
  description: 'European region',
  latitude: 50.8503,
  longitude: 4.3517,
  region: 'Europe',
  x_opencti_location_type: 'Region',
  x_opencti_aliases: ['EU'],
};
