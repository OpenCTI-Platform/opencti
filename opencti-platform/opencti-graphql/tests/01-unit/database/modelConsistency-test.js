...
describe('Testing schema relations ref definition', () => {
  ...
  it('Relations Cyber Observable testing', () => {
    ...
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_DIRECTORY, ENTITY_DIRECTORY)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_DIRECTORY, ENTITY_DOMAIN_NAME)).toBe(false);
    ...
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_DIRECTORY, ENTITY_DOMAIN_NAME)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_DIRECTORY, ENTITY_HASHED_OBSERVABLE_STIX_FILE)).toBe(false);
    ...
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_EMAIL_MESSAGE, ENTITY_EMAIL_ADDR)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_EMAIL_MESSAGE, ENTITY_IPV4_ADDR)).toBe(false);
    ...
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_EMAIL_ADDR, ENTITY_USER_ACCOUNT)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_EMAIL_ADDR, ENTITY_IPV4_ADDR)).toBe(false);

    relationsRef = schemaRelationsRefDefinition.getRelationsRef(ENTITY_EMAIL_MESSAGE);
    [relationRef] = relationsRef.filter((rel) => rel.name === INPUT_BODY_MULTIPART);
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_EMAIL_MESSAGE, ENTITY_EMAIL_MIME_PART_TYPE)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_EMAIL_MESSAGE, ENTITY_IPV4_ADDR)).toBe(false);

    relationsRef = schemaRelationsRefDefinition.getRelationsRef(ENTITY_EMAIL_MESSAGE);
    [relationRef] = relationsRef.filter((rel) => rel.name === INPUT_RAW_EMAIL);
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_EMAIL_MESSAGE, ENTITY_HASHED_OBSERVABLE_ARTIFACT)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_EMAIL_MESSAGE, ENTITY_IPV4_ADDR)).toBe(false);
    ...
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_EMAIL_MIME_PART_TYPE, ENTITY_HASHED_OBSERVABLE_ARTIFACT)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_EMAIL_MIME_PART_TYPE, ENTITY_HASHED_OBSERVABLE_STIX_FILE)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_EMAIL_MIME_PART_TYPE, ENTITY_EMAIL_MESSAGE)).toBe(false);
    ...
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_HASHED_OBSERVABLE_ARTIFACT, ENTITY_HASHED_OBSERVABLE_ARTIFACT)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_HASHED_OBSERVABLE_ARTIFACT, ENTITY_IPV4_ADDR)).toBe(false);
    [relationRef] = relationsRef.filter((rel) => rel.name === INPUT_OPERATING_SYSTEM);
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_HASHED_OBSERVABLE_ARTIFACT, ENTITY_SOFTWARE)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_HASHED_OBSERVABLE_ARTIFACT, ENTITY_TYPE_MALWARE)).toBe(false);
    ...
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_HASHED_OBSERVABLE_ARTIFACT, ABSTRACT_STIX_CYBER_OBSERVABLE)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_HASHED_OBSERVABLE_ARTIFACT, ENTITY_DIRECTORY)).toBe(true);
    relationsRef = schemaRelationsRefDefinition.getRelationsRef(ENTITY_HASHED_OBSERVABLE_STIX_FILE);
    [relationRef] = relationsRef.filter((rel) => rel.name === INPUT_PARENT_DIRECTORY);
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_HASHED_OBSERVABLE_ARTIFACT, ENTITY_DIRECTORY)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_HASHED_OBSERVABLE_ARTIFACT, ABSTRACT_STIX_CYBER_OBSERVABLE)).toBe(false);
    [relationRef] = relationsRef.filter((rel) => rel.name === INPUT_CONTENT);
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_HASHED_OBSERVABLE_ARTIFACT, ENTITY_HASHED_OBSERVABLE_ARTIFACT)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_HASHED_OBSERVABLE_ARTIFACT, ENTITY_DIRECTORY)).toBe(false);

    relationsRef = schemaRelationsRefDefinition.getRelationsRef(ENTITY_IPV4_ADDR);
    [relationRef] = relationsRef.filter((rel) => rel.name === INPUT_RESOLVES_TO);
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_IPV4_ADDR, ENTITY_MAC_ADDR)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_IPV4_ADDR, ENTITY_AUTONOMOUS_SYSTEM)).toBe(false);
    [relationRef] = relationsRef.filter((rel) => rel.name === INPUT_BELONGS_TO);
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_IPV4_ADDR, ENTITY_AUTONOMOUS_SYSTEM)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_IPV4_ADDR, ENTITY_MAC_ADDR)).toBe(false);

    relationsRef = schemaRelationsRefDefinition.getRelationsRef(ENTITY_IPV6_ADDR);
    [relationRef] = relationsRef.filter((rel) => rel.name === INPUT_RESOLVES_TO);
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_IPV6_ADDR, ENTITY_MAC_ADDR)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_IPV6_ADDR, ENTITY_AUTONOMOUS_SYSTEM)).toBe(false);
    [relationRef] = relationsRef.filter((rel) => rel.name === INPUT_BELONGS_TO);
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_IPV6_ADDR, ENTITY_AUTONOMOUS_SYSTEM)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_IPV6_ADDR, ENTITY_MAC_ADDR)).toBe(false);
    ...
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_NETWORK_TRAFFIC, ENTITY_DOMAIN_NAME)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_NETWORK_TRAFFIC, ENTITY_NETWORK_TRAFFIC)).toBe(false);
    [relationRef] = relationsRef.filter((rel) => rel.name === INPUT_DST);
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_NETWORK_TRAFFIC, ENTITY_IPV4_ADDR)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_NETWORK_TRAFFIC, ENTITY_NETWORK_TRAFFIC)).toBe(false);
    [relationRef] = relationsRef.filter((rel) => rel.name === INPUT_SRC_PAYLOAD);
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_NETWORK_TRAFFIC, ENTITY_HASHED_OBSERVABLE_ARTIFACT)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_NETWORK_TRAFFIC, ENTITY_IPV4_ADDR)).toBe(false);
    ...
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_NETWORK_TRAFFIC, ENTITY_NETWORK_TRAFFIC)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_NETWORK_TRAFFIC, ENTITY_HASHED_OBSERVABLE_ARTIFACT)).toBe(false);
    ...
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_PROCESS, ENTITY_NETWORK_TRAFFIC)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_PROCESS, ENTITY_HASHED_OBSERVABLE_ARTIFACT)).toBe(false);
    [relationRef] = relationsRef.filter((rel) => rel.name === INPUT_CREATOR_USER);
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_PROCESS, ENTITY_USER_ACCOUNT)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_PROCESS, ENTITY_NETWORK_TRAFFIC)).toBe(false);
    [relationRef] = relationsRef.filter((rel) => rel.name === INPUT_IMAGE);
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_PROCESS, ENTITY_HASHED_OBSERVABLE_STIX_FILE)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_PROCESS, ENTITY_NETWORK_TRAFFIC)).toBe(false);
    [relationRef] = relationsRef.filter((rel) => rel.name === INPUT_PARENT);
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_PROCESS, ENTITY_PROCESS)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_PROCESS, ENTITY_NETWORK_TRAFFIC)).toBe(false);
    ...
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_WINDOWS_REGISTRY_KEY, ENTITY_WINDOWS_REGISTRY_VALUE_TYPE)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_WINDOWS_REGISTRY_KEY, ENTITY_NETWORK_TRAFFIC)).toBe(false);
    [relationRef] = relationsRef.filter((rel) => rel.name === INPUT_CREATOR_USER);
    expect(relationRef).not.toBeNull();
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_WINDOWS_REGISTRY_KEY, ENTITY_USER_ACCOUNT)).toBe(true);
    expect(relationRef.isRefExistingForTypes.bind(this, ENTITY_WINDOWS_REGISTRY_KEY, ENTITY_NETWORK_TRAFFIC)).toBe(false);
  });
});
