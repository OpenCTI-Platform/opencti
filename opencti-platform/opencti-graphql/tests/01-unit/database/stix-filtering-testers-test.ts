...
describe('Stix filter testers', () => {
  ...
  describe('by description (key=description)', () => {
    const stixWithoutCMSTAR = stixIndicators[0];
    const stixWithCMSTAR = stixIndicators[1];
    const stixObservableWithDescription = stixObservables[0];
    const stixSightingWithoutDescription = stixSightings[0];

    it('should test valid for a filter with the contains operator', () => {
      const filter: Filter = {
        key: ['description'],
        mode: 'or',
        operator: 'contains',
      ...
      expect(testers.testDescription(stixWithCMSTAR, filter)).toEqual(true);
      expect(testers.testDescription(stixWithoutCMSTAR, filter)).toEqual(false);
      expect(testers.testDescription(stixWithCMSTAR, filter2)).toEqual(false);
    });

    it('should test valid for a filter with the not_contains operator', () => {
      const filter: Filter = {
        key: ['description'],
        mode: 'or',
      ...
      expect(testers.testDescription(stixWithoutCMSTAR, filter)).toEqual(true);
    });

    it('should test valid for a filter with the search operator', () => {
      ...
      expect(testers.testDescription(stixWithCMSTAR, filter1)).toEqual(true);
      expect(testers.testDescription(stixWithoutCMSTAR, filter1)).toEqual(false);
      expect(testers.testDescription(stixWithCMSTAR, filter2)).toEqual(true);
    });

    it('should test valid for a filter with the nil operator', () => {
      const filter: Filter = {
        key: ['description'],
        mode: 'or',
        operator: 'nil',
