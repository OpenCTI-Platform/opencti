...
test('should observables correctly extracted', async () => {
  ...
  expect(multipleHashes[0].hashes.MD5).toEqual('cead3f77f6cda6ec00f57d76c9a6879f');
  expect(multipleHashes[1].type).toEqual(C.ENTITY_HASHED_OBSERVABLE_STIX_FILE);
  expect(multipleHashes[1].hashes['SHA-256']).toEqual('bf07a7fbb825fc0aae7bf4a1177b2b31fcf8a3feeaf7092761e18c859ee52a9c');
  ...
  expect(domainAndIp.length).toEqual(2);
  expect(domainAndIp[0].type).toEqual(C.ENTITY_IPV4_ADDR);
  expect(domainAndIp[0].value).toEqual('198.51.100.1');
  expect(domainAndIp[1].type).toEqual(C.ENTITY_DOMAIN_NAME);
  expect(domainAndIp[1].value).toEqual('5z8.info');
  ...
  expect(simpleEmailAddress[0].type).toEqual(C.ENTITY_EMAIL_ADDR);
  expect(simpleEmailAddress[0].value).toEqual('jdoe@example.com');
});
