...
import { getXtmOneEntitlement, isXtmOneEntitlementGranted, registerWithXtmOne } from '../../../../src/modules/xtm/one/xtm-one';
...
describe('registerWithXtmOne: the XTM One entitlement', () => {
  ...
  it('is granted by an XTM license sub-licensing every OpenCTI platform', async () => {
    ...
    expect(await isXtmOneEntitlementGranted(settings)).toBe(true);
  ...
  it('is never granted by ee_enabled alone, and an XTM One without the license proof must be upgraded', async () => {
    ...
    const entitlement = await getXtmOneEntitlement(settings);
  ...
  it('is never granted by ee_sources claiming the XTM sub-license without the license', async () => {
    ...
    expect(await isXtmOneEntitlementGranted(settings)).toBe(false);
    expect(warnings()).toEqual(['[XTM One] XTM One entitlement not granted: XTM One reports an XTM sub-license without the XTM license certificate proving it']);
  });
  ...
  it('ends when the license expires, even between two heartbeats', async () => {
    ...
    expect(await getXtmOneEntitlement(settings)).toMatchObject({ granted: false, reason: 'the trial XTM license has expired' });
  });
