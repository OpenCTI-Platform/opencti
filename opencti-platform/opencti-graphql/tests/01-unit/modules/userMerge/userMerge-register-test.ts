...
/**
 * The v2 baseline, transcribed from the register page. It is asserted here rather than
 ...
 */
const V2_DISTRIBUTION: Record<UserMergeDisposition, number> = {
  [UserMergeDisposition.Transfer]: 39,
...
describe('User merge register', () => {
  it('should be pinned to the v2 registry version', () => {
    expect(USER_MERGE_REGISTRY_VERSION).toBe('v2');
  });

  it('should match the v2 distribution, disposition by disposition', () => {
    Object.entries(V2_DISTRIBUTION).forEach(([disposition, expectedCount]) => {
  ...
  it('should hold exactly 100 rows, and no row outside the known dispositions', () => {
    const total = Object.values(V2_DISTRIBUTION).reduce((acc, count) => acc + count, 0);
