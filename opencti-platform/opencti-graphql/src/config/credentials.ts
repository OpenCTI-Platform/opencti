...
export const getRemoteCredentialsProviderFields = (prefix: string, provider: string): string[] => {
  return (
    conf.get(`${prefix}:credentials_provider:${provider}:field_targets`) ||
...
export const enrichWithRemoteCredentials = async (prefix: string, baseConfiguration: any) => {
  ...
  if (provider) {
    logApp.info('[OPENCTI] Remote credentials configuration detected', {
      provider,
      source: prefix,
    });
    if (provider === CYBERARK_PROVIDER) {
      ...
      const rejectUnauthorized =
        conf.get(`${prefix}:credentials_provider:https_cert:reject_unauthorized`) || false;
      if (conf.get(`${prefix}:credentials_provider:https_cert:crt`)) {
        certs = {
          ...
          ca: (conf.get(`${prefix}:credentials_provider:https_cert:ca`) ?? []).map((path: string) =>
            loadCert(path),
          ),
        };
      }
      ...
      try {
        ...
        if (result.status === 200 && isNotEmptyField(result.data.Content)) {
          const defaultSplitter =
            conf.get(`${prefix}:credentials_provider:${provider}:default_splitter`) ?? ':';
          ...
          logApp.info('[OPENCTI] Remote credentials successfully fetched', {
            provider,
            source: prefix,
          });
          return secretResult;
        ...
        logApp.error('[OPENCTI] Remote credentials data fail to fetch, fallback', {
          cause: e,
          provider,
          source: prefix,
        });
      ...
      logApp.error('[OPENCTI] Remote credentials provider is not supported, fallback', {
        provider,
        source: prefix,
      });
    }
