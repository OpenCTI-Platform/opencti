...
const confProviders = Object.entries(conf.get('providers'));

const processProviders = async () => {
  const context = executionContext('auth_providers');
  const platformSettings = await getEntityFromCache(context, SYSTEM_USER, ENTITY_TYPE_SETTINGS);

  const processedProviders = [];
  const providers = [...platformSettings.platform_providers.map((p) => ({ ...p, dynamic: true })), ...confProviders.map(([identifier, c]) => ({ identifier, ...c, dynamic: false }))];

  // for (let i = 0; i < confProviders.length; i += 1) {
  //   const confProfiver = confProviders[i][1];
  //   const settingProviderIndex = providers.findIndex(({ identifier }) => identifier === confProfiver.identifier);
  //   if (settingProviderIndex >= 0) {
  //     providers[settingProviderIndex] = {
  //       identifier: confProviders[i][0],
  //       ...confProfiver,
  //       ...providers[settingProviderIndex],
  //     };
  //   } else {
  ...
  for (let i = 0; i < providers.length; i += 1) {
    const { identifier, strategy, dynamic } = providers[i];
    const config = typeof providers[i].config === 'string' ? JSON.parse(providers[i].config) : providers[i].config;
    const mappedConfig = configRemapping(config);
    const providerName = config?.label || identifier;
    if (!config?.disabled) {
      try {
        // FORM Strategies
        if (strategy === STRATEGY_LOCAL) {
          const localStrategy = new LocalStrategy({}, (username, password, done) => {
            return login(username, password)
              .then((info) => {
                ...
                addUserLoginCount();return done(null, info);
              })
              .catch((err) => {
                done(err);
              });
          ...
          processedProviders.push({ identifier: providerName, type: AUTH_FORM, strategy, provider: 'local', dynamic, config: JSON.stringify(mappedConfig) });
        }
        if (strategy === STRATEGY_LDAP) {
          ...
          const ldapStrategy = new LdapStrategy(ldapOptions, (user, done) => {
            logApp.info('[LDAP] Successfully logged', { user });addUserLoginCount();
          ...
          processedProviders.push({ identifier: providerName, type: AUTH_FORM, strategy, provider: providerRef, dynamic, config: JSON.stringify(mappedConfig) });
        }
        // SSO Strategies
        if (strategy === STRATEGY_SAML) {
          ...
          const samlStrategy = new SamlStrategy(samlOptions, (profile, done) => {
            ...
            addUserLoginCount();const { nameID, nameIDFormat } = profile;
          ...
          processedProviders.push({ identifier: providerName, type: AUTH_SSO, strategy, provider: providerRef, dynamic, config: JSON.stringify(mappedConfig) });
        }
        if (strategy === STRATEGY_OPENID) {
          ...
          const openIdClient = config.use_proxy ? getPlatformHttpProxyAgent(mappedConfig.issuer) : undefined;
          OpenIDCustom.setHttpOptionsDefaults({ timeout: 0, agent: openIdClient });
          const clientConfig = await enrichWithRemoteCredentials(`providers:${identifier}`, config);
          const issuer = await OpenIDIssuer.discover(config.issuer);
          const { Client } = issuer;
        ...
        if (strategy === STRATEGY_FACEBOOK) {
          ...
          const facebookStrategy = new FacebookStrategy(
            ...
            (_, __, ___, profile, done) => {
              ...
              addUserLoginCount();const { email } = data;
        ...
        if (strategy === STRATEGY_GOOGLE) {
          ...
          const googleStrategy = new GoogleStrategy(googleOptions, (_, __, ___, profile, done) => {
            ...
            addUserLoginCount();const email = R.head(profile.emails).value;
        ...
        if (strategy === STRATEGY_GITHUB) {
          ...
          const githubStrategy = new GithubStrategy(githubOptions, async (_, token, __, profile, done) => {
            logApp.info('[GITHUB] Successfully logged', { profile });addUserLoginCount();
            let authorized = true;
        ...
        if (strategy === STRATEGY_AUTH0) {
          ...
          OpenIDCustom.setHttpOptionsDefaults({ timeout: 0, agent: openIdClient });
          const issuer = await OpenIDIssuer.discover(auth0Issuer);
          const { Client } = issuer;
          ...
          const auth0Strategy = new OpenIDStrategy(options, debugCallback, (_, tokenset, userinfo, done) => {
            ...
            addUserLoginCount();const { email, name } = userinfo;
            providerLoginHandler({ email, name }, done);
          });
        ...
        if (strategy === STRATEGY_HEADER) {
          ...
          const reqLoginHandler = async (req) => {
            ...
            addUserLoginCount();return new Promise((resolve) => {
          ...
          const headerProvider = {
            ...
            dynamic,
            config: JSON.stringify(mappedConfig),
          };
          processedProviders.push(headerProvider);
          HEADERS_AUTHENTICATORS.push(headerProvider);
        }
      } catch (e) {
        logApp.warn(`[SSO] Error while processing provider ${providerName}`, { error: e });
        processedProviders.push({ identifier: providerName, valid: false, strategy, provider: identifier, disabled: true, dynamic, config: JSON.stringify(mappedConfig) });
      }
    } else {
      processedProviders.push({ identifier: providerName, strategy, provider: identifier, disabled: true, dynamic, config: JSON.stringify(mappedConfig) });
    }
  }
  ...
  if (!hasLocal) {
    ...
    processedProviders.push({ identifier: INTERNAL_SECURITY_PROVIDER, type: AUTH_FORM, strategy: STRATEGY_LOCAL, provider: 'local', dynamic: false, config: '' });
  }
  return processedProviders.map((p) => ({ ...p, valid: p.valid ?? true, config: p.config || '' }));
};

export const getProviders = () => processProviders();
export const isStrategyActivated = async (strategy) => {
  (await getProviders()).map((p) => p.strategy).includes(strategy);
};
