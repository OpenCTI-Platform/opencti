...
export const updateAttributeMetaResolved = async <T extends StoreObject>(
  ...
  try {
    ...
    if (updatedInputs.length > 0) {
      ...
      const changes = await buildChanges(context, user, updatedInstance.entity_type, updatedInputs) as Change[];
...
export const createRelationRaw = async (
  ...
  try {
    ...
    if (isStixRefRelationship(relationshipType)) {
      ...
      // TODO fix, broken following 13248 PR?
      event = await storeUpdateEvent(context, user, previous, instance, message, [], storeUpdateEventsOpts);
