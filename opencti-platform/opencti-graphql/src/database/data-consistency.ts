...
export const isIndividualAssociatedToUser = async (
  context: AuthContext,
  individual: BasicStoreEntity,
) => {
...
export const verifyCanDeleteIndividual = async (
  context: AuthContext,
  user: AuthUser,
  individual: BasicStoreEntity,
  throwErrors = true,
...
export const verifyCanDeleteOrganization = async (
  context: AuthContext,
  user: AuthUser,
  ...
  if (organization.authorized_authorities && organization.authorized_authorities.length > 0) {
    ...
    if (throwErrors)
      throw FunctionalError('Cannot delete the organization.', { id: organization.id });
    return false;
  }
...
export const canDeleteElement = async (
  context: AuthContext,
  user: AuthUser,
  element: StoreObject,
) => {
  ...
  if (element.entity_type === ENTITY_TYPE_IDENTITY_ORGANIZATION) {
    return verifyCanDeleteOrganization(
      context,
      user,
      element as unknown as BasicStoreEntityOrganization,
      false,
    );
  }
  return true;
