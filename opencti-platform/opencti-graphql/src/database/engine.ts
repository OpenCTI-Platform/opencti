...
export const isClientAbortError = (err: any): boolean => {
  return (
    err instanceof AbortError || err?.name === 'AbortError' || err?.name === 'RequestAbortedError'
  );
};
    ...
    if (elements.length > workingIds.length) {
      ...
      if (elements.length >= ES_MAX_PAGINATION) {
        throw DatabaseError(
          ...
          { resultCount: elements.length, queryCount: workingIds.length, duplicatedIds },
        );
      }
    }
    ...
    if (operator === 'within') {
      ...
      if (isTermsQuery) {
        ...
        for (let i = 0; i < values.length; i += 1) {
          ...
          } else if (RANGE_OPERATORS.includes(operator)) {
            // range operators
            if (arrayKeys.length > 1) {
...
export const elPaginate = async <T extends BasicStoreBase>(
  ...
  try {
    ...
    throw wrapEngineError('Fail to execute engine pagination', err, {
      root_cause,
      query,
...
export const elUpdate = async (
  ...
  const updateOperation = async () => {
    ...
    try {
      ...
      throw wrapEngineError('Update indexing fail', err, {
        documentId,
        entityType,
        ...extendedErrors({ documentBody }),
      });
