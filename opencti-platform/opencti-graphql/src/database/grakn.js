...
export const inferIndexFromConceptTypes = types => {
  ...
  if (includes('Stix-Domain', types)) return INDEX_STIX_ENTITIES;
  if (includes('Tag', types)) return INDEX_STIX_ENTITIES;
  ...
  if (includes('stix_relation_embedded', types)) return INDEX_STIX_RELATIONS;
  if (includes('relation_embedded', types)) return INDEX_STIX_RELATIONS;
...
const modifyIdForInferredRelation = rel => {
  // 01. First fix the direction of the relation
  const isInv = isInversed(rel.relationship_type, rel.fromRole);
  if (isInv) {
    logger.error('[GRAKN] Inverse relation, something need to be fixed');
  }
  // 02. Then change the id if relation is inferred
  if (rel.inferred) {
    const { fromId, fromRole, toId, toRole } = rel;
    const type = rel.relationship_type;
    const pattern = `{ $rel(${fromRole}: $from, ${toRole}: $to) isa ${type}; $from id ${fromId}; $to id ${toId}; };`;
    return assoc('id', Buffer.from(pattern).toString('base64'), rel);
  }
  return rel;
};
...
const loadConcept = async (concept, { relationsMap, reIndex } = conceptOpts) => {
  ...
  return Promise.all(attributesPromises)
    .then(attributesData => {
      ...
      const innerType = head(types).toLowerCase();
      return pipe(
        assoc('id', transform.internal_id_key),
        assoc('inferred', false),
        assoc('grakn_id', concept.id),
        assoc('entity_type', attributesData.entity_type || innerType),
    ...
    .then(async entityData => {
      ...
      return Promise.all([isInferredPromise, rolesPromises])
        .then(([isInferred, roles]) => {
          return pipe(
        ...
        .then(rel => modifyIdForInferredRelation(rel));
    })
    .then(async completeData => {
      // If data is not indexed or inferred, just return it
      if (!resolveIndex || completeData.inferred) return completeData;
      return elIndex(resolveIndex, completeData);
    })
    .then(async indexedData => {
      // Eventually complete the indexed data
      // Will be used for listing ordering and filtering
      // if (!resolveIndex || indexedData.inferred) return indexedData;
      // const rels = await getRelationsValuesToIndex(parentType, indexedData.id);
      // if (isEmpty(rels)) return indexedData;
      // await elUpdate(resolveIndex, indexedData.grakn_id, rels);
      // return mergeLeft(indexedData, rels);
      return indexedData;
    });
};
...
export const loadRelationById = async (id, fromInternalId) => {
  const eid = escapeString(id);
  const fromId = escapeString(fromInternalId);
  const query = `match $from has internal_id_key "${fromId}";
   $rel($from, $to) isa relation; $rel has internal_id_key "${eid}"; get;`;
 ...
 * @param input
 */
export const createRelation = async (fromId, input) => {
  const relationId = uuid();
  await executeWrite(async wTx => {
    const query = `match $from has internal_id_key "${escapeString(fromId)}";
      $to has internal_id_key "${escapeString(input.toId)}"; 
      insert $rel(${escape(input.fromRole)}: $from, ${escape(input.toRole)}: $to) 
      isa ${input.through}, has internal_id_key "${relationId}" ${
      input.stix_id_key ? `, has relationship_type "${escapeString(input.through)}"` : ''
    }
        ${
          // eslint-disable-next-line no-nested-ternary
          input.stix_id_key
        ...
        ${input.first_seen ? `, has first_seen ${prepareDate(input.first_seen)}` : ''} 
  ...
  // Index the relation id inside the concept
  const previousEntity = await elLoadById(fromId);
  const innerField = `${input.through}${FIELD_INDEX_SUFFIX}`;
  const dataField = {
    [innerField]: append(input.toId, previousEntity[innerField] || [])
  };
  // eslint-disable-next-line no-underscore-dangle
  await elUpdate(previousEntity._index, previousEntity.grakn_id, dataField);
  // Return the data
  const node = await elLoadById(fromId);
  const relation = await loadRelationById(relationId, fromId);
...
export const updateAttribute = async (id, input, wTx) => {
  ...
  if (currentIndex) {
    await elUpdate(currentIndex, currentInstanceData.grakn_id, assoc(key, val, {}));
  }
  return id;
};
...
export const deleteRelationById = async (entityId, relationId) => {
  ...
  const previousRelation = await elLoadById(eid);
  const targetEntity = await elLoadByGraknId(previousRelation.toId);
  const previousEntity = await elLoadById(entityId);
  const innerField = `${previousRelation.relationship_type}${FIELD_INDEX_SUFFIX}`;
  const dataField = {
    [innerField]: filter(x => x === targetEntity.id, previousEntity[innerField] || [])
  };
  await executeWrite(async wTx => {
    const query = `match $x has internal_id_key "${eid}"; delete $x;`;
    logger.debug(`[GRAKN - infer: false] deleteRelationById > ${query}`);
    // Delete the indexed instance
    await elDeleteInstanceIds([relationId]);
    // Delete the inner indexed relations in entities
    // eslint-disable-next-line
    await elUpdate(previousEntity._index, previousEntity.grakn_id, dataField);
    await wTx.tx.query(query, { infer: false });
  });
  return elLoadById(entityId).then(data => ({
    node: data,
