...
const isRedisStreamFullyDeprecated = async (streamName = LIVE_STREAM_NAME, rebitStreamInfo?: StreamInfo) => {
  const rabbitStreamInfo = rebitStreamInfo ?? await rabbitStreamClient.rawFetchStreamInfo(streamName);
  const rabbitLastDate = utcDate(rabbitStreamInfo.lastEventDate);
  const oneMonthAgo = utcDate().subtract(1, 'month');
  return rabbitLastDate.isBefore(oneMonthAgo);
};
...
const rawCreateStreamProcessor = <T extends BaseEvent> (
  ...
  const proxyCallback = async (events: Array<SseEvent<T>>, lastEventId: string) => {
    ...
    if (!isRabbitStreamProcessorActive && lastEventId === redisMostRecentEvendId) {
      await redisStreamProcessor.shutdown();
      await rabbitStreamProcessor.start('live');
    }
  };
  ...
  return {
    ...
    running: () => (isRabbitStreamProcessorActive ? redisStreamProcessor.running() : rabbitStreamProcessor.running()),
    start: async (start = 'live') => {
      ...
      if (start === 'live') {
        ...
        const redisFirstOffsetTime = redisStreamInfo.firstEventId.split('-')[0];
        if (startOffsetTime < redisFirstOffsetTime) {
