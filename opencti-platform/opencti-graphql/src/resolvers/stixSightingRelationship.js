...
const stixSightingRelationshipResolvers = {
  Query: {
    ...
    stixSightingRelationships: (_, args, context) =>
      findStixSightingsPaginated(context, context.user, args),
    stixSightingRelationshipsTimeSeries: (_, args, context) =>
      timeSeriesRelations(context, context.user, {
        ...args,
        relationship_type: [STIX_SIGHTING_RELATIONSHIP],
      }),
    stixSightingRelationshipsDistribution: (_, args, context) =>
      distributionRelations(context, context.user, {
        ...args,
        relationship_type: [STIX_SIGHTING_RELATIONSHIP],
      }),
    stixSightingRelationshipsNumber: (_, args, context) =>
      stixSightingRelationshipsNumber(context, context.user, args),
  },
  StixSightingRelationship: {
    ...
    objectLabel: (rel, _, context) =>
      loadThroughDenormalized(context, context.user, rel, INPUT_LABELS, { sortBy: 'value' }),
    objectOrganization: (rel, _, context) =>
      loadThroughDenormalized(context, context.user, rel, INPUT_GRANTED_REFS, { sortBy: 'name' }),
    // endregion
    // region inner listing - cant be batch loaded
    externalReferences: (rel, args, context) =>
      externalReferencesPaginated(context, context.user, rel.id, args),
    ...
    status: (entity, _, context) =>
      entity.x_opencti_workflow_id
        ? findStatusById(context, context.user, entity.x_opencti_workflow_id)
        : null,
    ...
    // Figures
    containersNumber: (rel, args, context) =>
      numberOfContainersForObject(context, context.user, { ...args, objectId: rel.id }),
  },
  Mutation: {
    stixSightingRelationshipEdit: (_, { id }, context) => ({
      ...
      fieldPatch: ({ input, commitMessage, references }) =>
        stixSightingRelationshipEditField(context, context.user, id, input, {
          commitMessage,
          references,
        }),
      contextPatch: ({ input }) =>
        stixSightingRelationshipEditContext(context, context.user, id, input),
      contextClean: () => stixSightingRelationshipCleanContext(context, context.user, id),
      relationAdd: ({ input }) =>
        stixSightingRelationshipAddRelation(context, context.user, id, input),
      relationsAdd: ({ input, commitMessage, references }) =>
        stixSightingRelationshipAddRelations(context, context.user, id, input, {
          commitMessage,
          references,
        }),
    ...
    stixSightingRelationshipAdd: (_, { input }, context) =>
      addStixSightingRelationship(context, context.user, input),
  },
  Subscription: {
    stixSightingRelationship: {
