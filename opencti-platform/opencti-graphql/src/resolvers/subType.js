...
const subTypeResolvers = {
  ...
  SubType: {
    workflowEnabled: (current, _, context) =>
      isGlobalWorkflowEnabled(context, context.user, current.id),
    statuses: (current, _, context) => context.batch.globalStatusBatchLoader.load(current.id),
    statusesRequestAccess: (current, _, context) =>
      context.batch.requestAccessStatusBatchLoader.load(current.id),
  ...
  Mutation: {
    subTypeEdit: (_, { id }, context) => ({
      statusAdd: ({ input }) =>
        createStatus(context, context.user, id, input).then(() => findSubTypeById(id)),
      statusFieldPatch: ({ statusId, input }) =>
        statusEditField(context, context.user, id, statusId, input),
