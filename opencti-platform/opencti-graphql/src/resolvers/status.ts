...
const statusResolvers: Resolvers = {
  Query: {
    ...
    statusTemplatesByStatusScope: (_, args, context) =>
      findAllTemplatesByStatusScope(context, context.user, args),
  ...
  Status: {
    template: (current, _, context) =>
      current.template ?? findTemplateById(context, context.user, current.template_id),
  },
  StatusTemplate: {
    usages: (current, _, context) => statusTemplateUsagesNumber(context, context.user, current.id),
  },
  Mutation: {
    statusTemplateAdd: (_, { input }, context) =>
      createStatusTemplate(context, context.user, input),
    statusTemplateDelete: (_, { id }, context) => statusTemplateDelete(context, context.user, id),
    statusTemplateFieldPatch: (_, { id, input }, context) =>
      statusTemplateEditField(context, context.user, id, input),
    statusTemplateContextPatch: (_, { id, input }, context) =>
      statusTemplateEditContext(context, context.user, id, input),
    statusTemplateContextClean: (_, { id }, context) =>
      statusTemplateCleanContext(context, context.user, id),
  },
};
