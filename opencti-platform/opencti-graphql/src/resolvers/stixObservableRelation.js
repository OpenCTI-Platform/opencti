...
const stixObservableRelationResolvers = {
  ...
  Mutation: {
    stixObservableRelationEdit: (_, { id }, { user }) => ({
      ...
      fieldPatch: ({ input }) =>
        stixObservableRelationEditField(user, id, input),
      contextPatch: ({ input }) =>
        stixObservableRelationEditContext(user, id, input),
      contextClean: () => stixObservableRelationCleanContext(user, id),
      relationAdd: ({ input }) =>
        stixObservableRelationAddRelation(user, id, input),
      relationDelete: ({ relationId }) =>
        stixObservableRelationDeleteRelation(user, id, relationId)
    }),
    stixObservableRelationAdd: (_, { input }, { user }) =>
      addStixObservableRelation(user, input)
  },
  Subscription: {
    stixObservableRelation: {
