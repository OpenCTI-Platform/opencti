import {
  addRegion,
  childRegionsPaginated,
  countriesPaginated,
  findRegionPaginated,
  findById,
  parentRegionsPaginated,
} from '../domain/region';
import {
...
const regionResolvers = {
  ...
  Region: {
    parentRegions: (region, args, context) =>
      parentRegionsPaginated(context, context.user, region.id, args),
    subRegions: (region, args, context) =>
      childRegionsPaginated(context, context.user, region.id, args),
    countries: (region, args, context) =>
      countriesPaginated(context, context.user, region.id, args),
  },
  Mutation: {
    regionEdit: (_, { id }, context) => ({
      delete: () => stixDomainObjectDelete(context, context.user, id, ENTITY_TYPE_LOCATION_REGION),
