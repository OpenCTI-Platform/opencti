...
export const getExportFilter = async (
  user: AuthUser,
  { markingList, contentMaxMarkings, objectIdsList }: GetExportFilterType,
) => {
  const { max_shareable_marking } = user;

  const contentMaxMarkingsList = markingList.filter(({ id }) => contentMaxMarkings.includes(id));
  const maxShareableMarkings = markingList.filter(({ id }) =>
    max_shareable_marking.some((m) => m.id === id),
  );
  const notShareableMarkings = markingList
    .filter(
      ({ definition_type }) =>
        !max_shareable_marking.some((m) => m.definition_type === definition_type),
    )
    .map(({ id }) => id);
  ...
  maxShareableMarkings.forEach((m) => {
    ...
    const contentMaxMarking = contentMaxMarkingsList.find(
      (c) => c.definition_type === definition_type,
    );
    if (contentMaxMarking) {
      const { x_opencti_order: contentMaxMarkingOrder } = contentMaxMarking;
      effectiveContentMaxMarkings.push(
        contentMaxMarkingOrder <= x_opencti_order ? contentMaxMarking : m,
      );
    } else {
  ...
  const contentMarkings = effectiveContentMaxMarkings.length
    ? await getExportContentMarkings(markingList, effectiveContentMaxMarkings)
    : [];
    ...
    ? [
        {
          ...
          mode: 'and',
          operator: 'not_eq',
          values: [...contentMarkings, ...notShareableMarkings],
        },
      ]
    : [];
