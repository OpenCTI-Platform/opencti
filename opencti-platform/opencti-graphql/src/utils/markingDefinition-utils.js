...
export const cleanMarkings = async (context, values) => {
  ...
  return Object.entries(defaultGroupedMarkings)
    .map(([_, markingValues]) => {
...
export const handleMarkingOperations = async (context, currentMarkings, refs, operation) => {
  ...
  const markingsAdded = refs
    .filter((r) => markingsMap.has(r.internal_id))
    .map((r) => markingsMap.get(r.internal_id));
  ...
  const markingsInCommon = markings.filter((item) =>
    markingsAddedCleaned.some((m) => m.definition_type === item.definition_type),
  );
  if (operation === UPDATE_OPERATION_ADD) {
    ...
    if (
      markingsAddedCleaned.some((mark) =>
        markings.some(
          (mark2) =>
            mark2.definition_type === mark.definition_type &&
            mark2.x_opencti_order !== mark.x_opencti_order,
        ),
      )
    ) {
      const markingsToKeep = await cleanMarkings(context, [...markings, ...markingsAddedCleaned]);

      const markingsAddedHasHigherOrder = markingsToKeep.some((markingAdded) =>
        markings.some(
          (currentMarking) =>
            currentMarking.definition_type &&
            markingAdded.x_opencti_order &&
            markingAdded.x_opencti_order > currentMarking.x_opencti_order,
        ),
      );

      const markingsNotInCommon = markingsToKeep.filter(
        (item) => !markings.some((m) => m.definition_type === item.definition_type),
      );
