 ...
 */
export const testGenericFilter = ({ mode, operator }, adaptedFilterValues, stixCandidates) => {
 ...
 */
export const testStringFilter = (filter, stixCandidates) => {
    ...
    return testGenericFilter(filter, filterValuesLowerCase, stixValuesLowerCase);
};
/**
 ...
 */
export const testBooleanFilter = (filter, stixCandidate) => {
    const filterValuesAsBooleans = filter.values.map((v) => v.toLowerCase() === 'true' || v.toLowerCase() === 'yes' || v === '1');
    return testGenericFilter(filter, filterValuesAsBooleans, toValidArray(stixCandidate));
};
/**
 * Implementation of testGenericFilter for numerical values.
 * Filter values are parsed as floats.
 */
export const testNumericFilter = (filter, stixCandidate) => {
    const filterValuesAsNumbers = filter.values.map((v) => parseFloat(v)).filter((n) => !Number.isNaN(n));
    return testGenericFilter(filter, filterValuesAsNumbers, toValidArray(stixCandidate));
};
/**
 * Specific tester using the Moment.js library to compare dates.
 */
export const testDateFilter = ({ mode, operator, values }, stixCandidate) => {
 ...
 */
export const testFilterGroup = (data, filterGroup, testerByFilterKeyMap) => {
    if (!isFilterGroupNotEmpty(filterGroup))
        return true; // no filters -> stix always match
    if (filterGroup.mode === 'and') {
        ...
        if (filterGroup.filters.length > 0) {
            ...
            results.push(filterGroup.filters.every((filter) => { var _a; return (_a = testerByFilterKeyMap[filter.key[0]]) === null || _a === void 0 ? void 0 : _a.call(testerByFilterKeyMap, data, filter); }));
        }
    ...
    if (filterGroup.mode === 'or') {
        const results = [];
        if (filterGroup.filters.length > 0) {
            results.push(filterGroup.filters.some((filter) => { var _a; return (_a = testerByFilterKeyMap[filter.key[0]]) === null || _a === void 0 ? void 0 : _a.call(testerByFilterKeyMap, data, filter); }));
        }
        if (filterGroup.filterGroups.length > 0) {
