...
import {
  testMembersGroup,
  testMembersOrganization,
  testMembersUser,
} from '../filtering-activity-event/activity-event-testers';
import {
...
const validateFilterForStreamOriginMatch = (filter: Filter) => {
  if (!Array.isArray(filter.key) || filter.key.length !== 1) {
    throw UnsupportedError('Stream origin filtering can only be executed on a unique filter key', {
  ...
  if (STREAM_ORIGIN_TESTERS_MAP[filter.key[0]] === undefined) {
    ...
    throw UnsupportedError(
      'Stream origin filtering is not compatible with the provided filter key',
...
export const isOriginMatchFilterGroup = (
  eventData:
    | { origin?: { user_id?: string; group_ids?: string[]; organization_ids?: string[] } }
    | undefined,
