...
import { DOC_INSUFFICIENT_CONFIDENCE_LEVEL, FunctionalError } from '../config/errors';
...
export const controlUserConfidenceAgainstElement = <T extends ObjectWithConfidence>(user: AuthUser, existingElement: T, noThrow = false) => {
  ...
  if (!isConfidenceMatch) {
    ...
    const data = { user_id: user.id, element_id: existingElement.id, doc_code: DOC_INSUFFICIENT_CONFIDENCE_LEVEL };
