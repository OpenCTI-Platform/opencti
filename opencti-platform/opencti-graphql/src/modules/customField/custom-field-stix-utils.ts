...
import { testBooleanFilter, testDateFilter, testNumericFilter, testStringFilter, toValidArray } from '../../utils/filtering/boolean-logic-engine';
  ...
  return (stix: any, filter: Filter, changeContext?: { filterKey: string; eventContext: FilterEventContext }) => {
    ...
    switch (field_type) {
      ...
      case 'multi_select':
        ...
        // other types hold a single (possibly nullish) value that toValidArray safely wraps (or empties).
        return testStringFilter(filter, Array.isArray(customFieldStixValue) ? customFieldStixValue : toValidArray(customFieldStixValue), changeContext);
      case 'integer':
