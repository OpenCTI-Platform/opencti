...
export const validateCustomFieldValues = async (
  ...
  if (values.length > 0) {
    ...
    if (fieldNames.length !== uniqueFieldNames.size) {
      throw FunctionalError('Duplicate custom field entries found in customFieldValues', {
        duplicates: fieldNames.filter((n, i) => fieldNames.indexOf(n) !== i),
      });
    }
    ...
    for (const value of values) {
      ...
      if (!definition) {
        throw FunctionalError('Custom field definition not found for this entity type', {
  ...
  const mandatoryDefs = definitions.filter(
    (d) => d.entity_type_settings?.find((s) => s.entity_type === entityType)?.mandatory,
  );
  for (const def of mandatoryDefs) {
    const valueEntry = values.find((v) => v.field_name === def.name);
    if (!valueEntry) {
      throw FunctionalError('Mandatory custom field is missing', {
        field_name: def.name,
        label: def.label,
      });
    }
...
const validateSingleCustomFieldValue = (
  ...
  switch (field_type) {
    ...
    case 'multi_select':
      ...
      break;
    default:
      throw FunctionalError('Unknown custom field type', {
        field_type,
        field_name: value.field_name,
      });
  }
...
const validateIntegerField = (
  ...
  if (value.int_value === undefined || value.int_value === null) {
    throw FunctionalError('int_value is required for integer type custom field', {
      field_name: value.field_name,
    });
  }
  if (!Number.isInteger(value.int_value)) {
    throw FunctionalError('int_value must be an integer', {
      field_name: value.field_name,
      value: value.int_value,
    });
  }
  if (definition.min_value != null && value.int_value < definition.min_value) {
    throw FunctionalError('int_value is below minimum', {
      field_name: value.field_name,
      value: value.int_value,
  ...
  if (definition.max_value != null && value.int_value > definition.max_value) {
    throw FunctionalError('int_value is above maximum', {
      field_name: value.field_name,
      value: value.int_value,
...
const validateStringField = (
  value: CustomFieldValue,
  _definition: BasicStoreEntityCustomFieldDefinition,
): void => {
  if (value.string_value === undefined || value.string_value === null) {
    throw FunctionalError('string_value is required for string type custom field', {
      field_name: value.field_name,
    });
  }
...
const validateBooleanField = (value: CustomFieldValue): void => {
  if (value.boolean_value === undefined || value.boolean_value === null) {
    throw FunctionalError('boolean_value is required for boolean type custom field', {
      field_name: value.field_name,
    });
  }
...
const validateDateField = (value: CustomFieldValue): void => {
  if (value.date_value === undefined || value.date_value === null) {
    throw FunctionalError('date_value is required for date type custom field', {
      field_name: value.field_name,
    });
  }
  ...
  if (Number.isNaN(date.getTime())) {
    throw FunctionalError('date_value must be a valid ISO date string', {
      field_name: value.field_name,
...
const validateSelectField = (
  ...
  if (value.select_value === undefined || value.select_value === null) {
    throw FunctionalError('select_value is required for select type custom field', {
      field_name: value.field_name,
    });
  }
  if (!definition.select_options || definition.select_options.length === 0) {
    throw FunctionalError('No select_options configured for this custom field', {
      field_name: value.field_name,
    });
  }
...
const validateMultiSelectField = (
  ...
  if (value.select_values === undefined || value.select_values === null) {
    throw FunctionalError('select_values is required for multi_select type custom field', {
      field_name: value.field_name,
    });
  }
