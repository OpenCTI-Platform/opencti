...
export const findById: DomainFindById<BasicStoreEntityCustomFieldDefinition> = (
  context: AuthContext,
  user: AuthUser,
...
export const findCustomFieldDefinitionsPaginated = (
  context: AuthContext,
  user: AuthUser,
  opts: EntityOptions<BasicStoreEntityCustomFieldDefinition>,
) => {
  return pageEntitiesConnection<BasicStoreEntityCustomFieldDefinition>(
    context,
    user,
    [ENTITY_TYPE_CUSTOM_FIELD_DEFINITION],
    opts,
...
export const findCustomFieldDefinitionsForEntityType = (
  context: AuthContext,
  user: AuthUser,
  entityType: string,
) => {
...
export const customFieldDefinitionAdd = async (
  context: AuthContext,
  user: AuthUser,
  ...
  if (existingDefinitions.some((def) => def.name === input.name)) {
    throw ValidationError('A custom field with this technical name already exists', 'nameSuffix', {
      name: input.name,
    });
  }
  if (existingDefinitions.some((def) => def.label === input.label)) {
    throw ValidationError('A custom field with this label already exists', 'label', {
      label: input.label,
    });
  }
  // Validate field_type is supported
  const allowedTypes: CustomFieldType[] = [
    'integer',
    'string',
    'markdown',
    'boolean',
  ...
  if (!allowedTypes.includes(input.field_type as CustomFieldType)) {
    throw FunctionalError('Unsupported custom field type', {
  ...
  if (
    input.field_type === 'integer' &&
    input.min_value != null &&
    input.max_value != null &&
    input.min_value > input.max_value
  ) {
    throw FunctionalError('min_value cannot be greater than max_value', {
  ...
  if (
    ...
    (!input.select_options || input.select_options.length === 0)
  ) {
 ...
 */
const scheduleCustomFieldValuesCleanupTask = async (
  fieldName: string,
  fieldId: string,
  entityTypes: string[],
): Promise<void> => {
  ...
  const taskFilters = {
    ...
    filters: [
        ...
        : []),
      {
        ...
        values: [{ key: 'field_name', values: [fieldName], operator: FilterOperator.Eq }],
      },
    ],
    filterGroups: [],
  };
...
export const customFieldDefinitionDelete = async (
  context: AuthContext,
  user: AuthUser,
  customFieldDefinitionId: string,
) => {
  enforceEnableFeatureFlag(CUSTOM_FIELDS_FEATURE_FLAG);
  const element = await deleteElementById<StoreEntityCustomFieldDefinition>(
    context,
    user,
  ...
  await publishUserAction({
    ...
    context_data: {
      ...
      entity_type: ENTITY_TYPE_CUSTOM_FIELD_DEFINITION,
      input: element,
    },
  });
...
export const customFieldDefinitionEdit = async (
  context: AuthContext,
  user: AuthUser,
  customFieldDefinitionId: string,
  input: EditInput[],
  ...
  if (attemptedForbidden.length > 0) {
    throw FunctionalError('Cannot modify immutable fields on a custom field definition', {
      keys: attemptedForbidden.map((i) => i.key),
    });
  }
  ...
  if (optionsEdit) {
    ...
    if (
      definition &&
      (definition.field_type === 'select' || definition.field_type === 'multi_select')
    ) {
      const previousOptions = definition.select_options ?? [];
      const editValues = (
        Array.isArray(optionsEdit.value) ? optionsEdit.value : [optionsEdit.value]
      ) as string[];
  ...
  if (minEdit || maxEdit) {
    ...
    if (definition && definition.field_type === 'integer') {
      ...
      const buildOutOfBoundFilter = (
        operator: FilterOperator,
        bound: number,
      ): FilterGroupWithNested => ({
        mode: FilterMode.And,
  ...
  const { element: updatedElem } = await updateAttribute<StoreEntityCustomFieldDefinition>(
    context,
    user,
    customFieldDefinitionId,
    ENTITY_TYPE_CUSTOM_FIELD_DEFINITION,
...
export const customFieldDefinitionUpdateEntityType = async (
  ...
  if (!currentTypes.includes(entityType)) {
    throw FunctionalError('Custom field definition is not attached to this entity type', {
      id: customFieldDefinitionId,
      entityType,
    });
  }
...
export const customFieldDefinitionRemoveEntityType = async (
  ...
  const nextSettings = (definition.entity_type_settings ?? []).filter(
