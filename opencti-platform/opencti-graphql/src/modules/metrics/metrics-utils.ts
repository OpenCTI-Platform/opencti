...
export const loadEntityMetricsConfiguration = () => {
  ...
  if (metricsConfigurationEnv) {
    for (const metricKey in metricsConfigurationEnv) {
      ...
      const metricListForCurrentEntity: MetricDescription[] = metricsConfigurationEnv[
        metricKey
      ] as unknown as MetricDescription[];
      if (metricListForCurrentEntity) {
        if (metricListForCurrentEntity.length > 1) {
          throw UnsupportedError('Several metric per entity is not supported yet', {
            metricsEntity: metricKey,
          });
        }
        if (metricListForCurrentEntity.length > 0) {
          const metricAttributes: MetricDescription =
            metricListForCurrentEntity[0] as unknown as MetricDescription;
          metricsConfiguration.push({
            entity_type: `${metricKeyNoCase}`,
