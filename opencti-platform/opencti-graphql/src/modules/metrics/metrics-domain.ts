...
export const patchMetric = async (
  context: AuthContext,
  user: AuthUser,
  entityId: string,
  ...
  const entityConfiguration = allKnownAttributes.find(
    (metricDefinition) =>
      metricDefinition.entity_type.toLowerCase() === entity.entity_type.toLowerCase(),
  );
  if (!entityConfiguration) {
    throw UnknownError('The metric entity is not allowed', {
      name: input.name,
      entityId,
  ...
  if (
    !entityConfiguration.metrics.some(
      (metricDescription) => metricDescription.attribute === input.name,
    )
  ) {
  ...
  const metrics =
    entity.metrics?.filter((existingMetric) => existingMetric.name !== input.name) || [];
