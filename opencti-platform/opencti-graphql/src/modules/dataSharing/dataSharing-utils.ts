...
const resolveValidUser = async (context: AuthContext, userId: string): Promise<AuthUser> => {
  ...
  if (!user) {
    throw FunctionalError('The user configured for this public sharing no longer exists', {
      userId,
    });
  }
  return user;
 ...
 */
export const resolvePublicUser = async (
  context: AuthContext,
  userId: string | null | undefined,
): Promise<AuthUser> => {
  if (!userId) {
    logApp.warn(
      '[DATA_SHARING] No public user configured for this public sharing, falling back to SYSTEM_USER (deprecated - please configure a dedicated user)',
    );
    return SYSTEM_USER;
  }
...
const PUBLIC_SHARING_CONFIGS: SharingConfig[] = [
  {
    entityType: ENTITY_TYPE_FEED,
    publicField: 'feed_public',
    userIdField: 'feed_public_user_id',
    entityLabel: 'csv feed',
  },
  {
    entityType: ENTITY_TYPE_TAXII_COLLECTION,
    publicField: 'taxii_public',
    userIdField: 'taxii_public_user_id',
    entityLabel: 'Taxii collection',
  },
  {
    entityType: ENTITY_TYPE_STREAM_COLLECTION,
    publicField: 'stream_public',
    userIdField: 'stream_public_user_id',
    liveField: 'stream_live',
    entityLabel: 'live stream',
  },
];
 ...
 */
export const disablePublicSharingForDeletedUser = async (
  context: AuthContext,
  userId: string,
): Promise<void> => {
  await Promise.all(
    PUBLIC_SHARING_CONFIGS.map(
      async ({ entityType, publicField, userIdField, liveField, entityLabel }) => {
        const filters = {
          mode: FilterMode.And,
          filterGroups: [],
          filters: [{ key: [userIdField], values: [userId] }],
        };
        const entities = await fullEntitiesList<BasicStoreEntity>(
          context,
          SYSTEM_USER,
          [entityType],
          { filters },
        ...
        await Promise.all(
          entities.map(async (entity) => {
            ...
            if (liveField) {
              const { element: liveElement } = await patchAttribute(
                context,
                SYSTEM_USER,
                entity.id,
                entityType,
              ...
              await publishUserAction({
                ...
                context_data: {
                  ...
                  input: [{ key: liveField, value: ['false'] }],
                },
              });
              await notify(
                BUS_TOPICS[ENTITY_TYPE_STREAM_COLLECTION].EDIT_TOPIC,
                liveElement,
                SYSTEM_USER,
              );
            }
            ...
            const publicPatch: Record<string, string | boolean | null> = {
              [publicField]: false,
              [userIdField]: null,
            };
            ...
            await publishUserAction({
              ...
              context_data: {
                id: entity.id,
                entity_type: entityType,
                input: [{ key: publicField, value: ['false'] }],
              },
            });
            const busTopic = (BUS_TOPICS as Record<string, { EDIT_TOPIC?: string } | undefined>)[
              entityType
            ];
  ...
  logApp.info('[DATA_SHARING] Disabled public sharing for all entities referencing deleted user', {
    userId,
  });
};
