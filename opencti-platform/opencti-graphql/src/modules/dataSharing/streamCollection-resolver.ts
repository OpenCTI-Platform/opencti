...
const streamCollectionResolvers: Resolvers = {
  Query: {
    ...
    streamCollections: (_, args, context) =>
      findStreamCollectionPaginated(context, context.user, args),
  ...
  StreamCollection: {
    ...
    stream_public_user: (stream, _, context) =>
      stream.stream_public_user_id
        ? loadCreator(context, context.user, stream.stream_public_user_id)
        : null,
  },
  Mutation: {
    streamCollectionAdd: (_, { input }, context) =>
      createStreamCollection(context, context.user, input),
    streamCollectionEdit: (_, { id }, context) =>
      ({
        delete: () => streamCollectionDelete(context, context.user, id),
        fieldPatch: ({ input }: { input: EditInput[] }) =>
          streamCollectionEditField(context, context.user, id, input),
        contextPatch: ({ input }: { input: EditContext }) =>
          streamCollectionEditContext(context, context.user, id, input),
