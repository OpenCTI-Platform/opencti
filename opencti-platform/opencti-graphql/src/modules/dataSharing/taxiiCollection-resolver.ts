...
const taxiiCollectionResolvers: Resolvers = {
  Query: {
    ...
    taxiiCollections: (_, args, context) =>
  ...
  TaxiiCollection: {
    ...
    taxii_public_user: (taxii, _, context) =>
      taxii.taxii_public_user_id
        ? loadCreator(context, context.user, taxii.taxii_public_user_id)
        : null,
  },
  Mutation: {
    taxiiCollectionAdd: (_, { input }, context) =>
      createTaxiiCollection(context, context.user, input),
    taxiiCollectionEdit: (_, { id }, context) =>
      ({
        delete: () => taxiiCollectionDelete(context, context.user, id),
        fieldPatch: ({ input }: { input: EditInput[] }) =>
          taxiiCollectionEditField(context, context.user, id, input),
        contextPatch: ({ input }: { input: EditContext }) =>
          taxiiCollectionEditContext(context, context.user, id, input),
