...
import {
  isNotEmptyField,
  READ_STIX_DATA_WITH_INFERRED,
  READ_STIX_INDICES,
} from '../../database/utils';
import {
  ENTITY_TYPE_TAXII_COLLECTION,
  type BasicStoreEntityTaxiiCollection,
  type StoreEntityTaxiiCollection,
} from './taxiiCollection-types';
import {
  createEntity,
  deleteElementById,
  stixLoadByIds,
...
import type {
  EditContext,
  EditInput,
  QueryTaxiiCollectionsArgs,
  TaxiiCollectionAddInput,
...
export const createTaxiiCollection = async (
  context: AuthContext,
  user: AuthUser,
  ...
  if (input.taxii_public && !isUserHasCapability(user, SETTINGS_SET_ACCESSES)) {
    throw FunctionalError(
      'You must have the SETTINGS_SETACCESSES capability to create a public Taxii collection',
    );
  }
...
export const findById = (context: AuthContext, user: AuthUser, collectionId: string) => {
  return storeLoadById<BasicStoreEntityTaxiiCollection>(context, user, collectionId, [
    ENTITY_TYPE_TAXII_COLLECTION,
    ENTITY_TYPE_INGESTION_TAXII_COLLECTION,
  ]);
};
export const findTaxiiCollectionPaginated = (
  context: AuthContext,
  user: AuthUser,
  args: QueryTaxiiCollectionsArgs,
) => {
  if (user && isUserHasCapability(user, TAXIIAPI)) {
    const options = { ...args, includeAuthorities: true };
    return pageEntitiesConnection<BasicStoreEntityTaxiiCollection>(
      context,
      user,
      [ENTITY_TYPE_TAXII_COLLECTION],
      options,
...
export const taxiiCollectionEditField = async (
  context: AuthContext,
  user: AuthUser,
  collectionId: string,
  input: EditInput[],
  ...
  if (
    input.some((item) => publicFields.includes(item.key)) &&
    !isUserHasCapability(user, SETTINGS_SET_ACCESSES)
  ) {
    throw FunctionalError(
      'You must have the SETTINGS_SETACCESSES capability to modify public Taxii collection settings',
    );
  }
  ...
  const settingPublicTrue = input.find(
    (item) => item.key === 'taxii_public' && item.value?.[0] === 'true',
  );
  if (settingPublicTrue) {
    const existingCollection = await findById(context, user, collectionId);
    const effectiveUserId =
      publicUserIdItem?.value?.[0] ?? existingCollection?.taxii_public_user_id;
    if (!effectiveUserId) {
      throw FunctionalError('A user must be configured when the Taxii collection is public');
    }
  }
  ...
  const { element } = await updateAttribute<StoreEntityTaxiiCollection>(
    context,
    user,
    collectionId,
    ENTITY_TYPE_TAXII_COLLECTION,
...
export const taxiiCollectionDelete = async (
  context: AuthContext,
  user: AuthUser,
  collectionId: string,
) => {
  const deleted = await deleteElementById<StoreEntityTaxiiCollection>(
    context,
    user,
    collectionId,
    ENTITY_TYPE_TAXII_COLLECTION,
...
export const taxiiCollectionCleanContext = async (
  context: AuthContext,
  user: AuthUser,
  collectionId: string,
) => {
  await delEditContext(user, collectionId);
  return storeLoadById(context, user, collectionId, ENTITY_TYPE_TAXII_COLLECTION).then(
    (collectionToReturn) => {
      return notify(BUS_TOPICS[ENTITY_TYPE_TAXII_COLLECTION].EDIT_TOPIC, collectionToReturn, user);
    },
  );
};
export const taxiiCollectionEditContext = async (
  context: AuthContext,
  user: AuthUser,
  collectionId: string,
  input: EditContext,
...
export const collectionQuery = async (
  context: AuthContext,
  user: AuthUser,
  collection: BasicStoreEntity & Record<string, any>,
  ...
  const currentIndex = collection.include_inferences
    ? READ_STIX_DATA_WITH_INFERRED
    : READ_STIX_INDICES;
  return elPaginate(context, user, currentIndex, options) as Promise<
    BasicConnection<BasicStoreBase>
  >;
};
export const restCollectionStix = async (
  context: AuthContext,
  user: AuthUser,
  collection: BasicStoreEntity & Record<string, any>,
  args: Record<string, any>,
...
export const restCollectionManifest = async (
  context: AuthContext,
  user: AuthUser,
  collection: BasicStoreEntity & Record<string, any>,
  args: Record<string, any>,
...
export const restAllCollections = async (context: AuthContext, user: AuthUser) => {
  const collections = await fullEntitiesList(context, user, [
    ENTITY_TYPE_TAXII_COLLECTION,
    ENTITY_TYPE_INGESTION_TAXII_COLLECTION,
  ]);
  return collections
    .filter(
      (c: BasicStoreEntity & Record<string, any>) =>
        !(
          c.entity_type === ENTITY_TYPE_INGESTION_TAXII_COLLECTION && c.ingestion_running === false
        ),
    )
