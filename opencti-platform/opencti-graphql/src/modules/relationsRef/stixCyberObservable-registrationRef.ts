...
schemaRelationsRefDefinition.registerRelationsRef(ABSTRACT_STIX_CYBER_OBSERVABLE, [
  createdBy,
  objectMarking,
  objectLabel,
  externalReferences,
...
schemaRelationsRefDefinition.registerRelationsRef(ENTITY_DIRECTORY, [
  buildRelationRef(contains, (_: string, toType: string) =>
    [ENTITY_DIRECTORY, ENTITY_HASHED_OBSERVABLE_STIX_FILE].includes(toType),
  ),
]);
schemaRelationsRefDefinition.registerRelationsRef(ENTITY_DOMAIN_NAME, [
  buildRelationRef(resolvesTo, (_: string, toType: string) =>
    [ENTITY_DOMAIN_NAME, ENTITY_IPV4_ADDR, ENTITY_IPV6_ADDR].includes(toType),
  ),
]);
schemaRelationsRefDefinition.registerRelationsRef(ENTITY_EMAIL_MESSAGE, [
  ...
  buildRelationRef(
    bodyMultipart,
    (_: string, toType: string) => ENTITY_EMAIL_MIME_PART_TYPE === toType,
  ),
  buildRelationRef(
    rawEmail,
    (_: string, toType: string) => ENTITY_HASHED_OBSERVABLE_ARTIFACT === toType,
  ),
  buildRelationRef(contains, (_: string, toType: string) =>
    [
      ENTITY_HASHED_OBSERVABLE_STIX_FILE,
...
schemaRelationsRefDefinition.registerRelationsRef(ENTITY_EMAIL_MIME_PART_TYPE, [
  buildRelationRef(bodyRaw, (_: string, toType: string) =>
    [ENTITY_HASHED_OBSERVABLE_ARTIFACT, ENTITY_HASHED_OBSERVABLE_STIX_FILE].includes(toType),
  ),
]);
schemaRelationsRefDefinition.registerRelationsRef(ENTITY_HASHED_OBSERVABLE_STIX_FILE, [
  buildRelationRef(contains, (_: string, toType: string) =>
    getParentTypes(toType).includes(ABSTRACT_STIX_CYBER_OBSERVABLE),
  ),
...
schemaRelationsRefDefinition.registerRelationsRef(ENTITY_NETWORK_TRAFFIC, [
  buildRelationRef(src, (_: string, toType: string) =>
    [ENTITY_DOMAIN_NAME, ENTITY_IPV4_ADDR, ENTITY_IPV6_ADDR, ENTITY_MAC_ADDR].includes(toType),
  ),
  buildRelationRef(dst, (_: string, toType: string) =>
    [ENTITY_DOMAIN_NAME, ENTITY_IPV4_ADDR, ENTITY_IPV6_ADDR, ENTITY_MAC_ADDR].includes(toType),
  ),
  buildRelationRef(
    srcPayload,
  ...
  buildRelationRef(
    encapsulatedBy,
    (_: string, toType: string) => ENTITY_NETWORK_TRAFFIC === toType,
  ),
]);
schemaRelationsRefDefinition.registerRelationsRef(ENTITY_PROCESS, [
  ...
  buildRelationRef(
    image,
    (_: string, toType: string) => ENTITY_HASHED_OBSERVABLE_STIX_FILE === toType,
  ),
...
schemaRelationsRefDefinition.registerRelationsRef(ENTITY_WINDOWS_REGISTRY_KEY, [
  buildRelationRef(
    values,
    (_: string, toType: string) => ENTITY_WINDOWS_REGISTRY_VALUE_TYPE === toType,
  ),
  buildRelationRef(creatorUser, (_: string, toType: string) => ENTITY_USER_ACCOUNT === toType),
]);
