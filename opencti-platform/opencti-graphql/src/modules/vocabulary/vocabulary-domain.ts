...
export const findById: DomainFindById<BasicStoreEntityVocabulary> = (
  context: AuthContext,
  user: AuthUser,
  id: string,
) => {
...
export const findVocabularyPaginated = (
  context: AuthContext,
  user: AuthUser,
  opts: QueryVocabulariesArgs,
) => {
  const { category } = opts;
  let { filters } = opts;
  ...
  if (category) {
    ...
    const categories = entityTypes.values.flatMap((type) =>
    ...
    const filterGroup = filters
      ...
      : undefined;
    filters = addFilter(filterGroup, 'category', categories, entityTypes.operator ?? undefined);
  }
  const args = {
    orderBy: ['order', 'name'], // Default orderBy if none
    ...opts,
  };
  return pageEntitiesConnection<BasicStoreEntityVocabulary>(
    context,
    user,
    [ENTITY_TYPE_VOCABULARY],
    {
...
export const getVocabularyUsages = async (
  context: AuthContext,
  user: AuthUser,
  vocabulary: BasicStoreEntityVocabulary,
) => {
  const categoryDefinition = getVocabulariesCategories().find(
    ({ key }) => key === vocabulary.category,
  );
...
export const addVocabulary = async (
  context: AuthContext,
  user: AuthUser,
  vocabulary: VocabularyAddInput,
) => {
  const element = await createEntity(
    context,
    user,
    { ...vocabulary, order: vocabulary.order ?? 0 },
    ENTITY_TYPE_VOCABULARY,
  );
...
export const deleteVocabulary = async (
  ...
  const deletable =
    !vocabulary.builtIn &&
    (!completeCategory ||
      !completeCategory.fields.some(({ required }) => required) ||
      usages === 0);
  if (deletable) {
    if (completeCategory) {
      await elRawUpdateByQuery({
        ...
        body: {
          script: {
            source:
              'for(field in params.category.fields) if(ctx._source[field.key] instanceof List) ctx._source[field.key].remove(ctx._source[field.key].indexOf(params.oldName)); else ctx._source[field.key] = null;',
            lang: 'painless',
...
export const editVocabulary = async (
  ...
  const { element } = await updateAttribute<StoreEntityVocabulary>(
    context,
    user,
    id,
    ENTITY_TYPE_VOCABULARY,
