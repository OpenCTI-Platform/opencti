...
} from './task-template/task-template-types';

export const findById: DomainFindById<BasicStoreEntityTask> = (
  context: AuthContext,
  user: AuthUser,
  templateId: string,
) => {
...
export const findTaskPaginated = (
  context: AuthContext,
  user: AuthUser,
  opts: EntityOptions<BasicStoreEntityTask>,
) => {
  return pageEntitiesConnection<BasicStoreEntityTask>(
    context,
    user,
...
export const findAllByCaseTemplateId = async (
  context: AuthContext,
  user: AuthUser,
  ...
  return fullEntitiesList<BasicStoreEntityTaskTemplate>(
    context,
    user,
    [ENTITY_TYPE_TASK_TEMPLATE],
    opts,
...
export const caseTasksPaginated = async <T extends BasicStoreEntity>(
  context: AuthContext,
  user: AuthUser,
  caseId: string,
...
export const taskParticipantsPaginated = async (
  context: AuthContext,
  user: AuthUser,
  caseId: string,
...
export const taskContainsStixObjectOrStixRelationship = async (
  context: AuthContext,
  user: AuthUser,
  taskId: string,
  thingId: string,
  ...
  const resolvedThingId = isStixId(thingId)
    ? (await internalLoadById(context, user, thingId)).internal_id
    : thingId;
  const args = {
    first: 1,
    filters: {
...
export const taskAddRelation = async (
  context: AuthContext,
  user: AuthUser,
  taskId: string,
...
export const taskDeleteRelation = async (
  context: AuthContext,
  user: AuthUser,
  taskId: string,
  toId: string,
