...
export const addTrigger = async (
  ...
  if (
    type === TriggerTypeValue.Live &&
    (triggerInput as TriggerLiveAddInput).event_types.length === 0
  ) {
  ...
  if (recipient.entity_type === ENTITY_TYPE_USER) {
    ...
    recipient.entity_type === ENTITY_TYPE_GROUP ||
    recipient.entity_type === ENTITY_TYPE_IDENTITY_ORGANIZATION
  ) {
  ...
  const defaultOpts = {
    ...
    instance_trigger:
      type === TriggerTypeValue.Digest
        ? false
        : (triggerInput as TriggerLiveAddInput).instance_trigger,
...
export const addTriggerActivity = async (
  ...
  const members = (await internalFindByIds<BasicStoreEntity>(
    context,
    SYSTEM_USER,
    triggerInput.recipients,
  )) as BasicStoreEntity[];
  if (members.length === 0) {
...
export const getTriggerRecipients = async (
  context: AuthContext,
  user: AuthUser,
  element: BasicStoreEntityTrigger,
) => {
...
export const triggerEdit = async (
  context: AuthContext,
  user: AuthUser,
  triggerId: string,
  input: InternalEditInput[],
) => {
  ...
  if (
    ...
    ![MEMBER_ACCESS_RIGHT_EDIT, MEMBER_ACCESS_RIGHT_ADMIN].includes(userAccessRight)
  ) {
    throw ForbiddenAccess();
  }
  if (trigger.trigger_type === TriggerTypeValue.Live) {
    const emptyTriggerEvents = input.filter(
      (editEntry) => editEntry.key === 'event_types' && editEntry.value.length === 0,
    );
...
export const triggerActivityEdit = async (
  context: AuthContext,
  user: AuthUser,
  triggerId: string,
  input: EditInput[],
  ...
  for (let index = 0; index < input.length; index += 1) {
    ...
    if (inputElement.key === 'recipients') {
      const value = (inputElement.value ?? []).map((r) => ({
        id: r,
        access_right: MEMBER_ACCESS_RIGHT_VIEW,
      }));
...
export const triggerDelete = async (context: AuthContext, user: AuthUser, triggerId: string) => {
  ...
  const deleted = await deleteElementById<StoreEntityTrigger>(
    context,
    user,
    triggerId,
    ENTITY_TYPE_TRIGGER,
  ...
  const memberIds = (trigger.restricted_members ?? []).map((a: AuthorizedMember) => a.id);
  const recipients = (await internalFindByIds<BasicStoreEntity>(
    context,
    SYSTEM_USER,
    memberIds,
  )) as BasicStoreEntity[];
...
export const triggersKnowledgeFind = (
  context: AuthContext,
  user: AuthUser,
  ...
  return pageEntitiesConnection<BasicStoreEntityTrigger>(
    context,
    user,
    [ENTITY_TYPE_TRIGGER],
    queryArgs,
...
export const triggersKnowledgeCount = async (
  context: AuthContext,
  opts: QueryTriggersKnowledgeArgs,
) => {
  const finalFilter = addFilter(opts.filters, 'trigger_scope', 'knowledge');
...
export const triggersActivityFind = (
  context: AuthContext,
  user: AuthUser,
  opts: QueryTriggersActivityArgs,
) => {
...
export const notificationGet = (
  ...
  return storeLoadById(
    ...
    narrativeId,
    ENTITY_TYPE_NOTIFICATION,
  ) as unknown as BasicStoreEntityNotification;
};
export const notificationsFind = (
  context: AuthContext,
  user: AuthUser,
  opts: QueryNotificationsArgs,
) => {
  const queryArgs = { ...opts, includeAuthorities: true };
  return pageEntitiesConnection<BasicStoreEntityNotification>(
    context,
    user,
    [ENTITY_TYPE_NOTIFICATION],
    queryArgs,
...
export const myNotificationsFind = (
  context: AuthContext,
  user: AuthUser,
  opts: QueryNotificationsArgs,
) => {
...
export const myUnreadNotificationsCount = async (
  context: AuthContext,
  user: AuthUser,
  userId = null,
) => {
...
export const notificationDelete = async (
  ...
  await notify(
    BUS_TOPICS[NOTIFICATION_NUMBER].EDIT_TOPIC,
    { count: unreadNotificationsCount, user_id: notification.user_id },
    user,
  );
  return notificationId;
};
export const notificationEditRead = async (
  context: AuthContext,
  user: AuthUser,
  notificationId: string,
  read: boolean,
) => {
  const { element } = await patchAttribute<StoreEntityNotification>(
    context,
    user,
    notificationId,
    ENTITY_TYPE_NOTIFICATION,
  ...
  await notify(
    BUS_TOPICS[NOTIFICATION_NUMBER].EDIT_TOPIC,
    { count: unreadNotificationsCount, user_id: element.user_id },
    user,
  );
...
export const addNotification = async (
  context: AuthContext,
  user: AuthUser,
  notification: NotificationAddInput,
) => {
