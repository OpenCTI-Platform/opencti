...
const csvMapperResolvers: Resolvers = {
  Query: {
    ...
    csvMapperAddInputFromImport: (_, { file }, context) =>
      csvMapperAddInputFromImport(context, context.user, file),
  },
  CsvMapper: {
    errors: (csvMapper, _, context) => getCsvMapperErrorMessage(context, context.user, csvMapper),
    representations: (csvMapper, _, context) =>
      getParsedRepresentations(context, context.user, csvMapper),
    toConfigurationExport: (csvMapper, _, context) =>
      csvMapperExport(context, context.user, csvMapper),
  },
  Mutation: {
