...
} from '../../entitySetting/entitySetting-utils';
...
export const findById = async (context: AuthContext, user: AuthUser, csvMapperId: string) => {
  return storeLoadById<BasicStoreEntityCsvMapper>(
    context,
    user,
    csvMapperId,
    ENTITY_TYPE_CSV_MAPPER,
...
export const findCsvMapperPaginated = (
  context: AuthContext,
  user: AuthUser,
  opts: QueryCsvMappersArgs,
) => {
  return pageEntitiesConnection<BasicStoreEntityCsvMapper>(
    context,
    user,
    [ENTITY_TYPE_CSV_MAPPER],
    opts,
...
export const createCsvMapper = async (
  context: AuthContext,
  user: AuthUser,
  csvMapperInput: CsvMapperAddInput,
) => {
  ...
  return createInternalObject<StoreEntityCsvMapper>(
    context,
    user,
    csvMapperInput,
    ENTITY_TYPE_CSV_MAPPER,
...
export const fieldPatchCsvMapper = async (
  context: AuthContext,
  user: AuthUser,
  csvMapperId: string,
  input: EditInput[],
) => {
  return editInternalObject<StoreEntityCsvMapper>(
    context,
    user,
    csvMapperId,
    ENTITY_TYPE_CSV_MAPPER,
...
export const deleteCsvMapper = async (
  context: AuthContext,
  user: AuthUser,
  csvMapperId: string,
) => {
  ...
  const ingesters = await fullEntitiesList<BasicStoreEntityIngestionCsv>(
    context,
    user,
    [ENTITY_TYPE_INGESTION_CSV],
    opts,
  ...
  if (ingesters.length > 0) {
    throw FunctionalError(
      'Cannot delete this CSV Mapper: it is used by one or more IngestionCsv feed(s)',
      { id: csvMapperId },
    );
  }
...
export const csvMapperExport = async (
  context: AuthContext,
  user: AuthUser,
  csvMapper: BasicStoreEntityCsvMapper,
) => {
  const { name, has_header, separator, representations, skipLineChar } = csvMapper;
...
export const transformCsvMapperConfig = async (
  configuration: CsvMapperParsed,
  context: AuthContext,
  user: AuthUser,
): Promise<CsvMapperResolved> => {
...
export const csvMapperAddInputFromImport = async (
  context: AuthContext,
  user: AuthUser,
...
export const csvMapperSchemaAttributes = async (context: AuthContext, user: AuthUser) => {
  ...
  types.forEach((type) => {
    ...
    const attributes: CsvMapperSchemaAttribute[] = Array.from(attributesDef.values()).flatMap(
      (attribute) => {
  ...
  types.forEach((type) => {
    ...
    pushAll(
      ...
      Array.from(refs.values()).flatMap((ref) => {
  ...
  for (let schemaIndex = 0; schemaIndex < schemaAttributes.length; schemaIndex += 1) {
    ...
    if (entitySetting) {
      getAttributesConfiguration(entitySetting)?.forEach((userDefinedAttr) => {
        const attributeIndex = schemaAttribute.attributes.findIndex(
          (a) => a.name === userDefinedAttr.name,
        );
        if (attributeIndex > -1) {
          const attribute = schemaAttribute.attributes[attributeIndex];
          if (
            attribute.mandatoryType === 'customizable' &&
            isNotEmptyField(userDefinedAttr.mandatory)
          ) {
            attribute.mandatory = userDefinedAttr.mandatory;
          }
          if (isNotEmptyField(userDefinedAttr.default_values)) {
            attribute.defaultValues = (userDefinedAttr.default_values as string[])?.map((v) => ({
              id: v,
              name: v,
            }));
            ...
            if (attribute.name !== 'objectMarking' && refsNames.includes(attribute.name)) {
              attributesDefaultValuesToResolve[`${schemaIndex}-${attributeIndex}`] =
                userDefinedAttr.default_values ?? [];
            }
          }
        }
      });
  ...
  Object.keys(attributesDefaultValuesToResolve).forEach((indexes) => {
    ...
    if (defaultValues) {
      schemaAttributes[schemaIndex].attributes[attributeIndex].defaultValues = defaultValues.map(
        (val) => {
