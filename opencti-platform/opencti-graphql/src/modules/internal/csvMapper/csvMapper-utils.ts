...
const isCsvMapperRepresentation = (object: any): object is CsvMapperRepresentation => {
  ...
  return (
    object.id &&
    (object.type === CsvMapperRepresentationType.Entity ||
      object.type === CsvMapperRepresentationType.Relationship)
  );
};
...
export const parseCsvMapper = (mapper: any): CsvMapperParsed => {
  ...
  if (typeof mapper?.representations === 'string') {
    try {
      ...
      throw FunctionalError('Could not parse CSV mapper: representations is not a valid JSON', {
        name: mapper?.name,
        error,
      });
    }
...
export const parseCsvMapperWithDefaultValues = async (
  context: AuthContext,
  user: AuthUser,
  mapper: any,
...
export const validateCsvMapper = async (
  context: AuthContext,
  user: AuthUser,
  ...
  if (
    ...
    mapper.representations.some((rep) => !isCsvMapperRepresentation(rep))
  ) {
    throw FunctionalError(
      'CSV mapper representations is not an array of CsvMapperRepresentation objects',
    ...
    Array.from(mapper.representations.entries()).map(async ([idx, representation]) => {
      ...
      [...attributesDefs, ...refsDefs]
        ...
        .forEach((schemaAttribute) => {
          ...
          const isColumnEmpty =
            isEmptyField(attribute?.column?.column_name) &&
            isEmptyField(attribute?.based_on?.representations);
      ...
      representation.attributes.forEach((attribute) => {
        ...
        if (isNotEmptyField(attribute.based_on?.representations as string[] | undefined)) {
          ...
          // Possible cycle
          const representationRefs = mapper.representations.filter((r) =>
            attribute.based_on?.representations?.includes(r.id),
          );
...
export const getCsvMapperErrorMessage = async (
  context: AuthContext,
  user: AuthUser,
