...
export const parseJsonMapper = (mapper: any): JsonMapperParsed => {
  ...
  if (typeof mapper?.representations === 'string') {
    try {
      ...
      throw FunctionalError('Could not parse JSON mapper: representations is not a valid JSON', {
        name: mapper?.name,
        error,
      });
    }
      ...
      rep.attributes.forEach((attr) => {
        if (attr.mode === 'base' && !Array.isArray(attr.based_on.identifier)) {
          attr.based_on.identifier = (attr.based_on.representations ?? []).map((r) => {
            return {
              identifier: attr.based_on.identifier,
              representation: r,
...
export const parseJsonMapperWithDefaultValues = async (
  context: AuthContext,
  user: AuthUser,
  mapper: any,
): Promise<JsonMapperResolved> => {
  if (typeof mapper?.representations !== 'string') {
    return mapper;
  }
  ...
  const entities = (await internalFindByIds<BasicStoreEntity>(
    context,
    user,
    refDefaultValues,
  )) as BasicStoreEntity[];
      ...
      attributes: representation.attributes.map((attribute, j) => ({
        ...
        default_values: attribute.default_values?.map((val) => {
          ...
          return {
            id: val,
            name:
              refAttributesIndexes.includes(`${i}-${j}`) && representative ? representative : val,
          };
        }),
      })),
    })),
...
const isJsonMapperRepresentation = (object: any): object is JsonMapperRepresentation => {
  ...
  return (
    object.id &&
    (object.type === JsonMapperRepresentationType.Entity ||
      object.type === JsonMapperRepresentationType.Relationship)
  );
};
...
export const validateJsonMapper = async (
  context: AuthContext,
  user: AuthUser,
  mapper: JsonMapperParsed,
) => {
  if (
    !Array.isArray(mapper.representations) ||
    mapper.representations.some((rep) => !isJsonMapperRepresentation(rep))
  ) {
    throw FunctionalError(
      'JSON mapper representations is not an array of JsonMapperRepresentation objects',
  ...
  await Promise.all(
    Array.from(mapper.representations.entries()).map(async ([idx, representation]) => {
      ...
      [...attributesDefs, ...refsDefs]
        .filter((schemaAttribute) => schemaAttribute.mandatory)
      ...
      representation.attributes.forEach((attribute) => {
        ...
        if (attribute.mode === 'base') {
          const schemaAttribute = [...attributesDefs, ...refsDefs].find(
            (attr) => attr.name === attribute.key,
          );
          // Multiple
          if (
            !schemaAttribute?.multiple &&
            (attribute.based_on?.representations?.length ?? 0) > 1
          ) {
            throw FunctionalError("Attribute can't be multiple", {
              representation: representationLabel(idx, representation),
          ...
          if (attribute.based_on?.representations?.includes(representation.id)) {
            throw FunctionalError("Can't reference the representation itself", {
              representation: representationLabel(idx, representation),
              attribute: attribute.key,
            });
          }
          // Possible cycle
          const representationRefs = mapper.representations.filter(
            (r) => attribute.mode === 'base' && attribute.based_on?.representations?.includes(r.id),
          );
          const attributeRepresentationRefs = representationRefs
            .map((rr) =>
              rr.attributes
                .filter(
                  (rra) => rra.mode === 'base' && isNotEmptyField(rra.based_on?.representations),
                )
                .map(
                  (rra) =>
                    ((rra as BasedRepresentationAttribute).based_on?.representations as string[]) ??
                    [],
                )
                .flat(),
            )
          ...
          if (attributeRepresentationRefs.includes(representation.id)) {
            throw FunctionalError('Reference cycle found', {
              representation: representationLabel(idx, representation),
            });
          }
        }
...
export const getJsonMapperErrorMessage = async (
  context: AuthContext,
  user: AuthUser,
