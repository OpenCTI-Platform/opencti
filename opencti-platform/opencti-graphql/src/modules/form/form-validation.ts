...
export const validateFormSubmission = (
  ...
  if (
    !schema.mainEntityLookup &&
    schema.mainEntityMultiple &&
    schema.mainEntityFieldMode === 'multiple'
  ) {
    schema.fields
      .filter((field) => field.attributeMapping.entity === 'main_entity')
      .forEach((field) => {
  ...
  if (
    !schema.mainEntityLookup &&
    schema.mainEntityMultiple &&
    schema.mainEntityFieldMode === 'parsed'
  ) {
        ...
        .forEach((field) => {
          if ((field.isMandatory || field.required) && !(field.isReadOnly && !isBypass)) {
            const fieldValue =
              ...
              values.mainEntityFields[field.name];
            if (isEmptyField(fieldValue)) {
              errors.push(`Required field "${field.label || field.name}" is missing`);
            }
          }
  ...
  if (schema.additionalEntities) {
    for (let index = 0; index < schema.additionalEntities.length; index += 1) {
      ...
      if (
        additionalEntity.lookup &&
        isEmptyField(values[`additional_${additionalEntity.id}_lookup`]) &&
        (additionalEntity?.minAmount ?? 0) > 0
      ) {
      ...
      if (
        ...
        schema.fields
          .filter((field) => field.attributeMapping.entity === additionalEntity.id)
          .forEach((field) => {
            if ((field.isMandatory || field.required) && !(field.isReadOnly && !isBypass)) {
            ...
            .forEach((field) => {
              if ((field.isMandatory || field.required) && !(field.isReadOnly && !isBypass)) {
                const fieldValue =
                  ...
                  ] ?? values[`additional_${additionalEntity.id}_fields`][field.name];
                if (isEmptyField(fieldValue)) {
                  errors.push(`Required field "${field.label || field.name}" is missing`);
                }
              }
