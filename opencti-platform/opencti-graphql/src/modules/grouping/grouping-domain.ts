...
export const findById: DomainFindById<BasicStoreEntityGrouping> = (
  context: AuthContext,
  user: AuthUser,
  groupingId: string,
) => {
  return storeLoadById<BasicStoreEntityGrouping>(
    context,
    user,
    groupingId,
    ENTITY_TYPE_CONTAINER_GROUPING,
...
export const findGroupingPaginated = (
  context: AuthContext,
  user: AuthUser,
  opts: EntityOptions<BasicStoreEntityGrouping>,
) => {
  return pageEntitiesConnection<BasicStoreEntityGrouping>(
    context,
    user,
    [ENTITY_TYPE_CONTAINER_GROUPING],
    opts,
...
export const addGrouping = async (
  context: AuthContext,
  user: AuthUser,
  grouping: GroupingAddInput,
) => {
  const groupingToCreate = isEmptyField(grouping.created)
    ? { ...grouping, created: now() }
    : grouping;
...
// Entities tab
export const groupingContainsStixObjectOrStixRelationship = async (
  context: AuthContext,
  user: AuthUser,
  groupingId: string,
  thingId: string,
...
// region series
export const groupingsTimeSeries = (
  context: AuthContext,
  user: AuthUser,
  args: QueryGroupingsTimeSeriesArgs,
) => {
  return timeSeriesEntities(context, user, [ENTITY_TYPE_CONTAINER_GROUPING], args);
};

export const groupingsNumber = async (
  context: AuthContext,
  user: AuthUser,
  args: QueryGroupingsNumberArgs,
): Promise<GroupingNumberResult> => {
  const countPromise = elCount(context, user, READ_INDEX_STIX_DOMAIN_OBJECTS, {
    ...args,
    types: [ENTITY_TYPE_CONTAINER_GROUPING],
  }) as Promise<number>;
...
export const groupingsTimeSeriesByEntity = (
  context: AuthContext,
  user: AuthUser,
  args: QueryGroupingsTimeSeriesArgs,
) => {
...
export const groupingsTimeSeriesByAuthor = async (
  context: AuthContext,
  user: AuthUser,
  args: QueryGroupingsTimeSeriesArgs,
) => {
...
export const groupingsNumberByEntity = async (
  context: AuthContext,
  user: AuthUser,
  args: QueryGroupingsNumberArgs,
): Promise<GroupingNumberResult> => {
...
export const groupingsNumberByAuthor = async (
  context: AuthContext,
  user: AuthUser,
  args: QueryGroupingsNumberArgs,
): Promise<GroupingNumberResult> => {
...
export const groupingsDistributionByEntity = async (
  context: AuthContext,
  user: AuthUser,
  args: QueryGroupingsDistributionArgs,
) => {
  const { objectId } = args;
  const filters = addFilter(args.filters, buildRefRelationKey(RELATION_OBJECT, '*'), objectId);
  return distributionEntities(context, user, [ENTITY_TYPE_CONTAINER_GROUPING], {
    ...args,
    filters,
  });
};
