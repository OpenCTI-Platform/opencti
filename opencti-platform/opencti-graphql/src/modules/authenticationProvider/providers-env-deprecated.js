...
export const unregisterAuthenticationProvider = (providerRef) => {
  ...
  if (removedItem) {
    logAuthInfo(
      `Strategy ${providerRef} unregistered on node ${NODE_INSTANCE_ID}`,
      removedItem.strategy,
    );
  } else {
...
export const registerAuthenticationProvider = (providerRef, strategy, configuration) => {
  if (isAuthenticationActivatedByIdentifier(configuration.provider)) {
    logApp.warn(
      `[AUTH PROVIDER] identifier ${configuration.provider} already registered. Please check your configuration to see if there is not 2 time the same identifier.`,
    );
...
export const initializeEnvAuthenticationProviders = async () => {
    ...
    for (let i = 0; i < providerKeys.length; i += 1) {
      ...
      if (config === undefined || !config.disabled) {
        ...
        if (strategy === EnvStrategyType.STRATEGY_LDAP) {
          ...
          logApp.info(
            `[ENV-PROVIDER][LDAP] LdapStrategy found in configuration providerRef:${providerRef}`,
          );
          // region backward compatibility
          const allowSelfSigned =
            mappedConfig.allow_self_signed || mappedConfig.allow_self_signed === 'true';
          // Force bindCredentials to be a String
          mappedConfig.bindCredentials = `${mappedConfig.bindCredentials}`;
          const tlsConfig = R.assoc(
            'tlsOptions',
          ...
          const ldapStrategy = new LdapStrategy(ldapOptions, (user, done) => {
            ...
            const userMail = mappedConfig.mail_attribute
              ? user[mappedConfig.mail_attribute]
              : user.mail;
            const userName = mappedConfig.account_attribute
              ? user[mappedConfig.account_attribute]
              : user.givenName;
            const firstname = user[mappedConfig.firstname_attribute] || '';
            const lastname = user[mappedConfig.lastname_attribute] || '';
            const isGroupBaseAccess =
              isNotEmptyField(mappedConfig.groups_management) &&
              isNotEmptyField(mappedConfig.groups_management?.groups_mapping);
            ...
            const isOrgaMapping =
              isNotEmptyField(mappedConfig.organizations_default) ||
              isNotEmptyField(mappedConfig.organizations_management);
            const computeOrganizationsMapping = () => {
              const orgaDefault = mappedConfig.organizations_default ?? [];
              const orgasMapping =
                mappedConfig.organizations_management?.organizations_mapping || [];
              const orgaPath = mappedConfig.organizations_management?.organizations_path || [
                'organizations',
              ];
              ...
              return [
                ...orgaDefault,
                ...availableOrgas.map((a) => orgasMapper[a]).filter((r) => isNotEmptyField(r)),
              ];
            };
            ...
            if (!userMail) {
              ...
              logApp.info(
                `[ENV-PROVIDER][LDAP] Connecting/creating account with ${userMail} [name=${userName}]`,
              );
        ...
        if (strategy === EnvStrategyType.STRATEGY_SAML) {
          ...
          logApp.info(
            `[ENV-PROVIDER][SAML] SamlStrategy found in configuration providerRef:${providerRef}`,
          );
          ...
          const samlStrategy = new SamlStrategy(
            ...
            (profile, done) => {
              ...
              const groupAttributes = mappedConfig.groups_management?.group_attributes || [
                'groups',
              ];
              ...
              if (mappedConfig.mail_attribute && !samlAttributes[mappedConfig.mail_attribute]) {
                logApp.info(
                  `[ENV-PROVIDER][SAML] custom mail_attribute "${mappedConfig.mail_attribute}" in configuration but the custom field is not present SAML server response.`,
                );
              }
              ...
              const isGroupBaseAccess =
                isNotEmptyField(mappedConfig.groups_management) &&
                isNotEmptyField(mappedConfig.groups_management?.groups_mapping);
              logApp.info('[ENV-PROVIDER][SAML] Groups management configuration', {
                groupsManagement: mappedConfig.groups_management,
              });
              // region roles mapping
              const computeRolesMapping = () => {
                const attrRoles = roleAttributes.map((a) =>
                  Array.isArray(samlAttributes[a]) ? samlAttributes[a] : [samlAttributes[a]],
                );
              ...
              // region groups mapping
              const computeGroupsMapping = () => {
                const attrGroups = groupAttributes.map((a) =>
                  Array.isArray(samlAttributes[a]) ? samlAttributes[a] : [samlAttributes[a]],
                );
              ...
              const groupsToAssociate = R.uniq(
                computeGroupsMapping().concat(computeRolesMapping()),
              );
              // endregion
              // region organizations mapping
              const isOrgaMapping =
              ...
              const organizationsToAssociate = isOrgaMapping ? computeOrganizationsMapping() : [];
              // endregion
              logApp.info('[ENV-PROVIDER][SAML] Login handler', {
                isGroupBaseAccess,
                groupsToAssociate,
              });
                  ...
                  providerLoginHandler(
                    {
                      ...
                      firstname,
                      lastname,
                      provider_metadata: { nameID, nameIDFormat },
                    },
                    done,
                    opts,
                  ),
        ...
        if (strategy === EnvStrategyType.STRATEGY_OPENID) {
          ...
          logApp.info(
            `[ENV-PROVIDER][OPENID] OpenIDConnectStrategy found in configuration providerRef:${providerRef}`,
          );

          // region backward compatibility
          // Here we use directly the config and not the mapped one.
          // All config of openid lib use snake case.
          const openIdClient = config.use_proxy
            ? getPlatformHttpProxyAgent(config.issuer)
            : undefined;
          OpenIDCustom.setHttpOptionsDefaults({ timeout: 0, agent: openIdClient });
          enrichWithRemoteCredentials(`providers:${providerIdent}`, config)
            .then((clientConfig) => {
              OpenIDIssuer.discover(config.issuer)
                .then((issuer) => {
                  const { Client } = issuer;
                  const client = new Client(clientConfig);
                  // region scopes generation
                  const defaultScopes = mappedConfig.default_scopes ?? [
                    'openid',
                    'email',
                    'profile',
                  ];
                  ...
                  const organizationsScope =
                    mappedConfig.organizations_management?.organizations_scope;
                  ...
                  const options = {
                    ...
                    params: {
                      scope: openIdScope,
                      ...(mappedConfig.audience && { audience: mappedConfig.audience }),
                    },
                  };
                  ...
                  const openIDStrategy = new OpenIDStrategy(
                    options,
                    debugCallback,
                    (_, tokenset, userinfo, done) => {
                      logApp.info('[ENV-PROVIDER][OPENID] Successfully logged', { userinfo });
                      const isGroupMapping =
                        isNotEmptyField(mappedConfig.groups_management) &&
                        isNotEmptyField(mappedConfig.groups_management?.groups_mapping);
                      logApp.info('[ENV-PROVIDER][OPENID] Groups management configuration', {
                        groupsManagement: mappedConfig.groups_management,
                      });
                      // region groups mapping
                      const computeGroupsMapping = () => {
                        ...
                        const token =
                          mappedConfig.groups_management?.token_reference || 'access_token';
                        ...
                        if (!readUserinfo) {
                          logApp.info(`[ENV-PROVIDER][OPENID] Groups mapping on decoded ${token}`, {
                            decoded: decodedUser,
                          });
                        }
                        const availableGroups = R.flatten(
                          groupsPath.map((path) => {
                        ...
                        return availableGroups
                          .map((a) => groupsMapper[a])
                          .filter((r) => isNotEmptyField(r));
                      };
                      ...
                      const computeOrganizationsMapping = () => {
                        const orgaDefault = mappedConfig.organizations_default ?? [];
                        const readUserinfo =
                          mappedConfig.organizations_management?.read_userinfo || false;
                        const orgasMapping =
                          mappedConfig.organizations_management?.organizations_mapping || [];
                        const token =
                          mappedConfig.organizations_management?.token_reference || 'access_token';
                        const orgaPath = mappedConfig.organizations_management
                      ...
                      const organizationsToAssociate = isOrgaMapping
                        ? computeOrganizationsMapping()
                        : [];
                      // endregion
                      if (!isGroupMapping || groupsToAssociate.length > 0) {
                        ...
                        const get_user_attributes_from_id_token =
                          mappedConfig.get_user_attributes_from_id_token ?? false;

                        const user_attribute_obj = get_user_attributes_from_id_token
                  ...
                  openIDStrategy.logout = (_, callback) => {
                    ...
                    const endpointUri = issuer.end_session_endpoint
                      ? issuer.end_session_endpoint
                      : `${config.issuer}/oidc/logout`;
                    logApp.debug(
                      `[ENV-PROVIDER][OPENID] logout configuration, isSpecificUri:${isSpecificUri}, issuer.end_session_endpoint:${issuer.end_session_endpoint}, final endpointUri: ${endpointUri}`,
                    );
                    if (isSpecificUri) {
                ...
                .catch((err) => {
                  logApp.error(
                    '[ENV-PROVIDER][OPENID] Error initializing authentication provider',
                    { cause: err, provider: providerRef },
                  );
                });
            })
            .catch((reason) =>
              logApp.error('[ENV-PROVIDER][OPENID] Error when enrich with remote credentials', {
                cause: reason,
              }),
            );
          // end region backward compatibility
        ...
        if (strategy === EnvStrategyType.STRATEGY_FACEBOOK) {
          ...
          logApp.warn(
            `[ENV-PROVIDER][FACEBOOK] DEPRECATED Strategy found in configuration providerRef:${providerRef}, please consider using OpenID`,
          );
        ...
        if (strategy === EnvStrategyType.STRATEGY_GOOGLE) {
          ...
          logApp.warn(
            `[ENV-PROVIDER][GOOGLE] DEPRECATED Strategy found in configuration providerRef:${providerRef}, please consider using OpenID`,
          );
        ...
        if (strategy === EnvStrategyType.STRATEGY_GITHUB) {
          ...
          logApp.warn(
            `[ENV-PROVIDER][GITHUB] DEPRECATED Strategy found in configuration providerRef:${providerRef}, please consider using OpenID`,
          );
        ...
        if (strategy === EnvStrategyType.STRATEGY_AUTH0) {
          ...
          logApp.warn(
            `[ENV-PROVIDER][AUTH0] DEPRECATED Strategy found in configuration providerRef:${providerRef}, please consider using OpenID`,
          );
          ...
          // Here we use directly the config and not the mapped one.
          // All config of openid lib use snake case.
          const openIdClient = auth0config.use_proxy
            ? getPlatformHttpProxyAgent(auth0Issuer)
            : undefined;
          OpenIDCustom.setHttpOptionsDefaults({ timeout: 0, agent: openIdClient });
          OpenIDIssuer.discover(auth0Issuer)
            .then((issuer) => {
              const { Client } = issuer;
              ...
              const debugCallback = (message, meta) => logApp.info(message, meta);
              const auth0Strategy = new OpenIDStrategy(
                options,
                debugCallback,
                (_, tokenset, userinfo, done) => {
            ...
            .catch((reason) =>
              logApp.error('[ENV-PROVIDER][AUTH0] Error when enrich with remote credentials', {
                cause: reason,
              }),
            );
        }
