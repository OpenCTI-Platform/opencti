...
const encryptSmtpInput = async (
  ...
  for (const field of SMTP_SECRET_FIELDS) {
    if (field in result) {
      result[`${field}_encrypted`] = await encryptSmtpSecret(
...
const validateSmtpConfigurationInput = (input: SmtpConfigurationAddInput) => {
  ...
  if (
    input.auth_type === SmtpAuthType.Oauth2 &&
    (!input.oauth_client_id || !input.oauth_client_secret || !input.oauth_issuer)
  ) {
    throw FunctionalError(
      'oauth_client_id, oauth_client_secret and oauth_issuer are required for OAuth2 authentication',
    );
  }
};
...
export const smtpConfigurationEdit = async (
  ...
  await publishUserAction({
    ...
    context_data: {
      ...
      input: sanitizeInputForAudit(input as unknown as Record<string, unknown>),
    },
  });
