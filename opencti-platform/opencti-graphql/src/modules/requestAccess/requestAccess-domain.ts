...
export const configureRequestAccess = async (context: AuthContext, user: AuthUser, input: RequestAccessConfigureInput) => {
  ...
  let requestAccessConfigResult: RequestAccessConfiguration = {};
  if (rfiEntitySettings.request_access_workflow) {
    const { approval_admin } = input;
    let { approved_workflow_id } = rfiEntitySettings.request_access_workflow;
    let { declined_workflow_id } = rfiEntitySettings.request_access_workflow;
  ...
  // return getRequestAccessConfiguration(context, user, undefined);
