...
import {
  type BasicStoreEntitySupportPackage,
  ENTITY_TYPE_SUPPORT_PACKAGE,
  type StoreEntitySupportPackage,
  SUPPORT_BUS,
} from './support-types';
...
export const findById = (context: AuthContext, user: AuthUser, id: string) => {
  return storeLoadById<BasicStoreEntitySupportPackage>(
    context,
    user,
    id,
    ENTITY_TYPE_SUPPORT_PACKAGE,
...
export const findSupportPaginated = (
  context: AuthContext,
  user: AuthUser,
  args: QuerySupportPackagesArgs,
) => {
  return pageEntitiesConnection<BasicStoreEntitySupportPackage>(
    context,
    user,
    [ENTITY_TYPE_SUPPORT_PACKAGE],
    args,
...
export const deleteSupportPackage = async (
  context: AuthContext,
  user: AuthUser,
...
export const archiveFolderToZip = async (zipLocalFolder: string, zipFullpath: string) => {
  ...
  if (!closed) {
    throw FilesystemError('Zip creation timed out before completion', {
      zipFullpath,
 ...
 * @param entity
 */
export const sendCurrentNodeSupportLogToS3 = async (
  context: AuthContext,
  user: AuthUser,
  entity: StoreEntitySupportPackage,
) => {
  ...
  const supportFiles = findAllSupportFiles(
    fs.readdirSync(SUPPORT_LOG_RELATIVE_LOCAL_DIR),
    SUPPORT_LOG_FILE_PREFIX,
  );
  const telemetryFiles = findAllSupportFiles(
    fs.readdirSync(TELEMETRY_LOG_RELATIVE_LOCAL_DIR),
    TELEMETRY_LOG_FILE_PREFIX,
  );
...
const downloadAllLogFiles = async (
  context: AuthContext,
  user: AuthUser,
  s3Directory: string,
  ...
  for (let i = 0; i < allSupportFiles.length; i += 1) {
    ...
    if (
      supportFile &&
      supportFile.name.substring(supportFile.name.length - 4, supportFile.name.length) !== '.zip'
    ) {
      ...
      if (stream) {
        ...
        logApp.warn('Support file not found in S3, skipping', {
          fileId: supportFile.id,
...
const uploadArchivedSupportPackageToS3 = async (
  context: AuthContext,
  user: AuthUser,
  zipFullpath: string,
  zipFileName: string,
  entity: BasicStoreEntitySupportPackage,
) => {
  ...
  if (fs.existsSync(zipFullpath)) {
    ...
    const { upload } = await uploadToStorage(
      ...
      user,
      uploadDirectory,
      fileToReadStream(zipFullpath, zipFileName, zipFileName, ZIP_MIME_TYPE),
      {},
    );
 ...
 * @param entity
 */
export const zipAllSupportFiles = async (
  context: AuthContext,
  user: AuthUser,
  entity: BasicStoreEntitySupportPackage,
) => {
  ...
  const updatedArchive = await uploadArchivedSupportPackageToS3(
    context,
    user,
    SUPPORT_LOG_RELATIVE_LOCAL_DIR,
    zipName,
 ...
 * @param input
 */
export const prepareNewSupportPackage = async (
  context: AuthContext,
  user: AuthUser,
  input: SupportPackageAddInput,
) => {
  const settings = await getSettings(context);
  logApp.info('Starting support package generation', {
    number_of_nodes: settings.platform_cluster.instances_number,
  });
  ...
  const supportDataCreated = await createInternalObject<StoreEntitySupportPackage>(
    context,
    user,
    supportInput,
    ENTITY_TYPE_SUPPORT_PACKAGE,
 ...
 * @param input
 */
export const addSupportPackage = async (
  context: AuthContext,
  user: AuthUser,
  input: SupportPackageAddInput,
) => {
 ...
 */
export const requestZipPackage = async (
  context: AuthContext,
  user: AuthUser,
  supportPackage: SupportPackageForceZipInput,
) => {
 ...
 * @param newStatus
 */
export const registerNodeInSupportPackage = async (
  context: AuthContext,
  user: AuthUser,
  packageId: string,
  newStatus: PackageStatus,
) => {
  logApp.info('[OPENCTI-MODULE] Updating Support Package', {
    packageId,
  ...
  if (actualPackage) {
    ...
    if (newStatus === PackageStatus.Ready || newStatus === PackageStatus.InError) {
      const inProgressNodes = await redisCountSupportPackageNodeWithStatus(
        actualPackage.id,
        SUPPORT_NODE_STATUS_IN_PROGRESS,
      );
      if (inProgressNodes === 0) {
        ...
        const nodeInErrorCount = await redisCountSupportPackageNodeWithStatus(
          actualPackage.id,
          SUPPORT_NODE_STATUS_IN_ERROR,
        );
