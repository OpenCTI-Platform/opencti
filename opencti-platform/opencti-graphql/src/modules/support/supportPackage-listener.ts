...
export const onSupportPackageMessage = async (event: { instance: BasicStoreEntity }) => {
  logApp.info(
    `[OPENCTI-MODULE] Support Package got event. ${event.instance.id} on node ${NODE_INSTANCE_ID}`,
  );
  try {
    if (event.instance.entity_type === ENTITY_TYPE_SUPPORT_PACKAGE) {
      await registerNodeInSupportPackage(
        context,
        SYSTEM_USER,
        event.instance.id,
      ...
      await sendCurrentNodeSupportLogToS3(
        context,
        SYSTEM_USER,
        event.instance as StoreEntitySupportPackage,
      );
      await registerNodeInSupportPackage(
        context,
        SYSTEM_USER,
      ...
      logApp.warn('Entity cannot be sent to SupportPackage PubSubListener', {
        type: event.instance.entity_type,
      });
    }
  } catch (error) {
    logApp.error('Error generating support package (first round)', {
      cause: error,
      from: 'supportListener',
      nodeId: NODE_INSTANCE_ID,
      packageId: event.instance.id,
    });
    try {
      await registerNodeInSupportPackage(
        context,
      ...
      logApp.error('Error generating support package (second round)', {
        cause: errorInError,
        from: 'supportListener',
        nodeId: NODE_INSTANCE_ID,
        packageId: event.instance.id,
...
const initSupportPackageListener = () => {
  return {
    ...
    start: async () => {
      ...
      supportPackagePuSub = await pubSubSubscription<{ instance: StoreEntity }>(
        `${TOPIC_PREFIX}ENTITY_TYPE_SUPPORT_PACKAGE_EDIT_TOPIC`,
        onSupportPackageMessage,
      );
