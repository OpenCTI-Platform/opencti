...
import type { WorkflowDefinitionData, WorkflowSerializedState, WorkflowSerializedTransition } from '../types/workflow-types';
...
/**
 * Synthesizes a fully-connected transition graph over `stateIds` (per plan.md Task 6, Step 1.1b):
 * legacy `Status` data has no edge/transition concept, only a flat `order`, and today's
 * `StatusField` UI lets users jump to any status in any order. To introduce zero new ordering
 * restrictions on migrated data, every state must be able to transition to every other state.
 * One transition is synthesized per target state, with `from` listing every other state, rather
 * than one transition per ordered pair (N transitions instead of N*(N-1)).
 */
const synthesizeFullyConnectedTransitions = (stateIds: string[]): WorkflowSerializedTransition[] => {
...
const convertScopeGroup = (
  ...
  const states: WorkflowSerializedState[] = ordered.map((status) => ({
    statusId: status.template_id,
    order: hasOrder(status) ? status.order : undefined,
  }));
  ...
  return {
    definition: {
      ...
      states,
      transitions: synthesizeFullyConnectedTransitions(stateIds),
    },
    diagnostics,
  };
};
 ...
 * per `scope`, with diagnostics. No context/user/store access (per plan.md Task 6, Interfaces).
 *
 ...
 * `template_id` for diagnostics only; state identity is always `template_id`, never the name.
 */
