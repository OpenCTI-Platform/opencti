 ...
 *
 * The platform indices, plus the trash. The trash is restorable, so leaving deleted copies
 * untouched would re-inject source ids into live data on restore — including access control
 * pointing at a user that no longer exists. It is a single index, not a pattern, so the cost
