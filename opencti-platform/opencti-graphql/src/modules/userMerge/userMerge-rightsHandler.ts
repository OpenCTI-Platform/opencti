 ...
 *
 * Read on the projected rights rather than the target's own: under UNION the target inherits
 * the source capabilities, so a target that cannot manage authorized members today can manage
 * them once the merge is applied, and refusing on the pre-merge answer blocks a merge that
 * never leaves anyone locked out.
 */
...
export const userMergeRightsHandler: UserMergeHandler = {
  ...
  compute: async ({ context, sourceId, targetId, options, rights }: UserMergeHandlerContext): Promise<UserMergeHandlerPlan> => {
    ...
    const canManage = rights.projected.capabilities.includes(BYPASS) || rights.projected.capabilities.includes(KNOWLEDGE_KNUPDATE_KNMANAGEAUTHMEMBERS);
