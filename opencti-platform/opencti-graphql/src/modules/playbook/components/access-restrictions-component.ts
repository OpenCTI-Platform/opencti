    ...
    executor: async ({ dataInstanceId, playbookNode, bundle }) => {
      ...
      for (let index = 0; index < accessRestrictions.length; index += 1) {
        ...
        if (accessRestriction.value === 'AUTHOR') {
          ...
          const creators = (baseData.extensions[STIX_EXT_OCTI].creator_ids ?? []).filter((id) =>
            isNotEmptyField(id),
          );
          ...
          const assignees = (baseData.extensions[STIX_EXT_OCTI].assignee_ids ?? []).filter((id) =>
            isNotEmptyField(id),
          );
          ...
          const participants = (baseData.extensions[STIX_EXT_OCTI].participant_ids ?? []).filter(
            (id) => isNotEmptyField(id),
          );
          ...
          const bundleOrganizationsIds = bundleOrganizations
            .map((o) => o.extensions[STIX_EXT_OCTI].id)
            .filter((id) => isNotEmptyField(id));
          for (let index2 = 0; index2 < bundleOrganizationsIds.length; index2 += 1) {
            finalAccessRestrictions.push({
              ...accessRestriction,
              value: bundleOrganizationsIds[index2],
            });
          }
        } else {
      ...
      for (let index = 0; index < bundle.objects.length; index += 1) {
        ...
        const isFilteredElement = await isBundleElementMatchFilters(
          context,
          element,
          applyWithFilters,
        );
        ...
        if (isTypeCompatible && isFilteredElement && isElementInScope) {
          ...
          const restrictedMembers = (await buildRestrictedMembers(
            context,
            AUTOMATION_MANAGER_USER,
            args,
          )) as AuthorizedMember[];
