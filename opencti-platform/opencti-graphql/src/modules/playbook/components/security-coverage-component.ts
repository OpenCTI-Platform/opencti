...
} from '../../securityCoverage/securityCoverage-types';
...
const PLAYBOOK_SECURITY_COVERAGE_COMPONENT_SCHEMA: JSONSchemaType<SecurityCoverageConfiguration> = {
  ...
  properties: {
    ...
    auto_enrichment_disable: {
      type: 'boolean',
      $ref: 'Force manual coverage (prevent enrichment connectors from running)',
      default: false,
    },
...
export const PLAYBOOK_SECURITY_COVERAGE_COMPONENT: PlaybookComponent<SecurityCoverageConfiguration> =
  {
    ...
    executor: async ({ dataInstanceId, playbookNode, bundle }) => {
      ...
      const elementsToApply = await filterBundleElements(
        context,
        inScopeElements,
        applyWithFilters,
      );
