...
} from '../../case/case-incident/case-incident-types';
...
export const buildStixTaskFromTaskTemplate = (
  taskTemplate: BasicStoreEntityTaskTemplate,
...
const createTaskFromCaseTemplates = async (
  ...
  for (let i = 0; i < caseTemplates.length; i += 1) {
    const taskTemplates = await findAllByCaseTemplateId(
      context,
      AUTOMATION_MANAGER_USER,
      caseTemplates[i].value,
    );
...
export const PLAYBOOK_CONTAINER_WRAPPER_COMPONENT_SCHEMA: JSONSchemaType<ContainerWrapperConfiguration> =
  {
    ...
    properties: {
      ...
      copyFiles: {
        type: 'boolean',
        $ref: 'Copy files from main element to the container',
        default: false,
      },
...
export const PLAYBOOK_CONTAINER_WRAPPER_COMPONENT: PlaybookComponent<ContainerWrapperConfiguration> =
  {
    ...
    schema: async () => {
      const elements = PLAYBOOK_CONTAINER_WRAPPER_COMPONENT_AVAILABLE_CONTAINERS.map((t) => ({
        const: t,
        title: t,
      }));
      const schemaElement = { properties: { container_type: { oneOf: elements } } };
      return R.mergeDeepRight<JSONSchemaType<ContainerWrapperConfiguration>, any>(
        PLAYBOOK_CONTAINER_WRAPPER_COMPONENT_SCHEMA,
        schemaElement,
      );
    },
    executor: async ({ dataInstanceId, playbookNode, bundle }) => {
      ...
      if (!PLAYBOOK_CONTAINER_WRAPPER_COMPONENT_AVAILABLE_CONTAINERS.includes(container_type)) {
        throw FunctionalError(
          'this container type is incompatible with the Container Wrapper playbook component',
          { container_type },
        );
      }
      if (container_type) {
        ...
        const inScopeElements = bundle.objects.filter((object) =>
          isBundleElementInScope(object, applyToElements, dataInstanceId),
        );
        container.object_refs = (
          await filterBundleElements(context, inScopeElements, applyWithFilters)
        ...
        if ((baseData as StixReport).extensions[STIX_EXT_OCTI].content) {
          (container as StixReport).extensions[STIX_EXT_OCTI].content = (
            baseData as StixReport
          ).extensions[STIX_EXT_OCTI].content;
        }
        ...
        if (baseData.extensions[STIX_EXT_OCTI].participant_ids) {
          container.extensions[STIX_EXT_OCTI].participant_ids =
            baseData.extensions[STIX_EXT_OCTI].participant_ids;
        }
        if (baseData.extensions[STIX_EXT_OCTI].assignee_ids) {
          container.extensions[STIX_EXT_OCTI].assignee_ids =
            baseData.extensions[STIX_EXT_OCTI].assignee_ids;
        }
        ...
        if (<StixIncident>baseData && container_type === ENTITY_TYPE_CONTAINER_CASE_INCIDENT) {
          ...
          (<StixCaseIncident>container).external_references = (<StixIncident>(
            baseData
          )).external_references;
          (<StixCaseIncident>container).extensions[STIX_EXT_OCTI].granted_refs = (<StixIncident>(
            baseData
          )).extensions[STIX_EXT_OCTI].granted_refs;
        }
          ...
          for (let index = 0; index < stixFileExtensions.length; index += 1) {
            ...
            if (currentFile.data !== undefined && currentFile.data !== null) {
              ...
              try {
                ...
                logApp.error("[PLAYBOOK] Can't copy file from main element to the container", {
                  cause: e,
        ...
        if (
          STIX_DOMAIN_OBJECT_CONTAINER_CASES.includes(container_type) &&
          caseTemplates.length > 0
        ) {
          const tasks = await createTaskFromCaseTemplates(
            caseTemplates,
            container as StixContainer,
          );
