...
const PLAYBOOK_CREATE_INDICATOR_COMPONENT_SCHEMA: JSONSchemaType<CreateIndicatorConfiguration> = {
  ...
  properties: {
    ...
    applyToElements: {
      ...
      oneOf: [
        ...
        },
        {
          ...
          title: 'All observables in the bundle',
        },
        {
    ...
    wrap_in_container: {
      type: 'boolean',
      $ref: 'If main entity is a container, wrap indicators in container',
      default: false,
    },
  },
...
export const PLAYBOOK_CREATE_INDICATOR_COMPONENT: PlaybookComponent<CreateIndicatorConfiguration> =
  {
    ...
    schema: async () => {
      ...
      const elements = types
        .map((t) => ({ const: t, title: t }))
        .sort((a, b) => (a.title.toLowerCase() > b.title.toLowerCase() ? 1 : -1));
      const schemaElement = { properties: { types: { items: { oneOf: elements } } } };
      return R.mergeDeepRight<JSONSchemaType<CreateIndicatorConfiguration>, any>(
        PLAYBOOK_CREATE_INDICATOR_COMPONENT_SCHEMA,
        schemaElement,
      );
    },
    executor: async ({ playbookNode, dataInstanceId, bundle }) => {
      const { applyToElements, applyWithFilters, wrap_in_container, types } =
        playbookNode.configuration;
      const context = executionContext('playbook_components');
      const baseData = extractBundleBaseElement(dataInstanceId, bundle);
      const scopedElements = bundle.objects.filter((object) =>
        isBundleElementInScope(object, applyToElements, dataInstanceId),
      );
      const observablesToApply = await filterBundleElements(
        context,
        scopedElements,
        applyWithFilters,
      );
      ...
      for (let index = 0; index < observablesToApply.length; index += 1) {
        ...
        if (isStixCyberObservable(type) && (isEmptyField(types) || types.includes(type))) {
          ...
          const score =
            observable.x_opencti_score ?? observable.extensions[STIX_EXT_OCTI_SCO]?.score;
          ...
          if (pattern) {
            ...
            if (observable.extensions[STIX_EXT_OCTI_SCO]?.external_references) {
              indicator.external_references =
                observable.extensions[STIX_EXT_OCTI_SCO].external_references;
            }
            if (granted_refs) {
            ...
            const stixRelationshipsInBundle = bundle.objects.filter(
              (r) => r.type === 'relationship',
            ) as StixRelation[];
            const stixRelationships = stixRelationshipsInBundle.filter(
              (r) =>
                r.relationship_type === 'related-to' &&
            ...
            for (
              let indexStixRelationships = 0;
              indexStixRelationships < stixRelationships.length;
              indexStixRelationships += 1
            ) {
              ...
              const relationIndicatesStandardId = idGenFromData(
                'relationship',
