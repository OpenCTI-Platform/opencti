...
export const PLAYBOOK_UNSHARING_COMPONENT: PlaybookComponent<UnsharingConfiguration> = {
  ...
  executor: async ({ dataInstanceId, playbookNode, bundle }) => {
    ...
    const organizationsByIds = (await internalFindByIds<BasicStoreEntityOrganization>(
      context,
      SYSTEM_USER,
      organizationsValues,
      {
    ...
    for (let index = 0; index < bundle.objects.length; index += 1) {
      ...
      if (isInScope && isMatchingFilters) {
        ...
        const newGrantedRefs = currentElementGrantedRefs.filter(
          (o) => !organizationIds.includes(o),
        );
