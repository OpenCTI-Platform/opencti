...
const PLAYBOOK_CREATE_OBSERVABLE_COMPONENT_SCHEMA: JSONSchemaType<CreateObservableConfiguration> = {
  ...
  properties: {
    applyToElements: {
      ...
      oneOf: [
        ...
        },
        {
          ...
          title: 'All indicators in the bundle',
        },
        {
    ...
    wrap_in_container: {
      type: 'boolean',
      $ref: 'If main entity is a container, wrap observables in container',
      default: false,
    },
  },
    ...
    executor: async ({ playbookNode, dataInstanceId, bundle }) => {
      ...
      for (let indexIndicator = 0; indexIndicator < bundle.objects.length; indexIndicator += 1) {
        ...
        if (isElementInScope && isFilteredElement && element.type === 'indicator') {
          ...
          for (
            let indexObservable = 0;
            indexObservable < observables.length;
            indexObservable += 1
          ) {
            ...
            if (indicator.created_by_ref && stixObservable.extensions[STIX_EXT_OCTI_SCO]) {
              stixObservable.extensions[STIX_EXT_OCTI_SCO].created_by_ref =
            ...
            if (indicator.external_references && stixObservable.extensions[STIX_EXT_OCTI_SCO]) {
              stixObservable.extensions[STIX_EXT_OCTI_SCO].external_references =
      ...
      if (objectsToPush.length > 0) {
        ...
        return {
          ...
          bundle: {
            ...bundle,
            objects: bundle.objects.map((n) => (n.id === baseData.id ? baseData : n)),
          },
        };
      }
