    ...
    async () => {
      ...
      const schemaAttributes: EntitySettingSchemaAttribute[] = [
        ...
        ...Array.from(refsDefinition.values())
          .filter(
            (ref: RefAttribute) =>
              ref.mandatoryType === 'external' || ref.mandatoryType === 'customizable',
          )
          .map((ref) => ({
            name: ref.name,
            label: ref.label,
      ...
      getAttributesConfiguration(entitySetting)?.forEach((userDefinedAttr) => {
        ...
        if (schemaIndex > -1) {
          ...
          if (schemaAttribute) {
            if (
              schemaAttribute.mandatoryType === 'customizable' &&
            ...
            if (isNotEmptyField(userDefinedAttr.default_values)) {
              schemaAttribute.defaultValues = (userDefinedAttr.default_values as string[])?.map(
                (v) => ({ id: v, name: v }),
              );
              // If the default value is a ref with an id, save it to resolve it below.
              if (
                schemaAttribute.name !== 'objectMarking' &&
                refsNames.includes(schemaAttribute.name)
              ) {
                attributesDefaultValuesToResolve[schemaIndex] =
...
export const getMandatoryAttributesForSetting = async (
  ...
  if (isUserHasCapability(user, KNOWLEDGE_KNUPDATE_KNBYPASSFIELDS)) {
    return attributes
      .filter((a) => a.mandatory && a.mandatoryType !== 'customizable')
      .map((a) => a.name);
  }
