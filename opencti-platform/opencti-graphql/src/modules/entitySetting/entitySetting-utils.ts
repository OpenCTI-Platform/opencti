...
const templateObjectSettings = ['attributes_configuration', 'platform_entity_files_ref', 'platform_hidden_type', 'enforce_reference', 'workflow_configuration', 'sync_workflow_status_by_name', 'templates'];

// Available settings works by override.
export const availableSettings: Record<string, Array<string>> = {
  [ABSTRACT_STIX_DOMAIN_OBJECT]: ['attributes_configuration', 'platform_entity_files_ref', 'platform_hidden_type', 'enforce_reference', 'workflow_configuration', 'sync_workflow_status_by_name'],
  [ABSTRACT_STIX_CORE_RELATIONSHIP]: ['attributes_configuration', 'enforce_reference', 'workflow_configuration', 'sync_workflow_status_by_name'],
  ...
  [ENTITY_TYPE_CONTAINER_NOTE]: ['attributes_configuration', 'platform_entity_files_ref', 'platform_hidden_type', 'workflow_configuration', 'sync_workflow_status_by_name'],
  [ENTITY_TYPE_CONTAINER_OPINION]: ['attributes_configuration', 'platform_entity_files_ref', 'platform_hidden_type', 'workflow_configuration', 'sync_workflow_status_by_name'],
  [ENTITY_TYPE_CONTAINER_CASE]: ['attributes_configuration', 'platform_entity_files_ref', 'platform_hidden_type', 'workflow_configuration', 'sync_workflow_status_by_name'],
  [ENTITY_TYPE_CONTAINER_TASK]: ['attributes_configuration', 'platform_entity_files_ref', 'platform_hidden_type', 'workflow_configuration', 'sync_workflow_status_by_name'],
...
const getStaticDefaultValues = (input: any, entitySetting: BasicStoreEntityEntitySetting) => {
  const staticDefaultValues = new Map();
  if (!entitySetting) {
  ...
  attributesWithDefaultNullValues.forEach((attr) => {
    if (input[attr.name] === undefined || input[attr.name] === null) {
      staticDefaultValues.set(attr.name, attr.defaultValue);
    }
  });
  return staticDefaultValues;
};
