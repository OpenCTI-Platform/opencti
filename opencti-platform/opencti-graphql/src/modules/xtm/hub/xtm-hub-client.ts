...
export const xtmHubClient = {
  refreshRegistrationStatus: async ({ platformId, token, platformVersion }: { platformId: string, token: string, platformVersion: string }): Promise<RegistrationStatus> => {
    const mutation = `
      mutation RefreshPlatformRegistrationConnectivityStatus($input: PlatformRegistrationConnectivityStatusInput!) {
    ...
    try {
      const response = await httpClient.post('/graphql-api', { mutation, variables });
