...
import {
  ...
  NEWS_FEED_NUMBER,
  type BasicStoreEntityNewsFeedItem,
  type StoreEntityNewsFeedItem,
...
export const myUnreadNewsFeedsCount = (
  context: AuthContext,
  user: AuthUser,
  userId?: string | null,
) => {
...
export const myNewsFeedsFind = (
  context: AuthContext,
  user: AuthUser,
  opts: QueryMyNewsFeedsArgs,
) => {
  const queryFilters = addFilter(opts.filters, 'user_id', user.id);
  const queryArgs = { ...opts, filters: queryFilters };
  return pageEntitiesConnection<BasicStoreEntityNewsFeedItem>(
    context,
    user,
    [ENTITY_TYPE_NEWS_FEED_ITEM],
    queryArgs,
...
export const addNewsFeed = async (
  ...
  const created = await createInternalObject<StoreEntityNewsFeedItem>(
    context,
    user,
    newsFeedToCreate,
    ENTITY_TYPE_NEWS_FEED_ITEM,
  ...
  await notify(
    BUS_TOPICS[NEWS_FEED_NUMBER].EDIT_TOPIC,
    { count: unreadNewsFeedsCount, user_id: created.user_id },
    user,
  );
  return created;
};

const findNewsFeedByExternalId = async (
  context: AuthContext,
  user: AuthUser,
  userId: string,
  ...
  const existing = await fullEntitiesList<BasicStoreEntityNewsFeedItem>(
    context,
    user,
    [ENTITY_TYPE_NEWS_FEED_ITEM],
...
export const upsertNewsFeed = async (
  context: AuthContext,
  user: AuthUser,
  input: NewsFeedAddInput,
) => {
  const existing = await findNewsFeedByExternalId(
    context,
    user,
    input.user_id,
    input.news_feed_item_id,
  );
  if (!existing) {
  ...
  const { element } = await patchAttribute<StoreEntityNewsFeedItem>(
    context,
    user,
    existing.id,
    ENTITY_TYPE_NEWS_FEED_ITEM,
...
export const deleteNewsFeedItemsByExternalId = async (
  context: AuthContext,
  user: AuthUser,
  newsFeedItemId: string,
): Promise<number> => {
  ...
  const toDelete = await fullEntitiesList<BasicStoreEntityNewsFeedItem>(
    context,
    user,
    [ENTITY_TYPE_NEWS_FEED_ITEM],
    { filters: queryFilters },
...
export const markAllNewsFeedItemsAsRead = async (
  context: AuthContext,
  user: AuthUser,
): Promise<boolean> => {
  const queryFilters = {
  ...
  const unreadItems = await fullEntitiesList<BasicStoreEntityNewsFeedItem>(
    context,
    user,
    [ENTITY_TYPE_NEWS_FEED_ITEM],
    { filters: queryFilters },
  ...
  await promiseMap(
    unreadItems,
    (item) => patchAttribute(context, user, item.id, ENTITY_TYPE_NEWS_FEED_ITEM, { is_read: true }),
    5,
  );
  const remainingUnreadCount = await myUnreadNewsFeedsCount(context, user);
  await notify(
    BUS_TOPICS[NEWS_FEED_NUMBER].EDIT_TOPIC,
    { count: remainingUnreadCount, user_id: user.id },
    user,
  );
  return true;
};
...
export const cleanOldNewsFeedItems = async (
  ...
  while (true) {
    ...
    await promiseMap(
      ...
      async (edge: any) => {
        try {
          await deleteElementById(context, user, edge.node.internal_id, ENTITY_TYPE_NEWS_FEED_ITEM);
          totalDeleted += 1;
        } catch (err: any) {
