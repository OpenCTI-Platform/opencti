...
export const findById: DomainFindById<BasicStoreEntityDataComponent> = (
  context: AuthContext,
  user: AuthUser,
...
export const findDataComponentPaginated = (
  context: AuthContext,
  user: AuthUser,
  opts: QueryDataComponentsArgs,
) => {
  return pageEntitiesConnection<BasicStoreEntityDataComponent>(
    context,
    user,
    [ENTITY_TYPE_DATA_COMPONENT],
    opts,
...
export const dataComponentAdd = async (
  context: AuthContext,
  user: AuthUser,
  dataComponent: DataComponentAddInput,
) => {
  const created = await createEntity(context, user, dataComponent, ENTITY_TYPE_DATA_COMPONENT);
  return notify(BUS_TOPICS[ABSTRACT_STIX_DOMAIN_OBJECT].ADDED_TOPIC, created, user);
};

export const withDataSource = async <T extends BasicStoreEntity>(
  context: AuthContext,
  user: AuthUser,
  dataComponentId: string,
) => {
...
export const attackPatternsPaginated = async <T extends BasicStoreEntity>(
  context: AuthContext,
  user: AuthUser,
  dataComponentId: string,
