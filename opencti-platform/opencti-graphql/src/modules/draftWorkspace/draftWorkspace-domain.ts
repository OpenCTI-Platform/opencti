...
import {
  elDeleteDraftContextFromUsers,
  elDeleteDraftContextFromWorks,
  elDeleteDraftElements,
  resolveDraftUpdateFiles,
} from '../../database/draft-engine';
...
const resolveWorkflowInstanceStatusFilter = async (
  context: AuthContext,
  user: AuthUser,
  args: any,
): Promise<any> => {
  ...
  const workflowStatusFilters =
    filters.filters?.filter((f: any) => f.key?.includes(WORKFLOW_INSTANCE_STATUS_FILTER)) ?? [];
  ...
  const statusTemplateIds: string[] = workflowStatusFilters.flatMap(
    (f: any) => f.values as string[],
  );
  const executionCtx = bypassDraftContext(context);

  const workflowInstances = await fullEntitiesList(
    executionCtx,
    executionCtx.user!,
  ...
  const entityIds = workflowInstances.map((wi: any) => wi.entity_id as string).filter(Boolean);

  const remainingFilters = filters.filters.filter(
    (f: any) => !f.key?.includes(WORKFLOW_INSTANCE_STATUS_FILTER),
  );
  const idFilter = {
    key: ['id'],
    values: entityIds.length > 0 ? entityIds : ['<no-match>'],
    operator: FilterOperator.Eq,
    mode: FilterMode.Or,
  };
...
export const checkAndReturnDraft = async (
  context: AuthContext,
  user: AuthUser,
  draftId: string,
) => {
...
export const findById = (context: AuthContext, user: AuthUser, id: string) => {
  return storeLoadById<BasicStoreEntityDraftWorkspace>(
    context,
    user,
    id,
    ENTITY_TYPE_DRAFT_WORKSPACE,
...
const resolveSortByWorkflowInstance = async (
  context: AuthContext,
  user: AuthUser,
  ...
  const allWorkflowInstances: any[] = await fullEntitiesList(
    executionCtx,
    executionCtx.user!,
    [ENTITY_TYPE_WORKFLOW_INSTANCE],
    { first: 5000 },
  ...
  if (statusTemplateIds.length > 0) {
    const statusTemplates = (await elFindByIds(
      executionCtx,
      executionCtx.user!,
      statusTemplateIds,
      { type: ENTITY_TYPE_STATUS_TEMPLATE },
    )) as any[];
  ...
  const allDrafts: BasicStoreEntityDraftWorkspace[] = await fullEntitiesList(
    context,
    user,
    [ENTITY_TYPE_DRAFT_WORKSPACE],
    {
  ...
  return buildPagination<BasicStoreEntityDraftWorkspace>(
    first,
    args.after ?? null,
    instances,
    sorted.length,
...
const resolveSortByRefUsers = async (
  ...
  const allUsers: AuthUser[] = await getEntitiesListFromCache<AuthUser>(
    executionCtx,
    executionCtx.user!,
    ENTITY_TYPE_USER,
  );
...
export const findDraftWorkspacePaginated = async (
  context: AuthContext,
  user: AuthUser,
  args: QueryDraftWorkspacesArgs,
) => {
  ...
  const resolvedArgs = await resolveWorkflowInstanceStatusFilter(context, user, args);
  return pageEntitiesConnection<BasicStoreEntityDraftWorkspace>(
    context,
    user,
    [ENTITY_TYPE_DRAFT_WORKSPACE],
    resolvedArgs,
...
export const draftWorkspacesNumber = async (context: AuthContext, user: AuthUser, args: any) => {
  ...
  const [count, total] = await Promise.all([
    elCount(context, user, READ_INDEX_INTERNAL_OBJECTS, {
      ...resolvedArgs,
      types: [ENTITY_TYPE_DRAFT_WORKSPACE],
    }),
    elCount(context, user, READ_INDEX_INTERNAL_OBJECTS, {
      ...resolvedArgs,
      endDate: undefined,
...
export const draftWorkspacesTimeSeries = async (
  context: AuthContext,
  user: AuthUser,
  args: any,
) => {
...
const resolveWorkflowInstanceDistribution = async (
  context: AuthContext,
  user: AuthUser,
  args: any,
): Promise<{ label: string; value: number; entity: null }[]> => {
  const executionCtx = bypassDraftContext(context);
  ...
  for (const draft of allDrafts) {
    ...
    const statusName = statusTemplateId
      ? (statusNameMap.get(statusTemplateId) ?? statusTemplateId)
      : 'Unknown';
...
export const draftWorkspacesDistribution = async (
  context: AuthContext,
  user: AuthUser,
  args: any,
) => {
...
export const findDraftWorkspaceRestrictedPaginated = (
  context: AuthContext,
  user: AuthUser,
  args: QueryDraftWorkspacesArgs,
) => {
...
export const getObjectsCount = async (
  ...
  const distributionResult = await elAggregationCount(
    draftContext,
    user,
    READ_INDEX_DRAFT_OBJECTS,
    opts,
  ...
  const totalCount = computeSumOfList(
    distributionResult.map((r: { label: string; count: number }) => r.count),
  );
  const entitiesCount = computeSumOfList(
    distributionResult
      .filter(
        (r: { label: string }) =>
          isStixDomainObject(r.label) && !isStixDomainObjectContainer(r.label),
      )
      .map((r: { count: number }) => r.count),
  );
  const observablesCount = computeSumOfList(
    distributionResult
      .filter((r: { label: string }) => isStixCyberObservable(r.label))
      .map((r: { count: number }) => r.count),
  );
  const relationshipsCount = computeSumOfList(
    distributionResult
      .filter((r: { label: string }) => isStixCoreRelationship(r.label))
      .map((r: { count: number }) => r.count),
  );
  const sightingsCount = computeSumOfList(
    distributionResult
      .filter((r: { label: string }) => isStixSightingRelationship(r.label))
      .map((r: { count: number }) => r.count),
  );
  const containersCount = computeSumOfList(
    distributionResult
      .filter((r: { label: string }) => isStixDomainObjectContainer(r.label))
      .map((r: { count: number }) => r.count),
  );
...
export const getProcessingCount = async (
  context: AuthContext,
  user: AuthUser,
  draft: BasicStoreEntityDraftWorkspace,
) => {
  ...
  const draftIncompleteTasksCount = await elCount(
    context,
    user,
    READ_INDEX_INTERNAL_OBJECTS,
    tasksOpts,
...
export const listDraftObjects = async (
  context: AuthContext,
  user: AuthUser,
  args: QueryDraftWorkspaceEntitiesArgs,
) => {
  let types: string[] = [];
  const { draftId, draftOperation, ...listArgs } = args as QueryDraftWorkspaceEntitiesArgs & {
...
export const listDraftRelations = async (
  context: AuthContext,
  user: AuthUser,
  args: QueryDraftWorkspaceRelationshipsArgs,
) => {
  let types: string[] = [];
  ...
  const newArgs: EntityOptions<BasicStoreRelation> = {
    ...listArgs,
    types,
    indices: [READ_INDEX_DRAFT_OBJECTS],
    includeDeletedInDraft: true,
...
export const listDraftSightingRelations = async (
  context: AuthContext,
  user: AuthUser,
  args: QueryDraftWorkspaceSightingRelationshipsArgs,
) => {
  let types: string[] = [];
  const { draftId, ...listArgs } = args;
  await checkAndReturnDraft(context, user, draftId);
...
export const addDraftWorkspace = async (
  context: AuthContext,
  user: AuthUser,
  input: DraftWorkspaceAddInput,
) => {
  const { bypassMandatoryAttributes = false, ...inputWithBypassedMandatoryAttributes } =
  ...
  if (createdDraftWorkspace && inputWithBypassedMandatoryAttributes.entity_id) {
    ...
    const draftInEntity = await elLoadById(
      contextInDraft,
      user,
      inputWithBypassedMandatoryAttributes.entity_id,
    );
    if (draftInEntity) {
...
export const draftWorkspaceAddRelation = async (
  context: AuthContext,
  user: AuthUser,
  draftId: string,
  ...
  return notify(BUS_TOPICS[ENTITY_TYPE_DRAFT_WORKSPACE].EDIT_TOPIC, draft, executionUser).then(
    () => relationData,
  );
};

export const draftWorkspaceDeleteRelation = async (
  context: AuthContext,
  user: AuthUser,
  draftId: string,
  toId: string,
...
export const draftWorkspaceEditField = async (
  context: AuthContext,
  user: AuthUser,
  draftId: string,
  input: EditInput[],
...
const findAllUsersWithDraftContext = async (
  context: AuthContext,
  user: AuthUser,
  draftId: string,
) => {
  const listArgs = {
    ...
    filters: {
      mode: FilterMode.And,
      filters: [{ key: ['draft_context'], values: [draftId] }],
      filterGroups: [],
    },
  };
...
const deleteDraftContextFromUsers = async (
  context: AuthContext,
  user: AuthUser,
  draftId: string,
) => {
...
const findAllWorksWithDraftContext = async (
  context: AuthContext,
  user: AuthUser,
  draftId: string,
) => {
...
const deleteDraftContextFromWorks = async (
  context: AuthContext,
  user: AuthUser,
  draftId: string,
) => {
...
export const deleteDraftWorkspace = async (context: AuthContext, user: AuthUser, id: string) => {
  if (getDraftContext(context, user))
    throw UnsupportedError('Cannot delete draft while in draft context');
  ...
  const deleted = await deleteElementById<StoreEntityDraftWorkspace>(
    context,
    user,
    id,
    ENTITY_TYPE_DRAFT_WORKSPACE,
...
export const buildDraftValidationBundle = async (
  context: AuthContext,
  user: AuthUser,
  draft_id: string,
) => {
  ...
  const draftEntities = await elList(
    contextInDraft,
    user,
    READ_INDEX_DRAFT_OBJECTS,
    includeDeleteOption,
  ...
  const draftEntitiesMinusRefRel = draftEntities.filter(
    (e) => !isStixRefRelationship(e.entity_type) && e.entity_type !== ENTITY_TYPE_INTERNAL_FILE,
  );

  // We add all created elements as stix objects to the bundle
  const createEntities = draftEntitiesMinusRefRel.filter(
    (e) => e.draft_change?.draft_operation === DRAFT_OPERATION_CREATE,
  );
  ...
  const deletedEntities = draftEntitiesMinusRefRel.filter(
    (e) => e.draft_change?.draft_operation === DRAFT_OPERATION_DELETE,
  );
  const deleteEntitiesIds = deletedEntities.map((e) => e.internal_id);
  const deleteStixEntities = await stixLoadByIds(
    contextInDraft,
    user,
    deleteEntitiesIds,
    includeDeleteOption,
  ...
  const updateEntities = draftEntitiesMinusRefRel.filter(
    (e) =>
      e.draft_change?.draft_operation === DRAFT_OPERATION_UPDATE &&
      e.draft_change.draft_updates_patch,
  );
  ...
  const updateStixEntitiesWithPatchPromises = updateStixEntities.map(async (d: any) => {
    ...
    if (draftEntity) {
      ...
      stixPayloadReadyForExport = await resolveEmbeddedImagesInDescriptionFieldsForExport(
        contextInDraft,
        d,
        {
    ...
    const updateFieldPatchFromDraftChange = buildUpdateFieldPatch(
      draftEntity?.draft_change?.draft_updates_patch as string,
    );
    // Resolve patch values (files and markdown) so import can apply them out of draft.
    const updateFieldPatchReadyForImport = await resolveDraftUpdateFiles(
      contextInDraft,
      user,
      updateFieldPatchFromDraftChange,
      {
        entityType: draftEntity?.entity_type,
        entityId: draftEntity?.internal_id,
      },
  ...
  return buildStixBundle([
    ...createStixEntities,
    ...deleteStixEntitiesModified,
    ...updateStixEntitiesWithPatch,
  ]);
};

export const validateDraftWorkspace = async (
  context: AuthContext,
  user: AuthUser,
  draft_id: string,
) => {
  ...
  if (draftWorkspace.draft_status !== DRAFT_STATUS_OPEN) {
    throw FunctionalError('Draft workspace cannot be validated in this state', {
      draftId: draft_id,
      status: draftWorkspace.draft_status,
    });
  }
  ...
  const draftValidationInput = [
    { key: 'draft_status', value: [DRAFT_STATUS_VALIDATED] },
    { key: 'validation_work_id', value: [work.id] },
  ];
...
export const listDraftContainerObjects = async (
  context: AuthContext,
  user: AuthUser,
  args: { draftId: string; containerId: string },
) => {
...
export const draftWorkspaceEditContext = async (
  context: AuthContext,
  user: AuthUser,
  draftId: string,
  input?: EditContext,
...
export const resolveIdRepresentatives = async (
  ...
  if (ids.length > MAX_RESOLVE_IDS) {
    throw FunctionalError(`Too many IDs to resolve (max ${MAX_RESOLVE_IDS})`, {
      count: ids.length,
    });
  }
  await checkAndReturnDraft(context, user, draftId);
  const draftContext = { ...context, draft_context: draftId };
  const entities = (await elFindByIds<BasicStoreCommon>(
    draftContext,
    user,
    ids,
  )) as BasicStoreCommon[];
...
export const getEntityRelations = async (
  ...
  const directRelations = await fullRelationsList<BasicStoreRelation>(
    draftContext,
    user,
    [ABSTRACT_STIX_CORE_RELATIONSHIP, STIX_SIGHTING_RELATIONSHIP],
    {
      fromOrToId: entityId,
      indices: [READ_INDEX_DRAFT_OBJECTS],
      includeDeletedInDraft: true,
  ...
  const objectRefs = await fullRelationsList<BasicStoreRelation>(
    draftContext,
    user,
    RELATION_OBJECT,
    {
  ...
  if (relevantRefs.length > 0) {
    ...
    for (const ref of relevantRefs) {
      ...
      if (!target) continue;
      if (
        !isStixCoreRelationship(target.entity_type) &&
        !isStixSightingRelationship(target.entity_type)
      )
        continue;
  ...
  const directResults = filteredDirect.map((rel) => ({
    ...
    to_name: rel.toName,
    draft_operation:
      rel.draft_change!.draft_operation === DRAFT_OPERATION_CREATE
        ? DraftChangeType.Create
  ...
  const containerResults = containerRelations.map(({ rel, refOp }) => ({
    ...
    draft_operation:
      refOp === DRAFT_OPERATION_CREATE ? DraftChangeType.Add : DraftChangeType.Remove,
  }));
...
export const getEntityFields = async (
  ...
  await checkAndReturnDraft(context, user, draftId);
  const draftContext = { ...context, draft_context: draftId };
  const entities = (await elFindByIds<BasicStoreCommon>(draftContext, user, [entityId], {
    includeDeletedInDraft: true,
  })) as BasicStoreCommon[];
