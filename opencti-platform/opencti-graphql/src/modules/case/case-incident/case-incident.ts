...
import {
  convertCaseIncidentToStix_2_0,
  convertCaseIncidentToStix_2_1,
...
const CASE_INCIDENT_DEFINITION: ModuleDefinition<
  StoreEntityCaseIncident,
  StixCaseIncident,
  Stix2CaseIncident
> = {
  type: {
  ...
  identifier: {
    definition: {
      [ENTITY_TYPE_CONTAINER_CASE_INCIDENT]: [
        { src: NAME_FIELD },
        { src: 'created', dependencies: [NAME_FIELD] },
      ],
    },
    resolvers: {
      name(data: object) {
  ...
  attributes: [
    ...
    },
    {
      ...
      type: 'string',
      format: 'vocabulary',
      vocabularyCategory: 'incident_response_types_ov',
      mandatoryType: 'customizable',
      editDefault: true,
      multiple: true,
      upsert: true,
