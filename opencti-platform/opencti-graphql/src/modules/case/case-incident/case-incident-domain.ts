...
export const findById: DomainFindById<BasicStoreEntityCaseIncident> = (
  context: AuthContext,
  user: AuthUser,
...
export const findCaseIncidentPaginated = (
  context: AuthContext,
  user: AuthUser,
  opts: EntityOptions<BasicStoreEntityCaseIncident>,
) => {
  return pageEntitiesConnection<BasicStoreEntityCaseIncident>(
    context,
    user,
...
export const addCaseIncident = async (
  context: AuthContext,
  user: AuthUser,
  caseIncidentAdd: CaseIncidentAddInput,
) => {
  let caseToCreate = caseIncidentAdd.created
    ? caseIncidentAdd
    : { ...caseIncidentAdd, created: now() };
  ...
  if (isFeatureEnabled(CUSTOM_FIELDS_FEATURE_FLAG)) {
    ...
    const customFieldValues: CustomFieldValue[] = (caseIncidentAdd.customFieldValues ?? []).map(
      (v) => ({
        field_id: v.field_id,
...
export const caseIncidentContainsStixObjectOrStixRelationship = async (
  context: AuthContext,
  user: AuthUser,
  caseIncidentId: string,
  thingId: string,
