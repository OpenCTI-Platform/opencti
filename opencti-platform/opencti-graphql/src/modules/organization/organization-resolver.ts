...
const organizationResolvers: Resolvers = {
  Query: {
    ...
    securityOrganizations: (_, args, context) =>
      findAllSecurityOrganizations(context, context.user, args),
  },
  Organization: {
    sectors: (organization, args, context) =>
      organizationSectorsPaginated<any>(context, context.user, organization.id, args),
    members: (organization, args, context) =>
      organizationMembersPaginated<any>(context, context.user, organization.id, args),
    subOrganizations: (organization, args, context) =>
      childOrganizationsPaginated<BasicStoreEntityOrganization>(
        context,
        context.user,
        organization.id,
        args,
      ),
    parentOrganizations: (organization, args, context) =>
      parentOrganizationsPaginated<BasicStoreEntityOrganization>(
        context,
        context.user,
        organization.id,
        args,
    ...
    default_dashboard: (current, _, context) =>
      context.batch.idsBatchLoader.load({
        id: current.default_dashboard,
        type: ENTITY_TYPE_WORKSPACE,
      }),
    grantable_groups: (organization, _, context) =>
      findGrantableGroups(context, context.user, organization),
    authorized_members: (organization, _, context) =>
      getAuthorizedMembers(context, context.user, organization),
    currentUserAccessRight: (organization, _, context) =>
      getUserAccessRight(context.user, organization),
  },
  User: {
    administrated_organizations: (user, _, context) =>
      buildAdministratedOrganizations(context, context.user, user),
  },
  Mutation: {
    ...
    organizationContextPatch: (_, { id, input }, context) =>
      stixDomainObjectEditContext(context, context.user, id, input),
    organizationContextClean: (_, { id }, context) =>
      stixDomainObjectCleanContext(context, context.user, id),
    organizationRelationAdd: (_, { id, input }, context) =>
