...
const buildStixReportForExport = (
  workspace: BasicStoreEntityWorkspace,
  investigatedEntities: StoreEntity[],
): StixObject => {
  const id = generateStandardId(ENTITY_TYPE_CONTAINER_REPORT, {
    name: workspace.name,
...
export const toStixReportBundle = async (
  context: AuthContext,
  user: AuthUser,
  workspace: BasicStoreEntityWorkspace,
): Promise<string> => {
  ...
  const storeInvestigatedEntities = await storeLoadByIdsWithRefs<StoreEntity>(
    context,
    user,
    investigatedEntitiesIds,
    { indices: READ_STIX_INDICES },
  );
  const stixReportForExport = buildStixReportForExport(workspace, storeInvestigatedEntities);
  const bundle = buildStixBundle([
    stixReportForExport,
    ...storeInvestigatedEntities.map((s) => convertStoreToStix_2_1(s)),
  ]);
  return JSON.stringify(bundle);
};

export const investigationAddFromContainer = async (
  context: AuthContext,
  user: AuthUser,
  containerId: string,
) => {
  ...
  const filteredOutInvestigatedIds = await filterUnwantedEntitiesOut({
    context,
    user,
    ids: container.object,
  });

  const investigationInput = {
    type: 'investigation',
    name: investigationToStartCanonicalName,
    investigated_entities_ids: filteredOutInvestigatedIds,
  };
