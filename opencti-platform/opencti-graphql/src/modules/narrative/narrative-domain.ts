...
export const findById = (
  ...
  ) as unknown as BasicStoreEntityNarrative;
};

export const findNarrativePaginated = (
  context: AuthContext,
  user: AuthUser,
  opts: QueryNarrativesArgs,
) => {
  return pageEntitiesConnection<BasicStoreEntityNarrative>(
    context,
    user,
    [ENTITY_TYPE_NARRATIVE],
    opts,
...
export const addNarrative = async (
  context: AuthContext,
  user: AuthUser,
  narrative: NarrativeAddInput,
) => {
  const created = await createEntity(context, user, narrative, ENTITY_TYPE_NARRATIVE);
  return notify(BUS_TOPICS[ABSTRACT_STIX_DOMAIN_OBJECT].ADDED_TOPIC, created, user);
};

export const parentNarrativesPaginated = async <T extends BasicStoreEntity>(
  context: AuthContext,
  user: AuthUser,
  narrativeId: string,
...
export const childNarrativesPaginated = async <T extends BasicStoreEntity>(
  context: AuthContext,
  user: AuthUser,
  narrativeId: string,
  args: EntityOptions<BasicStoreCommon>,
