...
const BULK_LINE_PARSING_NUMBER =
  conf.get('import_csv_built_in_connector:bulk_creation_size') || 5000;
...
const processCSVforWorkbench = async (
  context: AuthContext,
  fileId: string,
  opts: CsvBundlerIngestionOpts,
) => {
  const { workId, applicantUser, csvMapper, entity } = opts;
  const stream: SdkStream<Readable> | null | undefined = (await downloadFile(fileId)) as
    | SdkStream<Readable>
    | null
    | undefined;
  if (stream) {
  ...
  await updateProcessedTime(
    context,
    applicantUser,
    workId,
    '1 generated bundle for workbench validation.',
  );
};

export const processCSVforWorkers = async (
  context: AuthContext,
  fileId: string,
  opts: CsvBundlerIngestionOpts,
) => {
  ...
  while (hasMoreBulk) {
    ...
    if (stream) {
      ...
      try {
        ...
        for await (const line of readStream) {
          if (
            ...
            lineNumber < startingLineNumber + BULK_LINE_PARSING_NUMBER
          ) {
        ...
        logApp.info(
          `${LOG_PREFIX} read lines end on ${new Date().getTime() - startDate} ms; hasMoreBulk=${hasMoreBulk}; lineNumber=${lineNumber}`,
        );
        ...
        if (lines.length > 0) {
          try {
            ...
            const errorData = {
              error: error.message,
              source: `${fileId}, from ${lineNumber} and ${BULK_LINE_PARSING_NUMBER} following lines.`,
            };
      ...
      hasMoreBulk = false;
      logApp.error(
        `${LOG_PREFIX} Cannot download file, please check the file storage dependency.`,
        { fileId, workId },
      );
  ...
  logApp.info(
    `${LOG_PREFIX} processing CSV ${fileId} DONE in ${new Date().getTime() - startDate2} ms for ${totalObjectsCount} objets in ${totalBundlesCount} bundles.`,
  );
  ...
  if (totalBundlesCount > 0) {
    await updateProcessedTime(
      ...
      applicantUser,
      workId,
      `${totalBundlesCount} bundle(s) send to worker for import.`,
    );
    ...
    await updateProcessedTime(
      ...
      applicantUser,
      workId,
      'No bundle send to worker for import.',
    );
...
const processValidateBeforeImport = async (
  context: AuthContext,
  ...
  if (draftId) {
    ...
    const { id } = await addDraftWorkspace(context, opts.applicantUser, {
      name: fileId,
...
const consumeQueueCallback = async (context: AuthContext, message: string) => {
  ...
  const entity = entityId
    ? await storeLoadByIdWithRefs(context, applicantUser, entityId)
    : undefined;
  ...
  try {
    ...
    await updateReceivedTime(
      context,
      applicantUser,
      workId,
      'Connector ready to process the operation',
    );
