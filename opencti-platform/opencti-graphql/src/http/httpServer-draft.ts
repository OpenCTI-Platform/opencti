...
import {
  ...
  type BasicStoreEntityDraftWorkspace,
} from '../modules/draftWorkspace/draftWorkspace-types';
...
export const checkDraftInContext = async (executeContext: AuthContext) => {
  ...
  if (executeContext.draft_context) {
    if (executeContext.user) {
      ...
      const draftWorkspace: BasicStoreEntityDraftWorkspace = draftWorkspaces.get(
        executeContext.draft_context,
      ) as BasicStoreEntityDraftWorkspace;
      ...
      const isUserCanAccess = await isUserCanAccessStoreElement(
        executeContext,
        executeContext.user,
        draftWorkspace,
      );
      if (!isUserCanAccess) {
        ...
        const serviceAccountHint =
          executeContext.user.user_service_account === true
            ? ''
            : ', consider switching the user associated to your connector to a service account (instead of a user)';
        throw FunctionalError(
          `Draft ${executeContext.draft_context} cannot be found${serviceAccountHint}`,
        );
      }
