...
const ruleObserveSightingBuilder = (): RuleRuntime => {
  ...
  const handleIndicatorUpsert = async (
    ...
    const baseOnRelations = await fullRelationsList<BasicStoreRelation>(
      context,
      RULE_MANAGER_USER,
      RELATION_BASED_ON,
      baseOnArgs,
    ...
    for (let index = 0; index < baseOnRelations.length; index += 1) {
      ...
      const objectsRelations = await fullRelationsList<BasicStoreRelation>(
        context,
        RULE_MANAGER_USER,
        RELATION_OBJECT,
        objectsArgs,
      ...
      for (let objectIndex = 0; objectIndex < objectsRelations.length; objectIndex += 1) {
        ...
        const observedData = (await internalLoadById(
          context,
          RULE_MANAGER_USER,
          observedDataId,
        ...
        if (organizationId) {
          const organization = (await internalLoadById(
            context,
            RULE_MANAGER_USER,
            organizationId,
          )) as unknown as BasicStoreEntity;
    ...
    if (organization) {
      ...
      for (let objectIndex = 0; objectIndex < objectsRelations.length; objectIndex += 1) {
        ...
        for (let index = 0; index < baseOnRelations.length; index += 1) {
          ...
          const indicator = (await internalLoadById(
            context,
            RULE_MANAGER_USER,
            indicatorId,
          )) as unknown as BasicStoreEntity;
  ...
  const handleObservableRelationUpsert = async (
    ...
    const baseOnIndicator = (await stixLoadById(
      context,
      RULE_MANAGER_USER,
      indicatorId,
    )) as unknown as StixIndicator;
    if (baseOnIndicator) {
