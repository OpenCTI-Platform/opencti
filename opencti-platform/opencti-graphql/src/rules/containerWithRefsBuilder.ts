...
import { objects, RELATION_OBJECT } from '../schema/stixRefRelationship';
...
const buildContainerRefsRule = (
  ...
  const typeRefFilter = (ref: string) => {
    ...
    return isSource
      ? internalTypes.includes(relationTypes.leftType)
      : internalTypes.includes(relationTypes.rightType);
  };
  const generateDependencies = (
    reportId: string,
    partOfFromId: string,
    partOfId: string,
    partOfTargetId: string,
  ) => {
    return [
      reportId,
      partOfFromId,
  ...
  type ArrayRefs = Array<{
    partOfFromId: string;
    partOfId: string;
    partOfStandardId: StixId;
    partOfTargetId: string;
    partOfTargetStandardId: StixId;
  }>;
  ...
  const createObjectRefsInferences = async (
    ...
    for (let index = 0; index < addedTargets.length; index += 1) {
      const { partOfFromId, partOfId, partOfStandardId, partOfTargetId, partOfTargetStandardId } =
        addedTargets[index];
      ...
      if (!reportObjectRefIds.includes(partOfStandardId)) {
        ...
        const inputForRelation = {
          fromId: reportId,
          toId: partOfId,
          relationship_type: RELATION_OBJECT,
        };
        const inferredRelation = (await createInferredRelation(
          context,
          inputForRelation,
          ruleRelationContent,
          opts,
      ...
      if (!reportObjectRefIds.includes(partOfTargetStandardId)) {
        const ruleIdentityContent = createRuleContent(
          id,
          dependencies,
          isSource ? [reportId, partOfTargetId] : [reportId, partOfFromId],
          {},
        );
        const inputForIdentity = {
          fromId: reportId,
          toId: isSource ? partOfTargetId : partOfFromId,
          relationship_type: RELATION_OBJECT,
        };
        const inferredTarget = (await createInferredRelation(
          context,
          inputForIdentity,
          ruleIdentityContent,
          opts,
    ...
    for (let indexDeletion = 0; indexDeletion < deletedTargets.length; indexDeletion += 1) {
      ...
      if (isDeletion) {
        ...
        const deletedTarget = (await internalLoadById(
          context,
          RULE_MANAGER_USER,
          inferenceToDelete.toId,
    ...
    if (createdTargets.length > 0 || deletedTargetRefs.length > 0) {
      ...
      const refsWithoutDeletion = (object_refs_inferred ?? []).filter(
        (o) => !deletedTargetIds.includes(o),
      );
      ...
      const changes = await buildChanges(
        context,
        RULE_MANAGER_USER,
        report.extensions[STIX_EXT_OCTI].type,
        inputs,
      );
  ...
  const handleReportCreation = async (
    ...
    if (addedRefs.length > 0) {
      const identities = (await internalFindByIds(
        context,
        RULE_MANAGER_USER,
        addedRefs,
      )) as Array<StoreObject>;
      ...
      const listAddedRefsCallback = async (relationships: Array<BasicStoreRelation>) => {
        if (relationships.length > 0) {
          ...
          const targets = (await internalFindByIds(
            context,
            RULE_MANAGER_USER,
            R.uniq(relationships.map((r) => (isSource ? r.toId : r.fromId))),
          )) as Array<StoreObject>;
          ...
          for (let relIndex = 0; relIndex < relationships.length; relIndex += 1) {
            const {
              internal_id: partOfId,
              standard_id: partOfStandardId,
              fromId: partOfFromId,
              toId: partOfTargetId,
      ...
      if (originIds.length > 0) {
        const listArgs = isSource
          ? { fromId: originIds, toTypes: [relationTypes.rightType] }
          : { toId: originIds, fromTypes: [relationTypes.leftType] };
        const fullListArgs = { ...listArgs, callback: listAddedRefsCallback };
        await fullRelationsList<BasicStoreRelation>(
          context,
          RULE_MANAGER_USER,
          relationTypes.creationType,
          fullListArgs,
        );
      }
    }
    ...
    if (removedRefs.length > 0) {
      const removedRefIdentities = (await internalFindByIds(
        context,
        RULE_MANAGER_USER,
        removedRefs,
      )) as Array<StoreObject>;
      ...
      const args = {
        fromId: report.extensions[STIX_EXT_OCTI].id,
        filters,
        noFiltersChecking: true,
        indices: READ_DATA_INDICES,
        callback: listRemovedRefsCallback,
      };
      await fullRelationsList<BasicStoreRelation>(
        context,
        RULE_MANAGER_USER,
  ...
  const handlePartOfRelationCreation = async (
    ...
    const {
      id: partOfId,
      source_ref: partOfFromId,
      target_ref: partOfTargetId,
    } = partOfRelation.extensions[STIX_EXT_OCTI];
    const listFromCallback = async (relationships: Array<BasicStoreRelation>) => {
      for (let objectRefIndex = 0; objectRefIndex < relationships.length; objectRefIndex += 1) {
        ...
        if (report) {
          const addedRefs = [
            { partOfFromId, partOfId, partOfStandardId, partOfTargetId, partOfTargetStandardId },
          ];
          await createObjectRefsInferences(context, report, addedRefs, []);
        }
      }
    };
    const listReportArgs = {
      fromTypes: [containerType],
      toId: isSource ? partOfFromId : partOfTargetId,
  ...
  const applyUpdate = async (data: StixObject, event: UpdateEvent): Promise<void> => {
    ...
    if (entityType === containerType) {
      ...
      const previousData = jsonpatch.applyPatch<StixReport>(
        structuredClone(report),
        previousPatch,
      ).newDocument;
      const previousRefIds = [
        ...(previousData.extensions[STIX_EXT_OCTI].object_refs_inferred ?? []),
        ...(previousData.object_refs ?? []),
      ];
      const previousRefIdsSet = new Set(previousRefIds);
      const newRefIds = [
        ...(report.extensions[STIX_EXT_OCTI].object_refs_inferred ?? []),
        ...(report.object_refs ?? []),
      ];
      const newRefIdsSet = new Set(newRefIds);
      // AddedRefs are ids not includes in previous data
      const addedRefs: Array<StixId> = await asyncFilter(
        newRefIds,
        (newId) => !previousRefIdsSet.has(newId),
      );
      // RemovedRefs are ids not includes in current data
      const removedRefs: Array<StixId> = await asyncFilter(
        previousRefIds,
        (newId) => !newRefIdsSet.has(newId),
      );
      // Apply operations
  ...
  const update = async (element: StixObject, event: UpdateEvent): Promise<void> =>
    applyUpdate(element, event);
