...
export const isSingleRelationsRef = (entityType: string, databaseName: string): boolean =>
...
export const instanceMetaRefsExtractor = (
  ...
  isInferred: boolean,
  data: BasicStoreObject,
) => {
  const refField =
    isStixRefRelationship(relationshipType) && isInferred ? ID_INFERRED : ID_INTERNAL;
...
const RELATIONS_STIX_ATTRIBUTES = [
  'source_ref',
  'target_ref',
  'sighting_of_ref',
  'where_sighted_refs',
...
const RELATIONS_EMBEDDED_STIX_ATTRIBUTES = [
  externalReferences.stixName,
  killChainPhases.stixName,
...
export const stixRefsExtractor = (data: any) => {
  ...
  const stixNames = schemaRelationsRefDefinition
    .getStixNames(data.extensions[STIX_EXT_OCTI].type)
