...
import { selectSourceActivityByIdQuery, selectIngestProgressByIdQuery } from '../schema/dynamodb/workActivity.js';
...
>>>>>>> origin/develop
  ...
  const activities = [];
  for (const dynamoDBRecord of data.Items) {
    const activity = unmarshall(dynamoDBRecord);
  ...
  for (const activity of activities) {
    ...
    for (const dynamoDBRecord of data.Items) {
      const progress = unmarshall(dynamoDBRecord);
