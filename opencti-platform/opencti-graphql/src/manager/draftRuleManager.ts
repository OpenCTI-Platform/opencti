...
import { createStreamProcessor, EVENT_CURRENT_VERSION, lockResource, REDIS_DRAFT_STREAM_NAME, type StreamProcessor } from '../database/redis';
...
import { DRAFT_RULE_MANAGER_USER, executionContext, RULE_MANAGER_USER } from '../utils/access';
...
const DRAFT_RULE_ENGINE_ID = 'draft_rule_engine_settings';
const DRAFT_RULE_ENGINE_KEY = conf.get('draft_rule_engine:lock_key');
const SCHEDULE_TIME = 10000;

export const getManagerInfo = async (context: AuthContext, user: AuthUser): Promise<RuleManager> => {
  const isRuleEngineActivated = await isModuleActivated('DRAFT_RULE_ENGINE');
  const ruleStatus = await internalLoadById(context, user, DRAFT_RULE_ENGINE_ID) as unknown as BasicManagerEntity;
  return { activated: isRuleEngineActivated, ...ruleStatus };
};

export const buildInternalEvent = (type: 'update' | 'create' | 'delete', stix: StixCoreObject, draftContext: string | undefined): StreamDataEvent => {
  return {
    version: EVENT_CURRENT_VERSION,
    type,
    scope: 'internal',
    message: 'rule internal event',
    origin: DRAFT_RULE_MANAGER_USER,
    data: stix,
    draftContext
  };
};
...
const ruleMergeHandler = async (event: MergeEvent): Promise<Array<BaseEvent>> => {
  const { data, context: eventContext, draftContext } = event;
  const events: Array<BaseEvent> = [];
  // region 01 - Generate events for sources deletion
  // -- sources
  const sourceDeleteEvents = (eventContext.sources || []).map((s) => buildInternalEvent(EVENT_TYPE_DELETE, s, draftContext));
  events.push(...sourceDeleteEvents);
  // endregion
  // region 03 - Generate event for merged entity
  const updateEvent = buildInternalEvent(EVENT_TYPE_UPDATE, data, draftContext) as UpdateEvent;
...
const applyCleanupOnDependencyIds = async (deletionIds: Array<string>, rules: Array<RuleRuntime>, event: DataEvent) => {
  const context = executionContext('draft_rule_cleaner', DRAFT_RULE_MANAGER_USER);
  const filters = {
    mode: FilterMode.And,
  ...
  const callback = async (elements: Array<BasicStoreCommon>) => {
    ...
    await rulesCleanHandler(context, DRAFT_RULE_MANAGER_USER, elements, rules, deletionIds, event);
  };
  const opts = { filters, noFiltersChecking: true, callback };
  await elList(context, DRAFT_RULE_MANAGER_USER, READ_DATA_INDICES, opts);
};
...
export const rulesApplyHandler = async (context: AuthContext, user: AuthUser, events: Array<DataEvent>, forRules: Array<RuleRuntime> = []) => {
  ...
  for (let index = 0; index < events.length; index += 1) {
    ...
    const { type, data, draftContext } = event;
    context.draftId = draftContext;
    try {
      ...
      if (type === EVENT_TYPE_DELETE) {
        ...
        await applyCleanupOnDependencyIds([internalId], rules, event);
      }
      ...
      if (type === EVENT_TYPE_UPDATE) {
        ...
        for (let ruleIndex = 0; ruleIndex < rules.length; ruleIndex += 1) {
          ...
          if (impactDependencies || (isPreviouslyMatched && !isCurrentMatched)) {
            await applyCleanupOnDependencyIds([internalId], [rule], event);
          }
      ...
      try {
        ...
        if (isElementCleanable) {
          ...
          if (processingElement) {
            await rule.clean(processingElement, deletedDependencies, event);
          }
        }
      } catch (err: any) {
...
const ruleStreamHandler = async (streamEvents: Array<SseEvent<DataEvent>>, lastEventId: string) => {
  const context = executionContext('draft_rule_manager', DRAFT_RULE_MANAGER_USER);
  ...
  if (compatibleEvents.length > 0) {
    ...
    // Execute the events
    await rulesApplyHandler(context, DRAFT_RULE_MANAGER_USER, ruleEvents);
  }
  // Save the last processed event
  logApp.debug(`[OPENCTI] Rule manager saving state to ${lastEventId}`);
  context.draftId = '';
  await patchAttribute(context, DRAFT_RULE_MANAGER_USER, DRAFT_RULE_ENGINE_ID, ENTITY_TYPE_RULE_MANAGER, { lastEventId });
};

const getInitRuleManager = async (): Promise<BasicStoreEntity> => {
  const context = executionContext('draft_rule_manager', DRAFT_RULE_MANAGER_USER);
  const ruleSettingsInput = { internal_id: DRAFT_RULE_ENGINE_ID, errors: [] };
  const created = await createEntity(context, DRAFT_RULE_MANAGER_USER, ruleSettingsInput, ENTITY_TYPE_RULE_MANAGER);
...
const initRuleManager = () => {
  ...
  const ruleHandler = async () => {
    ...
    try {
      // Lock the manager
      lock = await lockResource([DRAFT_RULE_ENGINE_KEY], { retryCount: 0 });
      running = true;
      ...
      const opts = { withInternal: true, streamName: REDIS_DRAFT_STREAM_NAME };
      streamProcessor = createStreamProcessor(DRAFT_RULE_MANAGER_USER, 'Draft Rule manager', ruleStreamHandler, opts);
      ...
      if (e.name === TYPE_LOCK_ERROR) {
        ...
        logApp.error(e, { manager: 'DRAFT_RULE_ENGINE' });
      }
    } finally {
      running = false;
  ...
  return {
    ...
    status: () => {
      return {
        id: 'DRAFT_RULE_ENGINE',
        enable: booleanConf('draft_rule_engine:enabled', false),
        running,
      };
    },
    shutdown: async () => {
...
const draftRuleEngine = initRuleManager();
...
export const cleanRuleManager = async (context: AuthContext, user: AuthUser, eventId: string) => {
  ...
  const { element } = await patchAttribute(context, user, DRAFT_RULE_ENGINE_ID, ENTITY_TYPE_RULE_MANAGER, patch);
  // Restart the manager
  await draftRuleEngine.shutdown();
  await draftRuleEngine.start();
  // Return the updated element
  return { activated: isRuleEngineActivated, ...element };
};

export default draftRuleEngine;
