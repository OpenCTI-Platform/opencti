...
import {
  confirmDelete,
  findOldDeleteOperations,
} from '../modules/deleteOperation/deleteOperation-domain';
...
const GARBAGE_COLLECTION_MANAGER_KEY =
  conf.get('garbage_collection_manager:lock_key') || 'garbage_collection_manager_lock';
...
export const garbageCollectionHandler = async () => {
  ...
  const deleteOperationsToManage = await findOldDeleteOperations(
    context,
    GARBAGE_COLLECTION_MANAGER_USER,
    deletedRetentionDaysToConsider,
    BATCH_SIZE,
  );
  let errorCount = 0;
  for (let i = 0; i < deleteOperationsToManage.length; i += 1) {
    try {
      ...
      logApp.error('[OPENCTI-MODULE] Garbage collection delete error', {
        cause: e,
        manager: 'GARBAGE_MANAGER',
        id: deleteOperationsToManage[i].id,
        errorCount,
      });
      errorCount += 1;
    }
  }
  logApp.debug('[OPENCTI-MODULE] Garbage collection manager deletion process complete', {
    count: deleteOperationsToManage.length,
  });
};
