...
} from '../modules/decayRule/decayRule-types';
...
const workflowStatuses = (context: AuthContext) => {
  const reloadStatuses = async () => {
    const templates = await fullEntitiesList<BasicWorkflowTemplateEntity>(context, SYSTEM_USER, [
      ENTITY_TYPE_STATUS_TEMPLATE,
    ]);
    const args: EntityOptions<BasicWorkflowStatusEntity> = {
      orderBy: ['order'],
      orderMode: OrderingMode.Asc,
    };
    const statuses = await fullEntitiesList<BasicWorkflowStatusEntity>(
      context,
      SYSTEM_USER,
      [ENTITY_TYPE_STATUS],
      args,
...
export const extractResolvedFiltersFromInstance = (instance: BasicStoreCommon) => {
  ...
  if (instance.entity_type === ENTITY_TYPE_STREAM_COLLECTION) {
    ...
    const connFilters =
      (instance as BasicStoreEntityConnector).connector_trigger_filters?.length > 0
    ...
    const definition = JSON.parse(
      (instance as BasicStoreEntityPlaybook).playbook_definition,
    ...
    const pirFilterIds = extractFilterGroupValuesToResolveForCache(
      JSON.parse((instance as BasicStoreEntityPir).pir_filters),
    );
    ...
    const decayExclusionRuleIds = extractFilterGroupValuesToResolveForCache(
      JSON.parse((instance as BasicStoreEntityDecayExclusionRule).decay_exclusion_filters),
    );
...
const platformResolvedFilters = (context: AuthContext) => {
  const reloadFilters = async () => {
    ...
    const streams = await fullEntitiesList<BasicStreamEntity>(context, SYSTEM_USER, [
      ENTITY_TYPE_STREAM_COLLECTION,
    ]);
    const triggers = await fullEntitiesList<BasicTriggerEntity>(context, SYSTEM_USER, [
      ENTITY_TYPE_TRIGGER,
    ]);
    const connectors = await fullEntitiesList<BasicStoreEntityConnector>(context, SYSTEM_USER, [
      ENTITY_TYPE_CONNECTOR,
    ]);
    const playbooks = await fullEntitiesList<BasicStoreEntityPlaybook>(context, SYSTEM_USER, [
      ENTITY_TYPE_PLAYBOOK,
    ]);
    const pirs = await fullEntitiesList<BasicStoreEntityPir>(context, SYSTEM_USER, [
      ENTITY_TYPE_PIR,
    ]);
    const decayRules = await fullEntitiesList<BasicStoreEntityDecayRule>(context, SYSTEM_USER, [
      ENTITY_TYPE_DECAY_RULE,
    ]);
    const decayExclusionRules = await fullEntitiesList<BasicStoreEntityDecayExclusionRule>(
      context,
      SYSTEM_USER,
    ...
    const filteringIds = allFilteredEntities
      .map((s) => extractResolvedFiltersFromInstance(s))
      .flat();
    // Resolve the filters ids
    if (filteringIds.length > 0) {
      const resolvingIds = R.uniq(filteringIds);
      const loadedDependencies = (await stixLoadByIds(
        context,
        SYSTEM_USER,
        resolvingIds,
      )) as S.StixObject[];
      return new Map(
        loadedDependencies.map((l: StixObject) => [l.extensions[STIX_EXT_OCTI].id, l]),
      );
    }
    return new Map();
  };
  const refreshFilter = async (values: Map<string, StixObject>, instance: BasicStoreCommon) => {
    ...
    if (
      instance.entity_type === ENTITY_TYPE_LABEL ||
      instance.entity_type === ENTITY_TYPE_MARKING_DEFINITION
    ) {
    ...
    const loadedDependencies = (await stixLoadByIds(
      ...
      SYSTEM_USER,
      R.uniq(idsToSolve),
    )) as S.StixObject[]; // fetch the stix instance of the ids
    // Add resolved stix entities to the cache map
    loadedDependencies.forEach((l: StixObject) =>
      currentFiltersValues.set(l.extensions[STIX_EXT_OCTI].id, l),
    );
...
const platformRunningPlaybooks = (context: AuthContext) => {
  const reloadPlaybooks = () => {
    ...
    return fullEntitiesList<BasicStoreEntityPlaybook>(
      context,
      SYSTEM_USER,
      [ENTITY_TYPE_PLAYBOOK],
      opts,
...
const platformUsers = (context: AuthContext) => {
  const loadUsers = async (ids?: string[]): Promise<AuthUser[]> => {
    const users = ids
      ? await internalFindByIds(context, SYSTEM_USER, ids)
...
const platformSettings = (context: AuthContext) => {
  const reloadSettings = async () => {
    const memberOfRelations = await fullRelationsList<BasicStoreRelation>(context, SYSTEM_USER, [
      RELATION_MEMBER_OF,
      RELATION_PARTICIPATE_TO,
    ]);
    const memberOfGroups = memberOfRelations
      .filter((m) => m.entity_type === RELATION_MEMBER_OF)
      .map((mr) => ({ group: mr.toId, user: mr.fromId }));
    const membersGroupMap = new Map(
      Object.entries(R.groupBy((r) => r.group, memberOfGroups)).map(([k, v]) => [
        k,
        (v || []).map((t) => t.user),
      ]),
    );
    const memberOfOrgs = memberOfRelations
      .filter((m) => m.entity_type === RELATION_PARTICIPATE_TO)
      .map((mr) => ({ organization: mr.toId, user: mr.fromId }));
    const membersOrganizationMap = new Map(
      Object.entries(R.groupBy((r) => r.organization, memberOfOrgs)).map(([k, v]) => [
        k,
        (v || []).map((t) => t.user),
      ]),
    );
    return fullEntitiesList<BasicStoreSettings>(context, SYSTEM_USER, [ENTITY_TYPE_SETTINGS]).then(
      (settings) => {
        return settings.map((s) => {
          const auditListenerIds = s.activity_listeners_ids ?? [];
          const ee_info = getEnterpriseEditionInfo(s);
          const activity_listeners_users = auditListenerIds
            .map((id: string) => membersGroupMap.get(id) ?? membersOrganizationMap.get(id) ?? [id])
            .flat();
          const platform_url = getBaseUrl(context.req);
          return {
            ...s,
            valid_enterprise_edition: ee_info.license_validated,
            activity_listeners_users,
            platform_url,
          };
        });
      },
...
const platformNotifiers = (context: AuthContext) => {
  const reloadNotifiers = async () => {
    const notifiers = await fullEntitiesList<BasicStoreEntityNotifier>(context, SYSTEM_USER, [
...
const platformPublicDashboards = (context: AuthContext) => {
  const reloadPublicDashboards = async () => {
    const publicDashboards = await fullEntitiesList<BasicStoreEntityPublicDashboard>(
      context,
      SYSTEM_USER,
      [ENTITY_TYPE_PUBLIC_DASHBOARD],
    );
...
const initCacheManager = () => {
  ...
  const initCacheContent = () => {
    ...
    writeCacheForEntity(
      ...
      platformCustomFieldDefinitions(context),
    );
  };
  return {
    ...
    start: async () => {
      ...
      subscribeReset = await pubSubSubscription<{ entityType: string }>(
        CACHE_RESET_TOPIC,
        (event) => {
    ...
    shutdown: async () => {
      ...
      logApp.info(
        `[OPENCTI-MODULE] Cache manager stopped in ${new Date().getTime() - startTime} ms`,
      );
      return true;
    },
  };
