...
import {
  findIndicatorsForDecay,
  updateIndicatorDecayScore,
} from '../modules/indicator/indicator-domain';

const INDICATOR_DECAY_MANAGER_ENABLED = booleanConf('indicator_decay_manager:enabled', true);
const INDICATOR_DECAY_MANAGER_KEY =
  conf.get('indicator_decay_manager:lock_key') || 'indicator_decay_manager_lock';
...
export const indicatorDecayHandler = async () => {
  ...
  for (let i = 0; i < indicatorsToUpdate.length; i += 1) {
    try {
      ...
      logApp.error('[OPENCTI-MODULE] Error when processing decay, skipping.', {
        cause: e,
        id: indicatorsToUpdate[i].id,
      });
      errorCount += 1;
    }
  }
  if (errorCount > 0) {
    logApp.error(
      '[OPENCTI-MODULE] Indicator decay manager got errors. Please have a look to previous warning.',
      {
    ...
    logApp.debug('[OPENCTI-MODULE] Indicator decay manager updated', {
      indicators_count: indicatorsToUpdate.length,
    });
  }
};
