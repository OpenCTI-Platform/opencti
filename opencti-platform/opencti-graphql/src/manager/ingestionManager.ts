...
const csvDataHandler = async (context: AuthContext, ingestion: BasicStoreEntityIngestionCsv) => {
  ...
  const csvMapper = ingestion.csv_mapper_type === IngestionCsvMapperType.Inline ? JSON.parse(ingestion.csv_mapper!) : await findById(context, user, ingestion.csv_mapper_id!);
