...
import { createStreamProcessor, lockResource, redisPlaybookUpdate, StreamProcessor } from '../database/redis';
...
const playbookStreamHandler = async (streamEvents: Array<SseEvent<DataEvent>>) => {
  try {
    ...
    const playbooks = await findAllPlaybooks(context, SYSTEM_USER, { connectionFormat: false }); // TODO need to be filter by INTERNAL_DATA_STREAM entry point
...
const initPlaybookManager = () => {
  ...
  return {
    ...
    status: () => {
      return {
        id: 'PLAYBOOK_MANAGER',
        enable: booleanConf('playbook_manager:enabled', false),
        running,
      };
    },
    shutdown: async () => {
