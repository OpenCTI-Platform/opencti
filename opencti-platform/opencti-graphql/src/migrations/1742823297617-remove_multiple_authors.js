...
const message =
  '[MIGRATION] remove multiple authors from entities that have multiple rel_created-by.internal_id';
...
export const up = async (next) => {
  ...
  logMigration.info(
    `[MIGRATION] Found ${multipleAuthorsHits?.length} entities with multiple authors`,
  );
  ...
  for (let index = 0; index < multipleAuthorsHits?.length; index += 1) {
    ...
    const currentAuthorsIds =
      currentEntityWithMultipleAuthors._source['rel_created-by.internal_id'];
    const authorIdToKeep = currentAuthorsIds[currentAuthorsIds.length - 1];
    // 1. Update the denormalized refs of the current entity
    const updateCreatedByWithUniqueIdSource =
      "ctx._source['rel_created-by.internal_id'] = [params.authorIdToKeep]";
    await elUpdate(
      context,
      currentEntityWithMultipleAuthors._index,
      currentEntityWithMultipleAuthors._id,
      {
        script: {
                    ...
                    must: [
                      {
                        term: {
                          'connections.internal_id.keyword': {
                            value: currentEntityWithMultipleAuthors._source.internal_id,
                          },
                        },
                      },
                    ],
    ...
    if ((index + 1) % 10 === 0) {
      logMigration.info(
        `[MIGRATION] Entity ${currentEntityWithMultipleAuthors._id} authors cleaned -- ${index + 1}/${multipleAuthorsHits.length}`,
      );
    }
  }
