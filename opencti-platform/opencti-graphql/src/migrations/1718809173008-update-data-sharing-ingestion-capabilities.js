...
export const up = async (next) => {
  ...
  const callback = async (roles) => {
    for (let i = 0; i < roles.length; i += 1) {
      ...
      const hasAccessDataSharingCapability = capabilities.some(
        (capability) => capability.name.startsWith('SETTINGS') || capability.name === 'TAXIIAPI',
      );
      if (hasAccessDataSharingCapability) {
        const input = {
          fromId: roleId,
          toId: accessIngestionCapability.id,
      ...
      const hasManageDataSharingCapability = capabilities.some(
        (capability) =>
          capability.name.startsWith('SETTINGS') || capability.name === 'TAXIIAPI_SETCOLLECTIONS',
      );
      if (hasManageDataSharingCapability) {
        const input = {
          fromId: roleId,
          toId: manageIngestionCapability.id,
          relationship_type: 'has-capability',
        };
        await createRelation(context, SYSTEM_USER, input);
      }
  ...
  const CSVCapability = await elLoadById(
    context,
    SYSTEM_USER,
    'capability--5407e8a4-0ff9-5253-89b1-c01a92ad9453',
  );
  if (CSVCapability) {
  ...
  const accessDataCapability = await elLoadById(
    context,
    SYSTEM_USER,
    'capability--d258afde-7a8a-5917-8b4b-83119d3f8e52',
  );
  const accessDataCapabilityPatch = { description: 'Access data sharing' };
  await elReplace(context, accessDataCapability._index, accessDataCapability.internal_id, {
    doc: accessDataCapabilityPatch,
  });

  // ------ Update description of Manage data sharing & ingestion => Manage data sharing
  const manageDataCapability = await elLoadById(
    context,
    SYSTEM_USER,
    'capability--24f9401c-8a77-59d5-8a8f-4ea21a1a733b',
  );
  const manageDataCapabilityPatch = { description: 'Manage data sharing' };
  await elReplace(context, manageDataCapability._index, manageDataCapability.internal_id, {
    doc: manageDataCapabilityPatch,
  });

  // ------ Update name + description + attribute_order of BYPASSREFERENCE
  const byPassRefCapability = await elLoadById(
    context,
    SYSTEM_USER,
    'capability--7ad2a72e-8dcd-569c-8d3f-469dce6fa6b0',
  );
  if (byPassRefCapability) {
    const byPassRefCapabilityPatch = {
      ...
      attribute_order: 320,
      standard_id: generateStandardId(ENTITY_TYPE_CAPABILITY, {
        name: 'KNOWLEDGE_KNUPDATE_KNBYPASSREFERENCE',
      }),
    };
    await elReplace(context, byPassRefCapability._index, byPassRefCapability.internal_id, {
      doc: byPassRefCapabilityPatch,
    });
  }

  // ------ Update attribute_order of Connector API usage
  const connectorAPICapability = await elLoadById(
    context,
    SYSTEM_USER,
    'capability--3b45a9ef-b336-5539-be9a-58e3509648e9',
  );
  const connectorAPICapabilityPatch = { attribute_order: 2300 };
  await elReplace(context, connectorAPICapability._index, connectorAPICapability.internal_id, {
    doc: connectorAPICapabilityPatch,
  });
