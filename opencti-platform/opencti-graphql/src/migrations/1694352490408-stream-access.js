...
export const up = async (next) => {
  ...
  for (let index = 0; index < streams.length; index += 1) {
    ...
    const args = {
      toId: stream.internal_id,
      fromTypes: [ENTITY_TYPE_GROUP],
      toTypes: [ENTITY_TYPE_STREAM_COLLECTION],
    };
  ...
  // ------ Access data sharing
  const accessTaxii = await elLoadById(
    context,
    SYSTEM_USER,
  ...
  // ------ Manage data sharing
  const manageTaxii = await elLoadById(
    context,
    SYSTEM_USER,
  ...
  const streamApi = await elLoadById(
    context,
    SYSTEM_USER,
    'capability--beaa8173-6da3-5930-a0df-00e8feac9d52',
  );
  const capabilityRelations = await fullRelationsList(
    context,
    SYSTEM_USER,
    RELATION_HAS_CAPABILITY,
    { toId: streamApi.internal_id },
  );
  for (let capaIndex = 0; capaIndex < capabilityRelations.length; capaIndex += 1) {
    ...
    await createRelationRaw(context, SYSTEM_USER, {
      fromId: capabilityRelation.fromId,
      relationship_type: RELATION_HAS_CAPABILITY,
      toId: accessTaxii.internal_id,
    });
  }
