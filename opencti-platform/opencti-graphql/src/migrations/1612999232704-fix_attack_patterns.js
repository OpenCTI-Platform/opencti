...
export const up = async (next) => {
  ...
  const concurrentUpdate = async (bulk) => {
    ...
    logApp.info(
      `[OPENCTI] Cleaning aliases and STIX IDs ${currentProcessing} / ${bulkOperations.length}`,
    );
  };
  await Promise.map(groupsOfOperations, concurrentUpdate, { concurrency: ES_MAX_CONCURRENCY });
  logApp.info(
    `[MIGRATION] Cleaning aliases and STIX IDs of attack patterns done in ${new Date() - start} ms`,
  );
  next();
};
