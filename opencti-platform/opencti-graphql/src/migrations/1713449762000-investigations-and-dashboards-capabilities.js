...
export const up = async (next) => {
  ...
  const managePublicDashboardsCapability = await elLoadById(
    context,
    SYSTEM_USER,
    'capability--40892bfe-13c2-5e3e-96a3-531760950451',
  );
  if (managePublicDashboardsCapability) {
    const managePublicDashboardsCapabilityPatch = { description: 'Manage Public Dashboards' };
    await elReplace(
      context,
      managePublicDashboardsCapability._index,
      managePublicDashboardsCapability.internal_id,
      { doc: managePublicDashboardsCapabilityPatch },
    );
  } else {
  ...
  const accessCapability = await elLoadById(
    context,
    SYSTEM_USER,
  ...
  await elReplace(context, accessCapability._index, accessCapability.internal_id, {
    doc: accessCapabilityPatch,
  });
  // ------ Create / Update exploration renaming
  const updateCapability = await elLoadById(
    context,
    SYSTEM_USER,
  ...
  await elReplace(context, updateCapability._index, updateCapability.internal_id, {
    doc: updateCapabilityPatch,
  });
