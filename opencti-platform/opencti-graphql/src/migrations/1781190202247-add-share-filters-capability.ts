...
const message =
  '[MIGRATION] Add KNOWLEDGE_KNSHAREFILTERS capability and grant to roles with KNOWLEDGE';
...
export const up = async (next: (error?: Error) => void) => {
  ...
  const capabilityId = generateStandardId(ENTITY_TYPE_CAPABILITY, {
    name: 'KNOWLEDGE_KNSHAREFILTERS',
  });
  const createRelationPromises = roles.map(async (role) => {
    const roleId = role.id;
    const capabilities = (await roleCapabilities(
      context,
      SYSTEM_USER,
      roleId,
    )) as BasicCapabilityEntity[];
    const hasKnowledgeCapability = capabilities.some(
