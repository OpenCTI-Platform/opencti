...
export const up = async (next) => {
  ...
  const callback = async (roles) => {
    for (let i = 0; i < roles.length; i += 1) {
      ...
      const hasAccessDashboardCapability = capabilities.some(
        (capability) => capability.name === 'EXPLORE',
      );
      if (hasAccessDashboardCapability) {
        const input = {
          fromId: roleId,
          toId: accessInvestigationsCapability.id,
      ...
      const hasCreateUpdateDashboardCapability = capabilities.some(
        (capability) => capability.name === 'EXPLORE_EXUPDATE',
      );
      if (hasCreateUpdateDashboardCapability) {
        const input = {
          fromId: roleId,
          toId: createUpdateInvestigationsCapability.id,
          relationship_type: 'has-capability',
        };
        await createRelation(context, SYSTEM_USER, input);
      }
      // Select 'Delete investigations' if 'Delete dashboard & investigations' is selected
      const hasDeleteDashboardCapability = capabilities.some(
        (capability) => capability.name === 'EXPLORE_EXUPDATE_EXDELETE',
      );
      if (hasDeleteDashboardCapability) {
        const input = {
          fromId: roleId,
          toId: deleteInvestigationsCapability.id,
          relationship_type: 'has-capability',
        };
        await createRelation(context, SYSTEM_USER, input);
      }
  ...
  const accessDashboardCapability = await elLoadById(
    context,
    SYSTEM_USER,
    'capability--c2f6d8be-29c7-5e7f-ab17-dfa14a349025',
  );
  const accessDashboardCapabilityPatch = { description: 'Access dashboards' };
  await elReplace(
    context,
    accessDashboardCapability._index,
    accessDashboardCapability.internal_id,
    { doc: accessDashboardCapabilityPatch },
  );

  // ------ Update description of Create / Update dashboard & investigation => Create / Update dashboard
  const createUpdateDashboardCapability = await elLoadById(
    context,
    SYSTEM_USER,
    'capability--722e8727-5e8a-5b5e-8c1e-3b71b8415170',
  );
  const createUpdateDashboardCapabilityPatch = { description: 'Create / Update dashboards' };
  await elReplace(
    context,
    createUpdateDashboardCapability._index,
    createUpdateDashboardCapability.internal_id,
    { doc: createUpdateDashboardCapabilityPatch },
  );

  // ------ Update description of Delete dashboard & investigation => Delete dashboard
  const deleteDashboardCapability = await elLoadById(
    context,
    SYSTEM_USER,
    'capability--287d57a8-5e9a-573f-8f22-ea321f5cbc90',
  );
  const deleteDashboardCapabilityPatch = { description: 'Delete dashboards' };
  await elReplace(
    context,
    deleteDashboardCapability._index,
    deleteDashboardCapability.internal_id,
    { doc: deleteDashboardCapabilityPatch },
  );
