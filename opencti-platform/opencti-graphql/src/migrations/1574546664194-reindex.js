...
export const up = async (next) => {
  ...
  try {
    await executeWrite((wTx) => {
      ...
      wTx.query(`match $r isa owned_by; delete;`);
    });
    logger.info(`[MIGRATION] reindex > Delete orphan stix_relation`);
    await executeWrite((wTx) => {
      wTx.query(`match $r isa stix_relation; not {$r ($x, $y) isa stix_relation;}; delete;`);
    });
    logger.info(`[MIGRATION] reindex > Delete orphan stix_relation_embedded`);
    await executeWrite((wTx) => {
      wTx.query(`match $r isa stix_relation_embedded; not {$r ($x, $y) isa stix_relation_embedded;}; delete;`);
    });
  } catch (err) {
