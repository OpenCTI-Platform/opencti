...
export const MIGRATION_MAX_BULK_OPERATIONS =
  conf.get('migrations:reindex_targets_rel:max_bulk_operations') || 1000;
export const MIGRATION_BULK_TIMEOUT =
  conf.get('migrations:reindex_targets_rel:bulk_timeout') || '30m';
export const MIGRATION_MAX_CONCURRENCY =
  conf.get('migrations:reindex_targets_rel:max_concurrency') || 2;
...
export const up = async (next) => {
  ...
  const reIndexTargetsRel = async (locations) => {
    ...
    for (let i = 0; i < locations.length; i += 1) {
      logMigration.info(
        `[OPENCTI] Resolving targets relationships ${currentProcessingLocations} / ${locations.length}`,
      );
      const location = locations[i];
      // Resolve targets relationships
      const args = { toId: location.internal_id, withInferences: true };
      const targetsRelationships = await fullRelationsList(
        context,
        SYSTEM_USER,
        RELATION_TARGETS,
        args,
      ...
      const updateQuery = [
        { update: { _index: location._index, _id: location._id, retry_on_conflict: 5 } },
        {
          script: {
            ...
            source:
              "ctx._source['rel_targets.internal_id'] = []; ctx._source['rel_targets.internal_id'].addAll(params['rel_targets.internal_id']);",
          },
        },
      ];
  ...
  const opts = {
    types: [ENTITY_TYPE_LOCATION_REGION, ENTITY_TYPE_LOCATION_COUNTRY, ENTITY_TYPE_IDENTITY_SECTOR],
    logForMigration: true,
    callback: reIndexTargetsRel,
  };
  ...
  const concurrentUpdate = async (bulk) => {
    ...
    logMigration.info(
      `[OPENCTI] Re-indexing targets for region / countries / sectors ${currentProcessing} / ${bulkOperationsTargets.length}`,
    );
  ...
  await Promise.map(groupsOfOperations, concurrentUpdate, {
    concurrency: MIGRATION_MAX_CONCURRENCY,
  });
  logMigration.info(
    `[MIGRATION] Re-indexed targets for region / countries / sectors in ${new Date() - startTargets} ms`,
  );
  next();
};
