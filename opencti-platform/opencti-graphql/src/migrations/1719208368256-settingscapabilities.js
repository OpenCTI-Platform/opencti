...
export const up = async (next) => {
  ...
  const adminCapability = await elLoadById(
    context,
    SYSTEM_USER,
    'capability--bb5ec6d0-0ffb-5b04-8fcf-c0d4447209a6',
  );
  const adminCapabilityPatch = { description: 'Access to admin functionalities' };
  await elReplace(context, adminCapability._index, adminCapability.internal_id, {
    doc: adminCapabilityPatch,
  });
  // ------ Update description of Manage labels & Attributes
  const taxonomiesCapability = await elLoadById(
    context,
    SYSTEM_USER,
    'capability--0f78ff7d-7197-568f-ac8c-5d7842c07f0f',
  );
  const taxonomiesCapabilityPatch = { description: 'Manage taxonomies' };
  await elReplace(context, taxonomiesCapability._index, taxonomiesCapability.internal_id, {
    doc: taxonomiesCapabilityPatch,
  });
  // ------ Update description of Access security activity
  const activityCapability = await elLoadById(
    context,
    SYSTEM_USER,
    'capability--3d467062-f044-50da-902c-627cfa739444',
  );
  const activityCapabilityPatch = { description: 'Access security activity' };
  await elReplace(context, activityCapability._index, activityCapability.internal_id, {
    doc: activityCapabilityPatch,
  });
