...
export const up = async (next) => {
  ...
  const callback = async (files) => {
    ...
    logApp.info(
      `[MIGRATION] (${iteration}) ${files.length} files to register (total: ${filesCount})`,
    );
    ...
    if (elementNotIndexed.length > 0) {
      logApp.error('[MIGRATION] Some files were not indexed, you will need to re-upload them', {
        elementNotIndexed,
      });
    }
    iteration += 1;
  };
  await loadedFilesListing(context, SYSTEM_USER, '', {
    recursive: true,
    dontThrow: true,
    callback,
  });
