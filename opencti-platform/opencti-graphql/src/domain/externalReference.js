...
export const findReferencesPaginated = (context, user, args) => {
  ...
  return pageEntitiesConnection(context, user, [ENTITY_TYPE_EXTERNAL_REFERENCE], {
    ...args,
    filters,
  });
};
...
export const findReferencesForExternalIdPaginated = async (
  context,
  user,
  externalReferenceId,
  args,
...
export const externalReferenceImportPush = async (
  context,
  user,
  externalReferenceId,
  file,
  ...
  const entitiesReferences = await findReferencesForExternalId(context, user, externalReferenceId, {
...
export const addExternalReference = async (context, user, externalReference) => {
  ...
  const isAllowedToByPass =
    userCapabilities.includes(BYPASS) ||
    userCapabilities.includes(KNOWLEDGE_KNUPDATE_KNBYPASSREFERENCE);
  if (!isAllowedToByPass && referenceAttachment && isEmptyField(externalReference.file)) {
    throw ValidationError(
      'You must provide an attachment to create a new external reference',
      'file',
    );
  }
...
export const externalReferenceAddRelation = async (context, user, externalReferenceId, input) => {
  const externalReference = await storeLoadById(
    context,
    user,
    externalReferenceId,
    ENTITY_TYPE_EXTERNAL_REFERENCE,
  ...
  if (!isStixRefRelationship(input.relationship_type)) {
    throw FunctionalError(
      `Only ${ABSTRACT_STIX_REF_RELATIONSHIP} can be added through this method, got ${input.relationship_type}.`,
    );
  }
...
export const externalReferenceDeleteRelation = async (
  context,
  user,
  externalReferenceId,
  fromId,
  ...
  if (!isStixRefRelationship(relationshipType)) {
    throw FunctionalError(
      `Only ${ABSTRACT_STIX_REF_RELATIONSHIP} can be deleted through this method, got ${relationshipType}.`,
    );
  }
...
export const externalReferenceEditField = async (
  context,
  user,
  externalReferenceId,
  input,
  ...
  const currentReference = await storeLoadById(
    context,
    user,
    externalReferenceId,
    ENTITY_TYPE_EXTERNAL_REFERENCE,
...
export const externalReferenceCleanContext = async (context, user, externalReferenceId) => {
  ...
  return storeLoadById(context, user, externalReferenceId, ENTITY_TYPE_EXTERNAL_REFERENCE).then(
    (externalReference) => {
      return notify(BUS_TOPICS[ENTITY_TYPE_EXTERNAL_REFERENCE].EDIT_TOPIC, externalReference, user);
    },
  );
