...
export const addThreatActorGroup = async (context, user, threatActorGroup) => {
  const threatActorGroupToCreate = pipe(
    assoc(
      'first_seen',
      isNil(threatActorGroup.first_seen) ? new Date(FROM_START) : threatActorGroup.first_seen,
    ),
    assoc(
      'last_seen',
      isNil(threatActorGroup.last_seen) ? new Date(UNTIL_END) : threatActorGroup.last_seen,
    ),
  )(threatActorGroup);
  const created = await createEntity(
    context,
    user,
    threatActorGroupToCreate,
