...
export const findRefRelationshipsPaginated = async (context, user, args) => {
  return pageRelationsConnection(
    context,
    user,
    args.relationship_type ?? STIX_REF_RELATIONSHIP_TYPES,
    args,
  );
};
...
export const findNestedPaginated = async (context, user, args) => {
  const relationTypes = schemaTypesDefinition
    .get(ABSTRACT_STIX_REF_RELATIONSHIP)
    .filter((type) => !notNestedRefRelation.includes(type));
...
export const schemaRefRelationships = async (context, user, id, toType) => {
  return findStixObjectOrStixRelationshipById(context, user, id).then((entity) => {
    const from = schemaRelationsRefDefinition
      .getRelationsRef(entity.entity_type)
      .filter((ref) => !notNestedRefRelation.includes(ref.databaseName))
      .filter((ref) =>
        !ref.isRefExistingForTypes ? true : ref.isRefExistingForTypes(entity.entity_type, toType),
      )
      .map((ref) => ref.databaseName)
      .sort();
    const to = schemaRelationsRefDefinition
      .getRelationsRef(toType)
      .filter((ref) => !notNestedRefRelation.includes(ref.databaseName))
      .filter((ref) =>
        !ref.isRefExistingForTypes ? true : ref.isRefExistingForTypes(toType, entity.entity_type),
      )
      .map((ref) => ref.databaseName)
      .sort();
...
export const schemaRefRelationshipsPossibleTypes = async (context, user, entityType) => {
  ...
  const possibleFromTypes = registeredTypes.filter((type) => {
    const reversedRelationRefs = schemaRelationsRefDefinition
      .getRelationsRef(type)
      .filter((ref) => !notNestedRefRelation.includes(ref.databaseName))
      .filter((ref) =>
        !ref.isRefExistingForTypes ? true : ref.isRefExistingForTypes(type, entityType),
      );
  ...
  if (cleanedPossibleTypes.includes(ABSTRACT_STIX_CORE_OBJECT)) {
    ...
    if (cleanedPossibleTypes.includes(ABSTRACT_STIX_CYBER_OBSERVABLE)) {
      cleanedPossibleTypes = cleanedPossibleTypes.filter(
        (t) => !scos.edges.map((n) => n.node.id).includes(t),
      );
    }
    if (cleanedPossibleTypes.includes(ABSTRACT_STIX_DOMAIN_OBJECT)) {
      cleanedPossibleTypes = cleanedPossibleTypes.filter(
        (t) => !sdos.edges.map((n) => n.node.id).includes(t),
      );
    }
  }
...
export const addStixRefRelationship = async (context, user, stixRefRelationship) => {
  if (!isStixRefRelationship(stixRefRelationship.relationship_type)) {
    throw FunctionalError(
      `Only ${ABSTRACT_STIX_REF_RELATIONSHIP} can be added through this method, got ${stixRefRelationship.relationship_type}.`,
    );
  }
...
export const stixRefRelationshipEditField = async (context, user, stixRefRelationshipId, input) => {
  // Not use ABSTRACT_STIX_REF_RELATIONSHIP to have compatibility on parent type with ABSTRACT_STIX_CYBER_OBSERVABLE_RELATIONSHIP type
  const stixRefRelation = await storeLoadById(
    context,
    user,
    stixRefRelationshipId,
    ABSTRACT_STIX_RELATIONSHIP,
  );
  if (!stixRefRelation) {
    throw FunctionalError('Cannot delete the relation, Stix-Ref-Relation cannot be found.', {
      id: stixRefRelationshipId,
    });
  }
...
export const stixRefRelationshipDelete = async (context, user, stixRefRelationshipId) => {
  ...
  await deleteElementById(
    ...
    user,
    stixRefRelationshipId,
    stixRefRelationship.relationship_type,
  );
...
export const stixRefRelationshipEditContext = async (
  context,
  user,
  stixRefRelationshipId,
  input,
