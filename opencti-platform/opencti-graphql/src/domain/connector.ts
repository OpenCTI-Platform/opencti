...
export const connectorForWork = async (context: AuthContext, user: AuthUser, id: string) => {
  const work = await elLoadById<Work>(context, user, id, {
...
export const computeWorkStatus = async (work: Work) => {
  if (work.status === 'complete') {
    return {
      import_processed_number: work.completed_number,
      import_expected_number: work.import_expected_number,
    };
  }
...
export const pingConnector = async (
  ...
  const connectorEntity = (await storeLoadById(
    context,
    user,
    id,
    ENTITY_TYPE_CONNECTOR,
  )) as unknown as BasicStoreEntityConnector;
  if (!connectorEntity) {
...
export const resetStateConnector = async (context: AuthContext, user: AuthUser, id: string) => {
  ...
  const { element } = await patchAttribute<BasicStoreEntityConnector>(
    context,
    user,
    id,
    ENTITY_TYPE_CONNECTOR,
  ...
  return storeLoadById(context, user, id, ENTITY_TYPE_CONNECTOR).then((data) =>
    completeConnector(data),
  );
};
...
export const registerConnectorsManager = async (
  context: AuthContext,
  user: AuthUser,
  ...
  if (connectorManagers.length > 0) {
    throw UnsupportedError('Only one connector manager is supported per instance', {
      id: input.id,
    });
  }
...
export const updateConnectorManagerStatus = async (
  context: AuthContext,
  user: AuthUser,
...
export const managedConnectorAdd = async (
  ...
  if (input.automatic_user) {
    const onTheFlyCreatedUser = await createOnTheFlyUser(context, user, {
      ...
      serviceAccount: true,
      confidenceLevel: input.confidence_level ? parseInt(input.confidence_level, 10) : null,
    });
  ...
  if (nameCollision) {
    logApp.info(
      `[CONNECTOR] Name collision detected: connector with name '${sanitizedName}' already exists`,
    );
  ...
  const createdConnector: any = await createEntity(
    context,
    user,
    connectorToCreate,
    ENTITY_TYPE_CONNECTOR,
...
export const registerConnector = async (
  ...
  const createdConnector = await createEntity(
    context,
    user,
    connectorToCreate,
    ENTITY_TYPE_CONNECTOR,
...
export const connectorDelete = async (
  context: AuthContext,
  user: AuthUser,
  connectorId: string,
) => {
  ...
  const { element } = await internalDeleteElementById<BasicStoreEntityConnector>(
    context,
    user,
    connectorId,
    ENTITY_TYPE_CONNECTOR,
...
const updateConnector = async (
  ...
  const { element } = await updateAttribute<BasicStoreEntityConnector>(
    context,
    user,
    connectorId,
    ENTITY_TYPE_CONNECTOR,
...
export const connectorUpdateLogs = async (
...
export const connectorUpdateHealth = async (
  _context: AuthContext,
  _user: AuthUser,
...
export const connectorGetHealth = async (
  _context: AuthContext,
  _user: AuthUser,
  connectorId: string,
...
export const connectorGetUptime = async (
  context: AuthContext,
  user: AuthUser,
  connectorId: string,
): Promise<number | null> => {
...
export const updateConnectorRequestedStatus = async (
  context: AuthContext,
  user: AuthUser,
...
export const updateConnectorCurrentStatus = async (
  context: AuthContext,
  user: AuthUser,
...
export const connectorTriggerUpdate = async (
  context: AuthContext,
  user: AuthUser,
  connectorId: string,
  input: EditInput[],
) => {
  const conn = (await storeLoadById(
    context,
    user,
    connectorId,
    ENTITY_TYPE_CONNECTOR,
  ...
  if (!conn) {
    throw FunctionalError('Cant find element to update', {
      id: connectorId,
      type: ENTITY_TYPE_CONNECTOR,
    });
  }
  if (!['INTERNAL_ENRICHMENT', 'INTERNAL_IMPORT_FILE'].includes(conn.connector_type)) {
    throw FunctionalError(
      'Update is only possible on internal enrichment or import file connectors types',
      { connectorId },
    );
  }
  ...
  if (input.some((item) => !supportedInputKeys.includes(item.key))) {
    throw FunctionalError(
      `Update is only possible on these input keys: ${supportedInputKeys.join(', ')}`,
      { connectorId },
    );
  }
  const filtersItem: EditInput | undefined = input.find(
    (item: EditInput) => item.key === 'connector_trigger_filters',
  );
...
export const registerConnectorForIngestion = async (
  context: AuthContext,
...
export const fetchRemoteStreams = async (
  context: AuthContext,
  user: AuthUser,
  ...
  `;
    ...
    if (e instanceof AxiosError) {
      logApp.error('[OPENCTI-MODULE] Issue when trying to call OpenCTI remote stream', {
        httpStatus: e.status,
        message: e.message,
        streamURI: uri,
      });
    ...
    throw ValidationError('Error getting the streams from remote OpenCTI', 'uri', {
      cause: errorMessage,
    });
  }
};
...
export const syncAddInputFromImport = async (file: Promise<FileHandle>) => {
  ...
  if (
    !isCompatibleVersionWithMinimal(
      ...
      MINIMAL_SYNCHRONIZER_COMPATIBLE_VERSION,
    )
  ) {
    throw FunctionalError(
...
export const synchronizerAddAutoUser = async (
  context: AuthContext,
  user: AuthUser,
  ...
  return syncEditField(context, user, synchronizerId, [
    { key: 'user_id', value: [onTheFlyCreatedUser.id] },
  ]);
};

export const syncEditField = async (
  context: AuthContext,
  user: AuthUser,
  syncId: string,
  input: EditInput[],
...
export const syncCleanContext = async (context: AuthContext, user: AuthUser, syncId: string) => {
  ...
  return storeLoadById(context, user, syncId, ENTITY_TYPE_SYNC).then((syncToReturn) =>
    notify(BUS_TOPICS[ENTITY_TYPE_SYNC].EDIT_TOPIC, syncToReturn, user),
  );
};
export const syncEditContext = async (
  context: AuthContext,
  user: AuthUser,
  syncId: string,
  input: EditContext,
...
export const createDraftAndAskJobImport = async (
  ...
  return askJobImport({ ...context, draft_context: id }, user, {
    fileName,
    connectorId,
    configuration,
    bypassEntityId,
