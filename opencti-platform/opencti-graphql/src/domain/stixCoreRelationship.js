...
const buildStixCoreRelationshipTypes = (relationshipTypes) => {
  ...
  if (!isValidRelationshipTypes) {
    ...
    throw new GraphQLError(
      'Invalid argument: relationship_type is not a stix-core-relationship',
      options,
    );
  }
...
export const stixCoreRelationshipsNumber = async (context, user, args) => {
  ...
  if (isNotEmptyField(authorId)) {
    const filters = addFilter(
      args.filters,
      buildRefRelationKey(RELATION_CREATED_BY, '*'),
      authorId,
    );
  ...
  const indices = args.onlyInferred
    ? [READ_INDEX_INFERRED_RELATIONSHIPS]
    : [READ_INDEX_STIX_CORE_RELATIONSHIPS, READ_INDEX_INFERRED_RELATIONSHIPS];
  return {
...
export const stixCoreRelationshipsExportAsk = async (context, user, args) => {
  ...
  const transformFilters = await exportTransformFilters(
    context,
    user,
    argsFilters,
    ordersOpts,
...
export const stixCoreRelationshipDeleteByFromAndTo = async (
  context,
  user,
  fromId,
  toId,
  ...
  if (!isStixCoreRelationship(relationshipType)) {
    throw FunctionalError(
      `Only stix-core-relationship can be deleted through this method, not ${relationshipType}.`,
    );
  }
...
export const stixCoreRelationshipEditField = async (
  context,
  user,
  stixCoreRelationshipId,
  input,
  ...
  const stixCoreRelationship = await storeLoadById(
    context,
    user,
    stixCoreRelationshipId,
    ABSTRACT_STIX_CORE_RELATIONSHIP,
...
// region relation ref
export const stixCoreRelationshipAddRelation = async (
  context,
  user,
  stixCoreRelationshipId,
  input,
...
export const stixCoreRelationshipAddRelations = async (
  context,
  user,
  stixCoreRelationshipId,
  input,
...
export const stixCoreRelationshipDeleteRelation = async (
  context,
  user,
  stixCoreRelationshipId,
  toId,
...
export const stixCoreRelationshipEditContext = async (
  context,
  user,
  stixCoreRelationshipId,
  input,
...
export const stixCoreRelationshipRemoveFromDraft = async (context, user, stixCoreObjectId) => {
  ...
  if (!stixCoreRelationship) {
    throw FunctionalError(
      'Cannot remove the object from draft, stix-core-relationship cannot be found.',
      { id: stixCoreObjectId },
    );
  }
