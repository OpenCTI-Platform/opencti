...
export const labelEditField = (user, tagId, input) => {
  return executeWrite((wTx) => {
    ...
    return notify(BUS_TOPICS[ENTITY_TYPE_LABEL].EDIT_TOPIC, tag, user);
  });
};

export const labelCleanContext = async (user, tagId) => {
  await delEditContext(user, tagId);
  return loadEntityById(tagId, ENTITY_TYPE_LABEL).then((tag) =>
    notify(BUS_TOPICS[ENTITY_TYPE_LABEL].EDIT_TOPIC, tag, user)
  );
};
