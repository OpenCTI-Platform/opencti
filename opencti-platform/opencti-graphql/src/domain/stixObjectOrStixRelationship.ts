...
import {
  ...
  type StixRefRelationshipsAddInput,
...
export const findById = async <T extends BasicStoreObject>(
  context: AuthContext,
  user: AuthUser,
  id: string,
): Promise<T> => {
...
export const findStixObjectOrRelationshipsPaginated = async <T extends BasicStoreObject>(
  context: AuthContext,
  user: AuthUser,
  ...
  return (await pageEntitiesOrRelationsConnection(
    context,
    user,
    [ABSTRACT_STIX_OBJECT, ABSTRACT_STIX_RELATIONSHIP],
    args,
  )) as unknown as BasicConnection<T>;
};
...
const patchElementWithRefRelationships = async (
  ...
  const fieldName = schemaRelationsRefDefinition.convertDatabaseNameToInputName(
    initial.entity_type,
    relationship_type,
  );
  if (!fieldName) {
  ...
  const { element: patchedFrom } = await updateAttributeFromLoadedWithRefs(
    context,
    user,
    initial,
    inputs,
...
export const stixObjectOrRelationshipAddRefRelation = async (
  ...
  // TODO remove any when all resolvers in ts
  ...
  const patchedFrom = await patchElementWithRefRelationships(
    context,
    user,
    stixObjectOrRelationshipId,
    type,
  ...
  const { element: refRelation } = await buildRelationData(context, user, {
    from: patchedFrom,
    to,
...
export const stixObjectOrRelationshipAddRefRelations = async (
  ...
  return patchElementWithRefRelationships(
    context,
    user,
    stixObjectOrRelationshipId,
    type,
...
export const stixObjectOrRelationshipDeleteRefRelation = async (
  ...
  // TODO remove any when all resolvers in ts
  const stixObjectOrRelationship = await storeLoadById(
    context,
    user,
    stixObjectOrRelationshipId,
    type,
  ...
  if (!stixObjectOrRelationship) {
    throw FunctionalError(
      'Cannot delete the relation, Stix-Object or Stix-Relationship cannot be found.',
      { id: stixObjectOrRelationshipId },
    );
  }
  if (!isStixRefRelationship(relationshipType)) {
    throw FunctionalError(
      `Only ${ABSTRACT_STIX_REF_RELATIONSHIP} can be deleted through this method.`,
      { id: stixObjectOrRelationshipId },
    );
  }
  return patchElementWithRefRelationships(
