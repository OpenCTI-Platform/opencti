  ...
  if (isStixCyberObservableHashedObservable(entityType)) {
    if (observable.hashes) {
      ...
      if (observable.hashes['SHA-512']) {
        ...
        value =
          value.length > 0
            ? `${value}__${observable.hashes['SHA-512']}`
            : observable.hashes['SHA-512'];
      }
      if (observable.hashes['SHA-1']) {
        key = key.length > 0 ? `${key}__${entityType}_sha1` : `${entityType}_sha1`;
        value =
          value.length > 0 ? `${value}__${observable.hashes['SHA-1']}` : observable.hashes['SHA-1'];
      }
    ...
    if (observable.fingerprint_md5) {
      key = key.length > 0 ? `${key}__${entityType}_md5` : `${entityType}_md5`;
      value =
        value.length > 0 ? `${value}__${observable.fingerprint_md5}` : observable.fingerprint_md5;
    }
...
export const promoteObservableToIndicator = async (context, user, observableId) => {
  ...
  if (!observable) {
    throw new GraphQLError('Observable not found', {
      id: observableId,
  ...
  return await createIndicatorFromObservable(
    context,
    user,
...
// region relation ref
export const stixCyberObservableAddRelation = async (
  context,
  user,
...
export const stixCyberObservableEditField = async (
  context,
  user,
  stixCyberObservableId,
  input,
  ...
  const originalStixCyberObservable = await storeLoadById(
    context,
    user,
    stixCyberObservableId,
    ABSTRACT_STIX_CYBER_OBSERVABLE,
  ...
  if (scoreInput) {
    ...
    await Promise.all(
      indicators.map((indicator) =>
        updateAttribute(context, user, indicator.id, ENTITY_TYPE_INDICATOR, [scoreInput], opts),
      ),
    );
  }
...
export const stixCyberObservableCleanContext = async (context, user, stixCyberObservableId) => {
  ...
  const stixCyberObservable = await storeLoadById(
    context,
    user,
    stixCyberObservableId,
    ABSTRACT_STIX_CYBER_OBSERVABLE,
...
export const stixCyberObservableEditContext = async (
  context,
  user,
  stixCyberObservableId,
  input,
...
export const stixCyberObservablesExportAsk = async (context, user, args) => {
  ...
  const observableContext = {
    ...exportContext,
    entity_type: exportContext.entity_type ?? 'Stix-Cyber-Observable',
  };
...
const extractInfectedZipFile = async (file) => {
  ...
  return {
    createReadStream: () => Readable.from(extracted),
...
export const artifactImport = async (context, user, args) => {
  ...
  const isOfficeXmlExtension = office_xml_extensions.some((ext) =>
    resolvedFile.filename?.toLowerCase().endsWith(ext),
  );
  ...
  await uploadToStorage(
    context,
    user,
    `import/${artifact.entity_type}/${artifact.id}`,
    resolvedFile,
    { entity: artifact, meta, noTriggerImport: true },
  );
  return artifact;
};
