...
export const findAttackPatternPaginated = (
  context: AuthContext,
  user: AuthUser,
...
export const addAttackPattern = async (
  context: AuthContext,
  user: AuthUser,
  ...
  const attackPatternToCreate = xMitreId
    ? R.assoc('x_mitre_id', xMitreId, attackPattern)
    : attackPattern;
  const created = await createEntity(
    context,
    user,
    attackPatternToCreate,
    ENTITY_TYPE_ATTACK_PATTERN,
  );
  return notify(BUS_TOPICS[ABSTRACT_STIX_DOMAIN_OBJECT].ADDED_TOPIC, created, user);
};

export const parentAttackPatternsPaginated = async (
  context: AuthContext,
  user: AuthUser,
...
export const childAttackPatternsPaginated = async (
  context: AuthContext,
  user: AuthUser,
  attackPatternId: string,
  args: EntityOptions<BasicStoreCommon>,
...
export const batchIsSubAttackPattern = async (
  context: AuthContext,
  user: AuthUser,
  attackPatternsIds: string[],
) => {
  const resultIds = await findEntitiesIdsWithRelations(
    context,
    user,
    attackPatternsIds,
    RELATION_SUBTECHNIQUE_OF,
...
export const coursesOfActionPaginated = async (
  context: AuthContext,
  user: AuthUser,
  attackPatternId: string,
  args: EntityOptions<BasicStoreCommon>,
...
export const dataComponentsPaginated = async (
  context: AuthContext,
  user: AuthUser,
  attackPatternId: string,
  args: EntityOptions<BasicStoreCommon>,
...
export const getAttackPatternsMatrix = async (context: AuthContext, user: AuthUser) => {
  ...
  const allAttackPatterns = await fullEntitiesList(
    context,
    user,
    [ENTITY_TYPE_ATTACK_PATTERN],
    attackPatternsArgs,
  ...
  const allKillChainPhases = await fullEntitiesList(context, user, [ENTITY_TYPE_KILL_CHAIN_PHASE], {
    indices: [READ_INDEX_STIX_META_OBJECTS],
  });
  const subTechniquesRelations = await fullRelationsList<BasicStoreRelation>(
    context,
    user,
    RELATION_SUBTECHNIQUE_OF,
  );
  ...
  const subTechniquesByParentId = new Map<
    string,
    { attack_pattern_id: string; name: string; description?: string }[]
  >();
  ...
  for (const s of subTechniquesRelations) {
    ...
    if (subAP) {
      ...
      subTechniquesByParentId
        .get(s.toId)!
        .push({ attack_pattern_id: subAP.id, name: subAP.name, description: subAP.description });
      searchTextPartsByParentId
        .get(s.toId)!
        .push(`${subAP.x_mitre_id} ${subAP.name} ${subAP.description}`);
    }
  }
