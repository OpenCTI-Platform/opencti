...
export const findTemplateById = (
  ...
  ) as unknown as StatusTemplate;
};
export const findTemplatePaginated = async (
  context: AuthContext,
  user: AuthUser,
  args: QueryStatusTemplatesArgs,
) => {
  return pageEntitiesConnection<BasicStoreEntity>(
    context,
    user,
    [ENTITY_TYPE_STATUS_TEMPLATE],
    args,
...
export const findAllTemplatesByStatusScope = async (
  context: AuthContext,
  user: AuthUser,
  args: QueryStatusTemplatesByStatusScopeArgs,
) => {
  ...
  return storeLoadByIds<BasicWorkflowStatus>(
    context,
    user,
    templateIds,
    ENTITY_TYPE_STATUS_TEMPLATE,
...
export const findById = async (
  ...
  return (
    ...
    ((await storeLoadById(
      ...
      statusId,
      ENTITY_TYPE_STATUS,
    )) as unknown as BasicWorkflowStatus)
  );
};
export const findByType = async (
  ...
  user: AuthUser,
  statusType: string,
): Promise<Array<BasicWorkflowStatus>> => {
  const platformStatuses = await getEntitiesListFromCache<BasicWorkflowStatus>(
    context,
    user,
    ENTITY_TYPE_STATUS,
...
export const findStatusPaginated = (
  context: AuthContext,
  user: AuthUser,
  args: QueryStatusesArgs,
) => {
...
export const batchRequestAccessStatusesByType = async (
  context: AuthContext,
  user: AuthUser,
  types: string[],
) => {
  const batchStatusesByTypeFn = async () => {
    const argsFilter = {
      ...
      filters: {
        mode: FilterMode.And,
        filters: [
          { key: ['type'], values: types },
          { key: ['scope'], values: [StatusScope.RequestAccess] },
        ],
        filterGroups: [],
      },
    };
    const statuses = await fullEntitiesList<BasicWorkflowStatus>(
      context,
      user,
      [ENTITY_TYPE_STATUS],
      argsFilter,
...
export const batchGlobalStatusesByType = async (
  context: AuthContext,
  user: AuthUser,
  types: string[],
) => {
...
export const createStatusTemplate = async (
  context: AuthContext,
  user: AuthUser,
  input: StatusTemplateAddInput,
) => {
  const { element } = await createEntity(context, user, input, ENTITY_TYPE_STATUS_TEMPLATE, {
    complete: true,
  });
...
export const statusEditField = async (
  context: AuthContext,
  user: AuthUser,
  subTypeId: string,
  statusId: string,
...
export const statusTemplateEditField = async (
  context: AuthContext,
  user: AuthUser,
  statusTemplateId: string,
  input: EditInput[],
) => {
  const { element } = await updateAttribute<StoreEntity>(
    context,
    user,
    statusTemplateId,
    ENTITY_TYPE_STATUS_TEMPLATE,
...
export const statusDelete = async (
  context: AuthContext,
  user: AuthUser,
  subTypeId: string,
  statusId: string,
  ...
  validateSetting(subTypeId, 'workflow_configuration');
  const { element: deleted } = await internalDeleteElementById(
    context,
    user,
    statusId,
    ENTITY_TYPE_STATUS,
...
export const statusTemplateDelete = async (
  context: AuthContext,
  user: AuthUser,
  statusTemplateId: string,
) => {
  ...
  await Promise.all(
    result.map((status) =>
      internalDeleteElementById(context, user, status.id, ENTITY_TYPE_STATUS).then(({ element }) =>
        notify(BUS_TOPICS[ABSTRACT_INTERNAL_OBJECT].DELETE_TOPIC, element, user),
      ),
    ),
  );
  const deleted = await deleteElementById<StoreEntity>(
    context,
    user,
    statusTemplateId,
    ENTITY_TYPE_STATUS_TEMPLATE,
...
export const statusTemplateEditContext = async (
  context: AuthContext,
  user: AuthUser,
  statusTemplateId: string,
  input: EditContext,
  ...
  return storeLoadById(context, user, statusTemplateId, ENTITY_TYPE_STATUS_TEMPLATE).then(
    (statusTemplate) => {
      return notify(BUS_TOPICS[ABSTRACT_INTERNAL_OBJECT].EDIT_TOPIC, statusTemplate, user);
    },
  );
};
export const statusTemplateCleanContext = async (
  context: AuthContext,
  user: AuthUser,
  statusTemplateId: string,
) => {
...
export const statusTemplateUsagesNumber = async (
  context: AuthContext,
  user: AuthUser,
  statusTemplateId: string,
) => {
...
export const isGlobalWorkflowEnabled = async (
  context: AuthContext,
  user: AuthUser,
  subTypeId: string,
) => {
  const entityStatusFromCache = await findByType(context, user, subTypeId);
  const globalStatuses = entityStatusFromCache.filter(
    (status) => status.scope === StatusScope.Global,
  );
