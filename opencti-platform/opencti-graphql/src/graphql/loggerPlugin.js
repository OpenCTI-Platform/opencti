  ...
  requestDidStart: /* v8 ignore next */ () => {
    ...
    return {
      willSendResponse: async (context) => {
        ...
        const isCallError =
          context.errors && context.errors.length > 0 && !isMutedError(context.errors[0]);
        ...
        if (isWrite) {
          ...
          if (input) {
            ...
            if (!isNil(input.markingDefinitions))
              innerRelationCount += innerCompute(input.markingDefinitions);
            if (!isNil(input.labels)) innerRelationCount += innerCompute(input.labels);
            if (!isNil(input.killChainPhases))
              innerRelationCount += innerCompute(input.killChainPhases);
            if (!isNil(input.objectRefs)) innerRelationCount += innerCompute(input.objectRefs);
            if (!isNil(input.observableRefs))
              innerRelationCount += innerCompute(input.observableRefs);
        ...
        if (appLogExtendedErrors) {
          ...
          callMetaData.operation_query = stripIgnoredCharacters(
        ...
        if (isCallError) {
          ...
          if (
            callError.extensions?.code &&
            Object.keys(ApolloServerErrorCode).includes(callError.extensions.code)
          ) {
            const data = { source: callError.message, ...callError.extensions };
            callError = ValidationError(
              '[OPENCTI] API GraphQL framework error',
              callError.extensions?.field,
              data,
            );
          }
