...
export const subscribeToAiEvents = async (
  context: any,
  id: string,
  topics: string | string[],
): Promise<AsyncIterable<any>> => {
...
export const subscribeToPlatformSettingsEvents = async (
  context: any,
): Promise<AsyncIterable<any>> => {
  ...
  const filtering = await withFilter(
    ...
    (payload) => {
      ...
      const oldMessages: BasicStoreSettingsMessage[] = getMessagesFilteredByRecipients(
        context.user,
        settings,
      );
      const newMessages: BasicStoreSettingsMessage[] = getMessagesFilteredByRecipients(
        context.user,
        payload.instance,
      );
