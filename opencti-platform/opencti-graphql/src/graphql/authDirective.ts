    ...
    authDirectiveTransformer: (schema: GraphQLSchema) =>
      mapSchema(schema, {
        ...
        [MapperKind.OBJECT_FIELD]: (
          ...
          const authDirective =
            (directive?.[0] as AuthDirectiveArgs | undefined) ??
          ...
          if (!authDirective && (typeName === TYPE_QUERY || typeName === TYPE_MUTATION)) {
            const publicDirective = getDirective(
              schema,
              fieldConfig,
              PUBLIC_PROTECT_DIRECTIVE,
            )?.[0];
            if (!publicDirective) {
              throw UnsupportedError('Unsecure schema: missing auth or public directive', {
                field: _fieldName,
              });
            }
          }
          ...
          if (authDirective) {
            const {
              for: requiredCapabilitiesBase,
              and: requiredAll,
              forDraft: requiredCapabilitiesInDraft,
            } = authDirective;
            ...
            if (requiredCapabilitiesBase || requiredCapabilitiesInDraft) {
              ...
              fieldConfig.resolve = (source: any, args: any, context: AuthContext, info: any) => {
                ...
                const { user, otp_mandatory, user_otp_validated, blocked_for_lts_validation } =
                  context;
                // User must be authenticated.
                if (!user) {
                  throw AuthRequired();
                }
                const allowUnprotectedOTP = !!getDirective(
                  schema,
                  fieldConfig,
                  OTP_PROTECT_DIRECTIVE,
                )?.[0];
                ...
                const allowUnlicensedLTS = !!getDirective(
                  schema,
                  fieldConfig,
                  LTS_PROTECT_DIRECTIVE,
                )?.[0];
                ...
                if (
                  requiredCapabilitiesBase.length === 0 &&
                  requiredCapabilitiesInDraft?.length === 0
                ) {
                ...
                const shouldBypass =
                  userBaseCapabilities.includes(BYPASS) || user.id === OPENCTI_ADMIN_UUID;
                if (shouldBypass) {
                  return resolve(source, args, context, info);
                }

                // Check base capabilities
                const baseGranted = checkCapabilities(
                  requiredCapabilitiesBase,
                  userBaseCapabilities,
                  !!requiredAll,
                );
                ...
                const draftGranted = requiredCapabilitiesInDraft?.length
                  ? checkCapabilities(
                      requiredCapabilitiesInDraft,
                      userCapabilitiesInDraft,
                      !!requiredAll,
                    )
                  : false;
                ...
                if (
                  typeName === ENTITY_TYPE_IDENTITY_ORGANIZATION &&
